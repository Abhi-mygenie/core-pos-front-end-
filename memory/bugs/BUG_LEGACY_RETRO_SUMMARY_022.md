# BUG Legacy Retro Summary — BUG-022
...
Cancelled Item — Not Shown as Strikethrough in Collect Bill Page "ITEMS" List (Order page correctly strikes it through)

## 3. Original April-2026 fix narrative (from master tracker)

FIXED (Apr-2026) — `CollectPaymentPanel.jsx` main Bill Summary and room-service items loops now compute `isCancelled = item.status === 'cancelled'` and apply strikethrough + gray (`#9CA3AF`) to item name and price plus a "(Cancelled)" label, matching `CartPanel.jsx`. Complimentary checkbox disabled for cancelled lines. No math / payload / socket changes.
...
`components/order-entry/CollectPaymentPanel.jsx` (main items loop + room-service items loop)
...
- Master tracker: `/app/memory/BUG_TEMPLATE.md` (row for BUG-022)
...
— End of BUG-022 Legacy Retro Summary —
