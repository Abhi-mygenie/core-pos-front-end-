...
## 11. Post-Session Owner Decisions (2026-08-27)

### Bill Content + Bill Style — `/bill-printer-config` endpoint confirmed live

**Full contract probed 2026-08-27:**
- GET returns `data.configs` with 3 keys: `"58mm"`, `"80mm"`, `"windows"`
- Android (58mm/80mm): section arrays = `[height, width, bold]`
- Windows: section arrays = `[height, bold]`
- Bill Content toggles (`print_phone`, `print_email`, `dotted_line_between_item`, `total_amount_bold`, `total_amount_placed_center`, `total_amount_in_word`) + dims (`padding`, `margin`, `paperwidth`) stored inside EACH config
- `show_address_on_bill` + `footer_text` NOT in `/bill-printer-config` → saved via `POST /update-settings` `{"basic":{...}}`

**OD-1 CONFIRMED — Bill Content save behaviour:**
> Owner confirmed: **Option A — Save same toggle values to ALL 3 configs (58mm + 80mm + windows) simultaneously.**
> Bill Content tab shows one global set of toggles — no paper-size sub-tabs.
> On save: POST /bill-printer-config for each bill_size with identical toggle values.

**Evidence:** `/app/memory/evidence/CR-LOCAL-PRINTER/OWNER_DECISIONS.md`
**Still open:** Q1 (batch POST vs 3 separate calls), Q2 (GET source for show_address + footer_text), Q3 (global vs per-size for those 2 fields)
