...
| OG-PMS-030 | **P1** | **OPEN — backend D15 (money)** | `room-extend-stay` shorten re-prices the remaining night(s) at the blended `rate_per_night` instead of the sold per-night rates (9,500 vs 10,100; drift compounds on re-extend). `evidence/CR-385/probes_2026_09_20_final/x3_shorten.json`. | Backend (BACKEND_BRIEF_CR-385_2026-09-20_FINAL_PACK) |
| OG-PMS-031 | **P1** | **OPEN — backend D16 (money)** | `room-extend-stay` with `new_restaurant_table_id` (same type/dates) recomputes GST at a single slab (2,188 → 3,150), losing N11 per-night slabs. `s5_move.json`. | Backend (same brief) |
| OG-PMS-032 | P2 | **OPEN — backend ask BQ-385-20** | `direct-reservation.advance` accepts `method:"split"` + `split_payments[]` but stores/echoes only the lump sum; legs/method/txn not confirmed. FE ships single-method advance until answered (OD-385-18 a). | Backend |
