...
**Status:** POPULATED
**Last Updated:** 2026-09-02 (CR-357 advance payment decisions frozen; OD-7 backend open)
...
| OG-PMS-010 | P2 | **OPEN** | After owner checkout of r2 (walk-in) on 2026-09-03, `room-status-board` shows `manual_status: null`, `display_status: booked` although `auto_hk_on_rm_checkout=true` — auto-HK did not fire or is overridden by booked precedence. **Reproduced 2nd time on r5 checkout (AIOSELL-linked) → `available`, `manual_status: null`** — likely backend bug. Verify at Phase 4 (S7) planning; may be backend bug. Evidence `evidence/CR-358-P3/probe_20_board_after_checkout.json`. | CR-358-P4 |
...
| OG-PMS-013 | P2 | **OPEN** (filed 2026-09-04, INV-PMS-ENH) | `room-status-board` **soft-allocates** unassigned pending bookings to physical rooms by room type (r4→`BDC7497606`, r5→`BDC6263973`, r1→`MG-69-8859D21E…` shown `display_status: booked` with `reservation.reservation_room_id`) while `local-reservations` has `rooms[].restaurant_table_id = null` for the same lines. Tape Chart (LR = truth) shows them *Unassigned*; Status Board shows them *Booked on a specific room*. Contradictory screens; blocks a clean CR-361 contract. Evidence `evidence/INV-PMS-ENH/probe_01_local_reservations.json` vs `probe_02_board.json`. | CR-361 Gate 2 — backend ruling B-361-02 (board must honour explicit assignment; fall back to type-level count only) |
| OG-PMS-014 | P1 | **OPEN** (filed 2026-09-06, INV-PMS-CRs-363-364-366) | `POST /api/v2/vendoremployee/pos/room-payment` returns **403 Forbidden** for owner@thegoankitchen.com on sandbox-pms (v1 path 404). Blocks CR-364 "Record Payment" action; read-only folio unaffected. Need backend ruling: permission gap on sandbox role vs intentional. Evidence `evidence/INV-PMS-CRs-363-364-366/`. | CR-364 Gate 2 |
| OG-PMS-015 | P2 | **OPEN** (filed 2026-09-06, INV-PMS-CRs-363-364-366) | No `no_show` status/count exposed on `dashboard-kpis` or `local-reservations`; CR-363 Night Audit no-show line must be derived FE-side (pending + check-in date < today) or wait for CR-358-P5 Mark No-Show backend field. | CR-363 Gate 2 |
| OG-PMS-016 | P3 | **OPEN** (filed 2026-09-06) | All `/api/v1/vendoremployee/*` order endpoints (`get-single-order-new`, `report/order-logs-report`, `pos/room-payment`) now return 404; `/api/v2/` is live. Frontend already on v2 — no runtime breakage — but all probes/briefs must reference v2. | Housekeeping |
