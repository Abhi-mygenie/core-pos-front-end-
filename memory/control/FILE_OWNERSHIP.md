...
## FILE_OWNERSHIP — BUG-455 + BUG-456 + BUG-457 IMPL (2026-09-24) — markers `// BUG-455`, `// BUG-456`, `// BUG-457`

| File | Change | Owner |
|---|---|---|
| `src/api/transforms/inventoryTransform.js` | +`displayQtyText: item.display_qty_text \|\| ''` in `ingredients()` L25 + `stockItems()` L71 | BUG-455 IMPL 2026-09-24 |
| `src/components/inventory/CurrentStockPanel.jsx` | E1a on-hand cell append muted text · E1b Excel +`Stock (text)` col · E1c PDF head+body +col | BUG-455 IMPL 2026-09-24 |
| `src/components/inventory/SubRecipeStockPanel.jsx` | E2a `getCurrentQty()` +`text` return · E2b destructure + append muted text in cell | BUG-455 IMPL 2026-09-24 |
| `src/components/inventory/StockAuditPanel.jsx` | E3a book-stock cell append muted text | BUG-455 IMPL 2026-09-24 |
| `src/utils/purchasePlanner.js` | E4a planner rows +`display_qty_text` · E4b alert rows +`display_qty_text` | BUG-455 IMPL 2026-09-24 |
| `src/components/inventory/smart/AutoShoppingList.jsx` | E5a first table on-hand +text span · E5b second table on-hand +text span | BUG-455 IMPL 2026-09-24 |
| `src/components/inventory/InventorySetupPanel.jsx` | E1a `startEdit()` +`stockQty` · E1b unit `<select>` `disabled={!!editIng.stockQty}` · E1c conv `<Input>` `disabled={!!editIng.stockQty}` | BUG-456 IMPL 2026-09-24 |
| `src/components/inventory/InventorySetupPanel.jsx` | F1-fix `startEdit()` L190: `Number` → `parseFloat` (NaN-safe on formatted displayQty strings) | BUG-456 F1-FIX 2026-09-25 |
| `src/components/inventory/IngredientBulkEditor.jsx` | E2a `buildRow()` +`stockQty` · E2b unit select `disabled={!_isNew && stockQty>0}` · E2c conv input same | BUG-456 IMPL 2026-09-24 |
| `src/api/services/recipeService.js` | E1 `deleteAddonRecipe(id, reason)` + `{ data: { reason } }` body | BUG-457 IMPL 2026-09-24 |

---

## FILE_OWNERSHIP — BUG-460 IMPL (2026-09-25) — marker `// BUG-460`

| File | Change | Owner |
|---|---|---|
| `src/utils/quantityBreakdown.js` | +`ceilToDisplayUnit(baseQty, factor)` export (L47-51) | BUG-460 IMPL 2026-09-25 |
| `src/utils/purchasePlanner.js` | L7 import · L127-128 velocity `hasCnv` + `ceilToDisplayUnit` · L175-177 alert `ceilToDisplayUnit` | BUG-460 IMPL 2026-09-25 |
| `src/__tests__/utils/purchasePlanner.bug460.test.js` | **NEW** — 13 tests (T1-T8d) | BUG-460 IMPL 2026-09-25 |
| `src/__tests__/utils/purchasePlanner.cr387.test.js` | P1 assertion updated: `suggest_qty` 1480→1950 (BUG-460 intentional change) | BUG-460 IMPL 2026-09-25 |

---

## FILE_OWNERSHIP — BUG-459 + CR-387 IMPL (2026-09-25, Gate 5a) — markers `// BUG-459`, `// CR-387`, OD-UNIFY-01 flag `SHOW_BUG455_PARENTHETICAL`

| File | Change | Owner |
|---|---|---|
| `src/utils/quantityBreakdown.js` | **NEW** — toBreakdown/fromBreakdown/normalizeBreakdown/hasConversion/minorDp (BUG-459) + rowQuantity (CR-387) | BUG-459 + CR-387 IMPL 2026-09-25 |
| `src/__tests__/utils/quantityBreakdown.bug459.test.js` | **NEW** — T1–T14 (14/14) | BUG-459 IMPL 2026-09-25 |
| `src/__tests__/utils/purchasePlanner.cr387.test.js` | **NEW** — P1–P4 + rowQuantity (8/8) | CR-387 IMPL 2026-09-25 |
| `src/api/transforms/inventoryTransform.js` | +`displayQtyParts` in `ingredients()` + `stockItems()` (after BUG-455 lines) | BUG-459 IMPL 2026-09-25 |
| `src/components/inventory/StockAuditPanel.jsx` | E4a–E4m: import+flag, two-box converter, base-unit getDrift + breakdown badge, display-unit payload, wastage toast, **E4m BUG-455 paren suppressed (OD-UNIFY-01)**, col widths | BUG-459 IMPL 2026-09-25 |
| `src/utils/purchasePlanner.js` | E-P1/E-P2: +`small_unit`,`conversion_factor`,`has_conversion`,`display_qty_parts` on velocity + alert rows (math untouched; BUG-455 field kept) | CR-387 IMPL 2026-09-25 |
| `src/components/inventory/smart/AutoShoppingList.jsx` | E-A1–E-A11: import+flag, ad-hoc seed fields, fmtBreak, two-box Qty-to-Buy, breakdown Projected/Gap/Suggested (both tables), rate hint via rowQuantity, **E-A10/E-A11 BUG-455 on-hand paren suppressed (OD-UNIFY-01)**, minWidth | CR-387 IMPL 2026-09-25 |
| `src/components/inventory/SmartPurchasePanel.jsx` | E-C0–E-C5: import, two-box seed, rowQuantity validate + submit (Unit=display unit, rate per display unit, amount unchanged) | CR-387 IMPL 2026-09-25 |
| `src/components/inventory/smart/GroupedVendorPreview.jsx` | E-G0/E-G1: import + rowQuantity breakdown text in item lines | CR-387 IMPL 2026-09-25 |

| `src/components/inventory/RecipeBulkEditor.jsx` | E2a AlertDialog import · E2b 3 state vars · E2c useEffect fetch reasons · E2d deleteRow→dialog + confirmDelete+onRefresh?.() · E2e AlertDialog JSX | BUG-457 IMPL 2026-09-24 |
| `src/components/inventory/RecipeManagementPanel.jsx` | E3 `const activeRecipes = useMemo(sortRecipes(...), deps)` before early return; `recipes={activeRecipes}` | BUG-457 IMPL 2026-09-24 |
