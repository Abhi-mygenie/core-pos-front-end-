...
| **S10** | Prep & Serve Time (lifecycle-mode-aware) | **2026-06-06** (Gate ①) | _____ | _____ | _____ | 🟡 **Mockup ready** — Seed data mockup delivered. 3 view tabs (Overview / By Item / By Station). KPI strip: Avg Prep, Avg Serve, Avg Total, Within-Target %. Daily trend area chart, hourly prep time bars (color-coded), distribution histogram, channel performance cards (lifecycle-aware: Dine-In/Delivery/Takeaway), station cards + comparison chart. Slowest/Fastest items tables. Download disabled (Gate ④). Sidebar linked (`/reports-module/kitchen-ops`). Route wired in App.js. |
...
| # | Screen | Mockup | Sign-off | API wired | API validated | Status |
|---|---|---|---|---|---|---|
| **S-FC** | Food Court — Station-wise Order Breakdown | **2026-06-06** | _____ | **2026-06-06** (live API wired — same order-logs-report) | _____ | 🟠 **Gate ① + ④ delivered** — CR-013. S6 clone + station dropdown. BAR (99 orders), KDS (1013 orders) verified on palmhouse 30D. Route: `/reports-module/food-court`. Sidebar entry added. Awaiting owner review (Gate ②). |
| **S-FC-AUDIT** | Food Court Audit Tab — Station × Time Bucket Cross-Tabulation | **2026-06-07** | **2026-06-07** (owner: "approved") | **2026-06-07** (live API wired — per-order pivot, 5 metrics, proportional discount/subtotal/total, drift column, cancelled item tax exclusion, UNASSIGNED station) | _____ | 🟠 **Gate ④ wired** — CR-013-AUDIT. All 174 orders ₹0 drift on Jun 1 across all 5 metrics. Proportional distribution formula locked. 3 backend gaps flagged (BE-ADDON-001, BE-CANCELLED-TAX-001, FE-PROPORTIONAL-001). Awaiting owner data validation (Gate ⑤). |
