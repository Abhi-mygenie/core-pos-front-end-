...
### Sales (4) — ✅ UNBLOCKED (data in `insights-sales`)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S11 | Daily Sales Summary | **2026-06-15** (`DailySalesMockup.jsx`) | _____ | **2026-06-15** (live via `fetchInsightsSales`) | _____ | 🟠 Gate ①+④ done — awaiting owner review | `insights-sales.daily[]` |
| S12 | Hourly Sales Curve | **2026-06-15** (`HourlySalesMockup.jsx`) | _____ | **2026-06-15** (live via `fetchInsightsSales`) | _____ | 🟠 Gate ①+④ done — awaiting owner review | `insights-sales.hourly[]` |
| S13 | Day-of-Week Trend | **2026-06-15** (`DayOfWeekMockup.jsx`) | _____ | **2026-06-15** (live via `fetchInsightsSales`) | _____ | 🟠 Gate ①+④ done — awaiting owner review | `insights-sales.daily[]` (group by weekday) |
| S14 | Channel × Status Pivot | **2026-06-15** (`ChannelPivotMockup.jsx`) | _____ | **2026-06-15** (live via `fetchInsightsSales`) | _____ | 🟠 Gate ①+④ done — awaiting owner review | `insights-sales.channels[]` + `insights-sales.payments[]` |

### Items — sub-tabs of S5 (4) — ✅ OWNER APPROVED (Gate ②–③ PASSED 2026-06-16)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S15 | Variation Sales tab | **2026-06-15** (tab in S5) | **2026-06-16** | **2026-06-15** (live via `fetchInsightsItems`) | **2026-06-16** (cafe103 30D: 2 variations, ₹15.2L) | ✅ FROZEN | `insights-items.items[].variations[]` |
| S16 | Addon Sales tab | **2026-06-15** (tab in S5) | **2026-06-16** | **2026-06-15** (live via `fetchInsightsItems`) | **2026-06-16** (cafe103 30D: 6 addons, 206 attachments) | ✅ FROZEN | `insights-items.items[].addons[]` |
| S17 | Complementary Items tab | **2026-06-15** (tab in S5) | **2026-06-16** | **2026-06-15** (live via `fetchInsightsItems`) | **2026-06-16** (cafe103: valid empty state) | ✅ FROZEN | `insights-items.items[].complementary` |
| S18 | Station Performance tab | **2026-06-15** (tab in S5) | **2026-06-16** | **2026-06-15** (live via `fetchInsightsItems`) | **2026-06-16** (cafe103 30D: 2 stations, ₹13.3L) | ✅ FROZEN | `insights-items.items[]` (group by `station`) |

### Payments (4) — ✅ UNBLOCKED (`insights-sales` + `insights-staff` LIVE)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S19 | Cashier Settlement | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data (🟡 BE-1 caveat for partial payments) | `insights-staff.by_cashier[]` + `insights-sales.payments[]` |
| S20 | Gateway Reconciliation | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-sales.payments[]` |
| S21 | Tip Report | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-staff.by_server[].tips` |
| S22 | Round-Off Report | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-sales.daily[]` |

### Tax (3) — ✅ UNBLOCKED (`insights-tax` LIVE + `tax_rate/tax_type/tax_calc` on items)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S23 | GST / VAT Detail | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-tax` + `insights-items` (per-item tax_rate, tax_type, tax_calc) |
| S24 | Tax Slab Summary | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-tax.by_slab[]` |
| S25 | Inclusive vs Exclusive Mix | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-tax.by_calc[]` |

### Discounts (2) — ✅ UNBLOCKED (`insights-discounts` LIVE)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S26 | Discount Report | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-discounts.summary + daily[] + by_employee[]` |
| S27 | Coupon Usage | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-discounts.coupons[]` |

### Cancellations (1) — ✅ UNBLOCKED (data in `insights-cancellations`)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S28 | Item Cancellation Detail | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-cancellations.items[]` (per-item detail view) |

### Locations (3) — ✅ UNBLOCKED (`insights-locations` LIVE)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S29 | Table-wise Sales | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-locations.by_table[]` |
| S30 | Delivery Charge Report | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-locations.delivery_charges` |
| S31 | Room Transfer Trail | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-locations.room_transfers[]` |

### Staff (2) — ✅ UNBLOCKED (`insights-staff` LIVE)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S32 | Server / Captain Performance | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-staff.by_server[]` |
| S33 | Cashier Activity | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data (🟡 BE-1 caveat for partial payments) | `insights-staff.by_cashier[]` |

### Audit (2) — ✅ BOTH UNBLOCKED (`notes` field now delivered)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S34 | Order Edit Audit | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-dashboard.audits.orders[]` (make_unpaid + payment_method_change) |
| S35 | Order Note Audit | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-cancellations.items[]` (filter notes != null, 41/343 have notes) |

### Customers (2) — ✅ UNBLOCKED (`insights-customers` LIVE)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S36 | Repeat Customer (RFM bands) | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-customers.summary + top_customers[] + rfm_bands[]` |
| S37 | Guest vs Registered Mix | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-customers.summary + daily[]` |

### Operational (1) — ✅ UNBLOCKED (derivable from existing data)
| # | Screen | Mockup | Sign-off | API wired | API validated | Status | Data Source |
|---|---|---|---|---|---|---|---|
| S38 | KOT-vs-Bill Variance | _____ | _____ | _____ | _____ | 🟠 Gate ①+④ done — verified with May data | `insights-items` (sold vs cancelled vs comp × station) + `insights-dashboard.kitchen` |
...
---
## AMENDMENT 2026-06-16 — Sidebar Navigation Rearrangement
**Trigger:** All 28 Phase 3 screens built. Owner chose Option B (category groups).
**Change:** Sidebar Insights section reorganized from flat 16-item list to grouped structure with 10 category headers. All `comingSoon` flags removed. All 28 Phase 3 screens now navigable from sidebar.
**File:** `Sidebar.jsx` — children array + render loop.
**Groups:** Sales (5), Items (2), Payments (5), Tax (3), Discounts (2), Cancellations (3), Locations (3), Staff (2), Audit (1), Customers (2), Operations (4).
**Plan doc:** `/app/memory/CR_011_SIDEBAR_REARRANGEMENT_PLAN.md`
