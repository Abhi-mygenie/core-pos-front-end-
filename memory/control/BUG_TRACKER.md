...
| BUG-168 | Place-order payload `add_on_qtys` sent as per-unit addon qty without multiplying by item qty. `buildCartItem` L698 sent raw `addonQtys` array. Backend stored `add_ons[0].quantity: 1` instead of `3`, causing KOT/Bill printer to show wrong addon qty (e.g., "Pack x1" instead of "Pack x3" when item qty=3). Identical pattern to BUG-166 (`addon_amount`) and BUG-VQTY (`variation_amount`). Also: addon qty display on CartPanel (3 paths) and CollectPaymentPanel (4 paths) showed per-unit instead of total. | P0 | MEDIUM | **IMPLEMENTED (2026-07-12). Payload: `orderTransform.js` L698: `addonQtys.map(q => q * (item.qty \|\| 1))`. Display: `CartPanel.jsx` (getAddonText helper + fallback), `CollectPaymentPanel.jsx` (4 locations inline). 57/57 tests PASS.** | 0-5a ✅ | `orderTransform.js` L698, `CartPanel.jsx` L8-23+L99+L124+L240, `CollectPaymentPanel.jsx` L1862+L1877+L2217+L2232 |
