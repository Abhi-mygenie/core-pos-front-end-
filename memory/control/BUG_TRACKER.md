...
| **BUG-446** | Old `/pms/arrivals` + `/pms/room-status` headers show the **browser date** (`new Date()`) instead of the business date (X-06 gap on legacy pages) | **P3** | LOW | **REVIEW-AFTER-P2-PLAN-NOTE (owner 2026-09-22, D80)** | AGENT-FOUND during owner smoke validation. MINOR. CODE_ERROR `ArrivalsPage.jsx` L102, `RoomStatusPage.jsx` L28. 2 files / 2 lines. Intake `change_requests/BUG-446_LEGACY_PAGES_SHOW_BROWSER_DATE_INTAKE.md`. |
