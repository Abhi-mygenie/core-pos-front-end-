...
| **BUG-426** | In-House Guests balance column shows room-only (₹950), missing transferred F&B (₹418) + room orders with GST (₹256) — should show ₹1,624 | **P1** | **HIGH** | **INTAKE — GATE 1.** pmsService.js Step 3 reads room_info only — ignores associated_order_list + orderDetails in same response. Config caveat: roomGstApplicable not accessible in service layer. Intake: `change_requests/BUG-426_INHOUSE_BALANCE_MISSING_FNB_ROOM_ORDERS_INTAKE.md` | `pmsService.js` |
