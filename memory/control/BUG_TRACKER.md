...
| **BUG-305** | `orderTransform.js` — `discountRatio` uses full subtotal in `calcOrderTotals` (line 823) + `buildBillPrintPayload` (line 1896): wrong GST in backend payload and bill print | **P1** | **CRITICAL** | **GATE 2 COMPLETE — ALL ODs RESOLVED — AWAITING GATE 3** | 1-2 ✅ | **1 file only** (intake overestimated). `orderTransform.js`: `buildCartItem` adds `_giveDiscount` marker; `calcOrderTotals` splits discountable/non-discountable GST; `buildBillPrintPayload` uses `food_details.give_discount`. Only `collectBillExisting` (Caller 3) is affected in calcOrderTotals (placeOrder/updatePlaceOrder pass discountAmount=0 → no bug). IA: `impact/BUG-305_IMPACT_ANALYSIS.md`. |
