**Last Updated:** 2026-09-23 — **INTAKE BUG-451 / BUG-452 / BUG-453** (new sprint **`sep_bug_closure`**, owner 2026-09-23) registered from the validated 2026-09-18 investigation reports (INV-LIMIT-001, INV-CART-PERSIST-001, INV-MUTE-RINGER-001). BUG-451 product `limit: 500` cap (P1 HIGH, R5 LoadingPage). BUG-452 stale cart under old key after type/table switch (P2 HIGH, **CONFLICT with BUG-334 carry-forward — OD-452-01 REOPENED**). BUG-453 Mute doesn't stop ringer — Fix A race + Fix B FCM-retry suppression **merged into one bug** (P1 MEDIUM). CR-386 (PWA) in CR_REGISTRY. Handover `handover/SESSION_HANDOVER_2026_09_23_INTAKE_SEP_BUG_CLOSURE.md`.
...
### 2026-09-23 — BUG-451, BUG-452, BUG-453 (INTAKE — sprint `sep_bug_closure`, from validated INV reports of 2026-09-18)

| Bug ID | Title | Priority | Risk | Status | Notes |
|---|---|---|---|---|---|
| **BUG-451** | Product list capped at `limit: 500` on boot/refresh — items 501+ never loaded; `PAGINATION.DEFAULT_LIMIT: 100` stale dead default | **P1** | **HIGH** | **GATE 2 IA WRITTEN 2026-09-23** (`impact/BUG-451_IMPACT_ANALYSIS.md`; D1 constant vs literal, D2 delete dead `getAllProducts`; **Gate 3 blocked on backend probe — needs credential**) — ODs LOCKED: limit **2000** + fix `DEFAULT_LIMIT` (backend probe at Planning) | OWNER-REPORTED (backend curl uses `limit=500`). DATA_EDGE. `LoadingPage.jsx` L422 · `useRefreshAllData.js` L29 · `productService.js` L28 · `constants.js` L463–467. `insightsService.js` L62 already 10000. R5 hotspot (param-only). **No curl evidence yet — Planning must probe backend max limit (R11).** DISTINCT. Intake `change_requests/BUG-451_PRODUCT_LIST_LIMIT_500_CAP_INTAKE.md` · evidence `evidence/BUG-451/` · source INV-LIMIT-001. |
| **BUG-452** | Stale cart restored from `cartsByTable[oldKey]` after order-type / table switch mid-build (OrderEntry save effect *copies* to old key instead of *moving*) | **P2** | **HIGH** | **GATE 2 IA WRITTEN 2026-09-23** (`impact/BUG-452_IMPACT_ANALYSIS.md`; shape **S1 remount-nonce in DashboardPage** rec vs S2; BUG-334 annotation needs owner OK) — OD-452-01 = OPTION B (always clear on switch — REVERSES BUG-334)**; OD-452-02/03 open | OWNER-REPORTED ("very rarely"). CODE_ERROR. `DashboardPage.jsx` L2069–2070 store/handler, L1504–1511 / L1462–1497 triggers; `OrderEntry.jsx` L378–391 write (L385), L504–508 BUG-334 carry-forward. **CONFLICT:** INV-CART-PERSIST-001 locked "always clear on switch" which reverses **BUG-334** (CLOSED, owner-verified 2026-08-20). INV planning-skip claim invalid (both files R5); INV Option A fix location does not work (effect re-writes after type change). RELATED PROD-004, INV-OE-001. **R20: INV doc §11 prints a raw password — mask when next edited.** Intake `change_requests/BUG-452_STALE_CART_RESTORED_AFTER_TYPE_TABLE_SWITCH_INTAKE.md` · evidence `evidence/BUG-452/`. |
| **BUG-453** | Scan & Order pop-out **Mute does not stop the ringer** — `soundManager.js` `error`/`play().catch` blindly null `currentAudio` (race on rapid FCM) **+** FCM retries restart sound after Mute (`snoozedOrders` never wired to `NotificationContext`) | **P1** | **MEDIUM** | **GATE 2 IA WRITTEN 2026-09-23** (`impact/BUG-453_IMPACT_ANALYSIS.md`; design **B1 mute set in soundManager**; Gate 3 validation = capture real FCM `order_id`) — all ODs LOCKED: Fix A + Fix B merged · anti-rule override APPROVED · **per-order manual mute toggle** (silent until Mute pressed again; no timer) | OWNER-REPORTED, CONFIRMED on preprod. CODE_ERROR + PLAN_GAP. `soundManager.js` L74–77, L79–83 (guard missing; `ended` L69–71 has it) · `NotificationContext.jsx` L132–135, L175–181 · `ScanOrderPopOut.jsx` L22–27 anti-rule (Jan-2026), L191–207 · `DashboardPage.jsx` L459, L1280–1290. RELATED BUG-081, BUG-122-POST. No hotspot unless wiring via DashboardPage prop. Intake `change_requests/BUG-453_MUTE_DOES_NOT_STOP_RINGER_INTAKE.md` · evidence `evidence/BUG-453/` · source INV-MUTE-RINGER-001. |
...
| **BUG-446** | Old `/pms/arrivals` + `/pms/room-status` headers show the **browser date** (`new Date()`) instead of the business date (X-06 gap on legacy pages) | **P3** | LOW | **DEFERRED-TO-FU-385-C (owner 2026-09-22, D81 — "c=defer to FU-385-C")** | AGENT-FOUND during owner smoke validation. MINOR. Not in `registry.json` (intake registered in tracker only — to be added at FU-385-C intake). CODE_ERROR `ArrivalsPage.jsx` L102, `RoomStatusPage.jsx` L28. 2 files / 2 lines. Intake `change_requests/BUG-446_LEGACY_PAGES_SHOW_BROWSER_DATE_INTAKE.md`. |
