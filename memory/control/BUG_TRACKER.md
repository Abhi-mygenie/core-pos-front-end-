...
| **BUG-248** | Bulk Editor: 9 columns missing from `isDirty()` (Part A — FE fix ready) + Backend silently drops `packed_food`, `is_inventory`, `stock_out`, `tax_calc` on food edit (Part B — BACKEND-BLOCKED). Curl-verified: API returns "success" but doesn't persist. Backend brief filed. | **P1** | LOW | **IMPACT ANALYSIS COMPLETE — Part A ready for Gate 3, Part B BACKEND-BLOCKED** | 2 | Part A: `BulkEditor.jsx` ~9 lines. Part B: backend fix for `/foods/{id}`. Impact: `/app/memory/impact/BUG-248_IMPACT_ANALYSIS.md` |
