...
**Last Updated:** 2026-07-23 (Session C) — Gate 2 Impact Analysis now COMPLETE for ALL 14 bugs. Batches 1–6 OWNER APPROVED. Batches 7–8 (BUG-217, BUG-219, BUG-220) Gate 2 complete — awaiting owner approval. Key findings: BUG-217 re-scoped (Unit, not Serves, blocks save); BUG-219 upgraded to HIGH (live data corruption on min_unit_alert); BUG-220 largely self-resolved (backend 409 already surfaced).
...
| BUG-217 | Sub-Recipe Serves Field Not Required But Blocks Save | P2 | MEDIUM | **GATE 2 COMPLETE — awaiting owner approval** | 0-2 ✅ | Curl-verified 2026-07-23: intake DISPROVEN — serves never blocks (FE `\|\|1` + backend coerces 0→1). Real blocker: blank Unit → backend 500 SQL null. Re-scope: require Unit. Conflict: after/with BUG-215. 1 file ~4 lines. |
...
| BUG-219 | Ingredient Form — Min Unit Text Input, Labels Unclear | P2 | **HIGH⬆** | **GATE 2 COMPLETE — awaiting owner approval** | 0-2 ✅ | Data-proven 2026-07-23: min_unit_alert = UNIT STRING (106/106 rows); FE Number() → 0 → edit-save writes "0" = live data corruption. Risk LOW→HIGH. 2 files (~20 lines). Conflict: after/with BUG-226. |
| BUG-220 | Ingredient Category — No Duplicate Alert | P2 | MEDIUM (LOW proposed⬇) | **GATE 2 COMPLETE — awaiting owner approval** | 0-2 ✅ | Curl-verified: backend 409 robust (case-insensitive+trim) and FE already shows the message — intake outdated. Fix: pre-call guard ~5 lines. Downgrade needs owner OK. CR-090 intel: delete endpoint exists. |
