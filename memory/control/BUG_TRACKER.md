...
| **BUG-113** | **Partial payment UI stuck — auto-fill locks Cash/Card/UPI amount fields, cannot re-enter** | **P1** | **IMPLEMENTED** | **POS 4.0** | **FE fix: removed real-time capping + auto-fill from onChange. Moved to onBlur — clamp + auto-fill only when other row is empty.** |
| **BUG-114** | **discount_type, discount_member_category_id/name sent as empty/0 when category discount applied** | **P1** | **IMPLEMENTED** | **POS 4.0** | **FE fix: threaded selectedDiscountType (id, name) through paymentData.discounts → transform builders read from discounts instead of hardcoded 0. Covers placeOrderWithPayment + collectBillExisting.** |
...
| **BUG-116** | **Out-of-kitchen/out-of-menu item toggle — API change needed so socket fires and realtime menu updates** | **P1** | **DISCOVERY COMPLETE** | **POS 4.0** | **Backend socket + FE: save not wired to API + no socket handler + no MenuContext updater** |
| **BUG-117** | **Audit Report side-sheet (right panel) — discount showing in text instead of proper format/value** | **P2** | **NEEDS RUNTIME VALIDATION** | **POS 4.0** | **Code-read found field name mismatch (discountAmount vs discount), but needs preprod login to validate with real orders** |
