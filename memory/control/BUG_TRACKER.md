**Last Updated:** 2026-09-26 — **INTAKE UPDATED: BUG-465 (P1/HIGH, sprint `sep_bug_closure`, Gate 1).** Reports charts crash in production build — `recharts@3.6.0` + `es-toolkit@1.47.1` Terser minification → `TypeError: mR is not a function` on first chart mount. Dev server unaffected; production `pos.mygenie.online` crashes all chart pages (30 files). Blast radius LARGE. Code reality NONE · DISTINCT. Root cause: no lockfile committed → `es-toolkit` floated to `1.47.1` on recent deploy (worked fine for 3 months prior). **npm vs yarn is NOT the cause** — both resolve to same version without lockfile; crash is in Terser minification step. Fix: add `"overrides": { "es-toolkit": "1.46.1" }` to `package.json` (npm overrides field — correct for npm 8.3+) + `rm -rf node_modules package-lock.json && npm install && npm run build`. **Owner direction 2026-09-26: npm is canonical build tool going forward (dev + prod); commit `package-lock.json` after fix to prevent future version float. OD-465-03 LOCKED: YES.** Investigation report `memory/PL_CHARTS_PROD_CRASH_INVESTIGATION_REPORT.md`. Intake `change_requests/BUG-465_REPORTS_CHARTS_PROD_CRASH_RECHARTS_ESKIT_MINIFICATION_INTAKE.md`. Registry 729 items. OD-465-01 (Option 1 pin vs Option 2 recharts downgrade) · OD-465-02 (regression scope) OPEN. **Next: owner "Gate 2 GO" → Planning Impact Analysis.**
...
| **BUG-462** | Order Entry: Empty-state not shown on first boot for non-Normal restaurants — `activeMenuType !== 'Normal'` guard at L1800 prevents empty-state when localStorage is cleared (defaults to 'Normal'); QA_HYATT sees blank item grid with zero guidance | **P2** | **MEDIUM** | **GATE_5A_IMPLEMENTED (2026-09-25).** Fix: +`availableMenuTypes` destructure at L57; replaced guard `activeMenuType !== 'Normal'` → `availableMenuTypes.length > 1` at L1804. Normal-only restaurants (length=1) regression-safe. QA_HYATT first-boot: empty-state confirmed present. Compile PASS. Intake: `change_requests/BUG-462_ORDER_ENTRY_EMPTY_STATE_MISSING_FIRST_BOOT_INTAKE.md` | `OrderEntry.jsx` L57 + L1804 (// BUG-462 markers) |
| **BUG-463** | Local Settings Active Menu: Two visually identical "Promotional Menu" pills — preprod data has `"Promational Menu"` (typo) + `"Promotional Menu"` (correct); both render, confusing to staff | **P3** | **LOW** | **BACKEND_BRIEF_FILED (2026-09-25). No FE code change needed.** Backend team to correct/merge `"Promational Menu"` → `"Promotional Menu"` for QA_HYATT products in preprod. Brief: `backend_briefs/BACKEND_BRIEF_BUG-463_2026_09_25.md` | `NONE (backend data)` |
| **BUG-464** | Order Entry chip reads "FOOD MENU Menu" — `{activeMenuType} Menu` at L1729 appends "Menu" unconditionally; redundant for types already containing "MENU" in their name | **P3** | **LOW** | **GATE_5A_IMPLEMENTED (2026-09-25).** Fix: `/menu$/i` regex — suppress " Menu" suffix when type name already ends with "menu". `"FOOD MENU"→"FOOD MENU"`, `"Premium"→"Premium Menu"` ✅. Compile PASS (0 new warnings). Self-test PASS. Intake: `change_requests/BUG-464_CHIP_TEXT_REDUNDANT_MENU_WORD_INTAKE.md` | `OrderEntry.jsx` L1729 (// BUG-464 marker) |
