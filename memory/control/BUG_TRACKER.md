...
| **BUG-439** | CR-385 P0 duplicate `data-testid` when a guest row is expanded — row actions re-rendered inside `RowExpansionStub` with the same ids (Arrivals/Departures/In-House) | **P3** | LOW | **INTAKE** | QA-FOUND 2026-09-21 (P0.5 re-test round 2, `/app/test_reports/iteration_9.json`, CONFIRMED by code read). QA severity **MINOR**. Intake: `change_requests/BUG-439_CR385_P0_DUPLICATE_TESTID_EXPANDED_ROW_INTAKE.md`. Evidence: `evidence/BUG-439/`. Scope SMALL, no hotspots; fix needs `GuestTable.jsx`/`InHousePanel.jsx` (outside P0.5 §4.3) → owner scope approval. Pre-existing P0 code, missed by P0 X-10 (rows collapsed). Owner decides: fix in P0.5 (§0-bis) or ship. |
