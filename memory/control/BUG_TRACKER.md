**Last Updated:** 2026-09-26 — **INTAKE: BUG-465 registered (P1/HIGH, sprint `sep_bug_closure`, Gate 1).** Reports charts crash in production build — `recharts@3.6.0` + `es-toolkit@1.47.1` Terser minification → `TypeError: mR is not a function` on first chart mount. Dev server unaffected; production `pos.mygenie.online` crashes all chart pages (30 files). Blast radius LARGE. Code reality NONE · DISTINCT. Fix: `package.json` `overrides: { "es-toolkit": "1.46.1" }` + clean reinstall. Investigation report `memory/PL_CHARTS_PROD_CRASH_INVESTIGATION_REPORT.md`. Intake `change_requests/BUG-465_REPORTS_CHARTS_PROD_CRASH_RECHARTS_ESKIT_MINIFICATION_INTAKE.md`. Registry 729 items. OD-465-01 (Option 1 pin vs Option 2 recharts downgrade) · OD-465-02 (regression scope) OPEN. **Next: owner "Gate 2 GO" → Planning Impact Analysis.**
