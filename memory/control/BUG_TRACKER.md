...
|---|---|---|---|---|---|---|
| BUG-214 | Addon Recipe Dropdown Shows Menu Items Instead of Addon Items | P1 | HIGH | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Impact doc: `/app/memory/BUG-214_IMPACT_ANALYSIS.md`. FE-only fix: `RecipeFormPanel.jsx` — fix `getActiveAddons()` silent catch + remove `foods` fallback. |
| BUG-215 | Recipe Forms — Validation Errors Not Shown on Save Failure | P1 | MEDIUM | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Impact doc: `/app/memory/BUG-215_IMPACT_ANALYSIS.md`. FE-only: `RecipeFormPanel.jsx` — add inline error state on save catch. |
| BUG-216 | Recipe Ingredient Row Shows Base Unit, Should Show Small Unit | P2 | HIGH ⬆ | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Data-confirmed: all 346 preprod recipe rows already store small units; form autofill is the outlier AND unit is SAVED to backend (not display-only). Sibling defect in `RecipeBulkEditor.jsx:185`. 2 files, 3 lines. Awaiting owner → Gate 3. |
| BUG-217 | Sub-Recipe Serves Field Not Required But Blocks Save | P2 | MEDIUM | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Curl-verified 2026-07-23: intake DISPROVEN — serves never blocks (FE `\|\|1` + backend coerces 0→1). Real blocker: blank Unit → backend 500 SQL null. Re-scope: require Unit. Conflict: after/with BUG-215. 1 file ~4 lines. |
| BUG-218 | Delete Ingredient — No Blocking Error When Used in Recipe | P1 | LOW (post-fix) | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Impact doc: `/app/memory/impact/BUG-218_IMPACT_ANALYSIS.md`. Curl-verified: backend HTTP 400 + `used_in_recipes[]`. FE-only: parse catch + Dialog. `InventorySetupPanel.jsx` 1 file. |
| BUG-219 | Ingredient Form — Min Unit Text Input, Labels Unclear | P2 | **HIGH⬆** | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Data-proven 2026-07-23: min_unit_alert = UNIT STRING (106/106 rows); FE Number() → 0 → edit-save writes "0" = live data corruption. Risk LOW→HIGH. 2 files (~20 lines). Conflict: after/with BUG-226. |
| BUG-220 | Ingredient Category — No Duplicate Alert | P2 | **LOW⬇ (owner-approved)** | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Curl-verified: backend 409 robust (case-insensitive+trim) and FE already shows the message — intake outdated. Fix: pre-call guard ~5 lines. Downgrade needs owner OK. CR-090 intel: delete endpoint exists. |
| BUG-221 | Bulk Ingredient Upload & Excel Download Not Working | P1 | HIGH | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Curl-verified 2026-07-23: export returns JSON `download_url` but `exportIngredients()` forces blob → corrupt xlsx. Bulk editor has NO import UI (`importIngredients()` unused). Import field = `file`; ⚠ 201+`status:false` trap; backend accepts junk file (backend brief candidate). 2 files. Awaiting owner → Gate 3. |
| BUG-222 | Bulk Recipe Excel — No Template/Export Split; File Won't Open | P2 | HIGH ⬆ | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Curl-verified 2026-07-23: export JSON-vs-blob mismatch → corrupt xlsx (won't-open CONFIRMED). Import sends `file`, backend requires `products_file` → always 422. Template endpoint live, unwired. Risk upgraded MEDIUM→HIGH (API contract). 2 files. Awaiting owner → Gate 3. |
| BUG-223 | Wastage & Recipe Deduction Auto-Trigger Without Explicit Save | P1 | LOW (post-fix) | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Impact doc: `/app/memory/BUG-223_IMPACT_ANALYSIS.md`. UX-only: `StockAuditPanel.jsx` — amber "preview" badge + unsaved banner. 1 file. |
| BUG-224 | Smart Purchase — Ingredients Without Recipes Never Appear | P2 | HIGH ⬆ | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Mechanism verified: all stock rows processed, but velocity-0 items always fail B2 gap<0 filter; min alerts never consulted. No new fetch needed. Amends locked B2 (owner Q3). 2-3 files. Awaiting owner → Gate 3. |
...
| BUG-226 | Conversion Factor Not Saved on Add or Edit Ingredient | P1 | LOW (post-fix) | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Impact doc: `/app/memory/BUG-226_IMPACT_ANALYSIS.md`. `inventoryTransform.js` — add `converion_factor` to ADD payload + fix `\|\| 1` on EDIT. 1 file. Q1 resolved: default=1. |
| BUG-227 | Smart Purchase — Vendor Column Shows No History Though Vendors Exist | P1 | HIGH ⬆ | **GATE 3 COMPLETE — awaiting Gate 4 GO** | 0-2 ✅ | Curl-verified: vendor master (12 vendors) never enters ranking; PLUS 54% of history rows have vendor_id null (evidence saved). 3 files (`SmartPurchasePanel`, `vendorRanking`, `VendorSuggestionCell`). Awaiting owner → Gate 3. |
...
|--------|-------------|----------|------|--------|------|-------|
| BUG-203 | Inline edit on Expense Setup missing unit price field — edit row only shows name + category dropdown, no ₹ price input. **BACKEND-BLOCKED:** PUT `/expenses/{id}` does not accept `unit_price` (§3.4 not delivered). Workaround: Unit Prices tab (CR-066). | P2 | MEDIUM | **IMPLEMENTED (2026-07-17). Sub-A: inline edit price (2-call). Sub-B: new row price in Bulk Editor. Sub-C: pricedItems edit-vs-add. Sub-D: edit row qty auto-calc for priced items.** | 0-5a ✅ | `ExpenseSetupPanel.jsx`, `ExpenseBulkEditor.jsx`, `ExpenseEntryPanel.jsx` |
