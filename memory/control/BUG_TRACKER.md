...
|--------|-------------|----------|------|--------|------|-------|
| BUG-203 | Inline edit on Expense Setup missing unit price field — edit row only shows name + category dropdown, no ₹ price input. **BACKEND-BLOCKED:** PUT `/expenses/{id}` does not accept `unit_price` (§3.4 not delivered). Workaround: Unit Prices tab (CR-066). | P2 | MEDIUM | **IMPLEMENTED (2026-07-17). 2-call workaround. Bulk Editor price column included.** | 0-5a ✅ | `ExpenseSetupPanel.jsx`, `ExpenseBulkEditor.jsx` |
...
| BUG-205 | Expense: Qty/Unit columns missing from transaction table + report. API returns `quantity` and `unit`, transform maps them, but neither ExpenseEntryPanel nor ExpenseReportPage renders them. ~20 lines, 2 files. | P2 | LOW | **GATE 3 COMPLETE (2026-07-17).** Plan ready: 6 edits, 2 files. Awaiting Gate 4 GO. | 0-3 ✅ | `ExpenseEntryPanel.jsx`, `ExpenseReportPage.jsx` |
