...
| **BUG-245** | Table card moves to top when order placed. `ChannelColumn.jsx:159` occupied-first bucketing `[...occupied, ...available]` causes tables with orders to jump to grid position 1. Owner expects stable label-numeric position regardless of status. | **P1** | LOW | **INTAKE COMPLETE** | 0-1 | 1 file: `ChannelColumn.jsx`. ~3 lines. |
| **BUG-246** | Customized items not merging in cart. `addCustomizedItemToCart()` (OrderEntry.jsx:676) always appends new line — zero merge logic. Same Half Cup added 3× = 3 separate lines at qty=1 (expected: 1 line at qty=3). Cascades to KOT print, bill print, prep timers. R5 hotspot. | **P1** | MEDIUM | **INTAKE COMPLETE** | 0-1 | 1 file: `OrderEntry.jsx` (R5). ~10 lines. |
| **BUG-247** | Smart Purchase ad-hoc typeahead blocks UI. Each keystroke re-renders entire table (50+ VendorSuggestionCell Popover/Command components). No React.memo, no debounce. RELATED to BUG-236 (clipping — different issue). | **P2** | LOW | **INTAKE COMPLETE** | 0-1 | 1-2 files: `AutoShoppingList.jsx`, `VendorSuggestionCell.jsx`. ~5 lines. |
