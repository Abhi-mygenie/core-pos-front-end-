...
| **BUG-441** | Legacy `/pms/arrivals` Cancel sends the public `booking_id` string as the local-reservation id (`ArrivalsPage.jsx` L273 `reservationId: row.bookingId`) → `POST local-reservations/MG-69-…/cancel` → **500 TypeError**; booking never cancels from the legacy page (new Front Desk path uses `row.id` and works) | **P1** | MEDIUM | **INTAKE (2026-09-21) — owner routing needed** | QA-FOUND 2026-09-21 (CR-385 P1 QA `iteration_12.json` C-3). QA severity **BLOCKER** on the legacy page. RCA CODE_ERROR (CR-362; masked until BUG-440 made Confirm clickable). Fix = `reservationId: row.id` (1 line); NOT applied — `ArrivalsPage.jsx` is out of P1 scope. Intake `change_requests/BUG-441_LEGACY_ARRIVALS_CANCEL_WRONG_ID_INTAKE.md`, evidence `evidence/CR-385/phase1_qa/c3_legacy_224.json`. Check `ReservationsPage.jsx` cancel wiring too. |
| **BUG-442** | Cancel booking posts `cancelled_by: "staff"` for every user — call sites read `restaurant.profile.fullName` (not on RestaurantContext; `fullName` is on `AuthContext.user`) | **P2** | LOW | **INTAKE (2026-09-22) — owner routing (P1.5 / legacy with BUG-441)** | QA-FOUND `iteration_14.json` R-M2-02 (CR-385 P1 Role-4 QA). MINOR. CODE_ERROR (CR-362 origin, inherited by CR-385 M2 per plan copy rule). 3 files / 3 lines. Intake `change_requests/BUG-442_CANCELLED_BY_ALWAYS_STAFF_INTAKE.md`. |
