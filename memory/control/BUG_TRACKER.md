**Last Updated:** 2026-09-16 — **BUG-430 INTAKE COMPLETE.** Room Orders GST missing add-ons calculation (BUG-429 implementation gap). P1 CRITICAL. folioTransform.js + pmsService.js calculate GST on (unit_price × qty) only, excluding add-ons. orderTransform.js correctly includes add-ons in GST base. Result: ₹1,930.40 vs ₹1,947 (₹16-18 under-counted). 2 files, ~10-14 lines. RELATED to BUG-429 (correction of BUG-429 fix). Intake doc: `change_requests/BUG-430_ROOM_ORDERS_GST_MISSING_ADDONS_INTAKE.md`. Registry: 687 items. **Awaiting Gate 2 GO.**
...
||---|---|---|---|---|---|
|| **BUG-430** | Room Orders GST missing add-ons calculation (BUG-429 implementation gap) — folioTransform + pmsService calculate GST on base price only, excluding add-ons | **P1** | **CRITICAL** | **GATE 3 — PLAN COMPLETE** | **Root cause:** BUG-429 copied GST formula from orderTransform.js but missed the add-ons calculation block (L1900-1904) that runs BEFORE GST. **Result:** GST calculated on `(unit_price × qty)` only; add-ons excluded from tax base. **Gap:** ₹1,930.40 vs ₹1,947 (₹16-18 = GST on missing add-ons). **Fix:** Add `addonPerUnit` reduction before `amt` calculation in both files. **Impact Analysis:** 2 files, 12 lines added, 2 modified. Risk: LOW (additive only, BUG-429 untouched). NO conflicts (builds on BUG-429). Doc: `impact/BUG-430_IMPACT_ANALYSIS.md`. **Implementation Plan:** E1 folioTransform L115-120 (+6 lines), E2 pmsService L125-130 (+6 lines). 14 verification checks. Doc: `plans/BUG-430_IMPLEMENTATION_PLAN.md`. OD-430-01 LOCKED (add-ons included per GST law). RELATED to BUG-429 (correction). Source: AGENT-DISCOVERED (Investigation role). **Awaiting Gate 4 GO.** |
