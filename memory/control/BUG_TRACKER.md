...
| **BUG-244** | add-purchase payload: `payment_method` wrong key (should be `payment_type`), missing `tot_amount`/`item_total` (default to 1), `converion_factor: 1` always sent (should omit). Affects ALL purchases via `addPurchase`. RELATED to BUG-243 (same endpoint, different root cause). | **P0** | MEDIUM | **INTAKE COMPLETE** | 0-1 | 1 file: `inventoryTransform.js`. ~6-8 lines. Intake: `/app/memory/change_requests/BUG-244_ADDPURCHASE_PAYLOAD_MISMATCH_INTAKE.md`. Investigation: `/app/memory/reports/INVESTIGATION_SMART_PURCHASE_SUBMIT_2026_07_25.md`. |
