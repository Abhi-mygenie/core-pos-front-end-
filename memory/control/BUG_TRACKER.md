...
| **BUG-248** | Bulk Editor: 9 columns missing from `isDirty()` (Part A — FE fix ready) + Backend silently drops `packed_food`, `is_inventory`, `stock_out`, `tax_calc` on food edit (Part B — BACKEND-BLOCKED). Curl-verified: API returns "success" but doesn't persist. Backend brief filed. | **P1** | LOW | **INTAKE COMPLETE — Part A ready, Part B BACKEND-BLOCKED** | 0-1 | Part A: `BulkEditor.jsx` ~9 lines. Part B: backend fix for `/foods/{id}`. |
