...
| **BUG-418** | Checkout Drawer — GST not in display | **P1** | **CRITICAL** | **INTAKE — GATE 1.** | `PmsCheckoutDrawer.jsx` |
...
| **BUG-426** | In-House Guests balance column shows room-only (₹950), missing transferred F&B (₹418) + room orders with GST (₹256) — should show ₹1,624 | **P1** | **HIGH** | **GATE 3 — PLAN READY. Awaiting Gate 4 GO.** OD-426-02 overridden: roomGstApplicable flag MUST be included (not just documented). OD-426-03: 2 different sub-totals stored separately. 2 files: pmsService.js (Step 3 extension) + InHouseGuestsPage.jsx (useRestaurant wire). Impact: `impact/BUG-426_IMPACT_ANALYSIS.md`. Plan: `plans/BUG-426_IMPLEMENTATION_PLAN.md` | `pmsService.js` + `InHouseGuestsPage.jsx` |
| **BUG-427** | Folio Total Balance Due excludes room orders — fnbTotal sums transferred only (₹418), room orders (₹256 post-GST) absent; shows ₹1,368 not ₹1,624. Room Orders section total pre-tax (₹228) not post-GST (₹256) | **P1** | **CRITICAL** | **GATE 3 — PLAN READY. Awaiting Gate 4 GO.** OD-427-04: 2 separate tiles (Transferred F&B + Room Orders). OD-427-05: roomGstApplicable flag required. totalAmount field added to folioTransform. 3-tile grid layout. Impact: `impact/BUG-427_IMPACT_ANALYSIS.md`. Plan: `plans/BUG-427_IMPLEMENTATION_PLAN.md` | `folioTransform.js` + `GuestFolioPage.jsx` |
| **BUG-428** | Checkout ROOM breakdown missing Lodging GST line — 1000 − 100 shows 950 (math appears wrong visually; GST ₹50 hidden in balance but not displayed as line item) | **P2** | **MEDIUM** | **GATE 3 — PLAN READY. Awaiting Gate 4 GO.** OD-428-03: gated on gstTax > 0 AND roomGstApplicable !== false. 1 JSX block (~6 lines). restaurant already in scope at L79. R5 regression checklist included. Impact: `impact/BUG-428_IMPACT_ANALYSIS.md`. Plan: `plans/BUG-428_IMPLEMENTATION_PLAN.md` | `CollectPaymentPanel.jsx` (R5) |
