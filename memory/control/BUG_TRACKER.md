...
| **BUG-312** | `fromAPI.ingredients()` missing `isSubRecipe`/`subrecipeId` — root cause for all sub-recipe misrouting | **P1** | **HIGH** | **INTAKE** | 1 | 1 file: `inventoryTransform.js:8-31`. 2 lines to add. ROOT CAUSE for BUG-313 and AdHocTypeahead + PurchaseEntryPanel paths. Fix this first. Planning skip eligible. |
| **BUG-313** | Sub-recipe appears in Stock Update auto-plan + `addPurchase()` called for all rows (no sub-recipe routing) | **P1** | **HIGH** | **INTAKE** | 1 | 4 files: `purchasePlanner.js` (G9 filter unreliable), `AutoShoppingList.jsx` (no isSubRecipe filter), `SmartPurchasePanel.jsx:194` (no routing), `PurchaseEntryPanel.jsx:195/95` (no routing). Depends on BUG-312. Full Gate 2-3. |
