...
| **BUG-249** | Current Stock: Negative quantity items show "In Stock" badge — `item.quantity` (raw, stale) used for status but `item.displayQty` (negative) shown in column. 10 sites affected. Fix: `effectiveQty()` helper using `displayQty || calQuantity || quantity`. | **P1** | LOW | **INVESTIGATION COMPLETE — ready for Planning** | 1-2 | 1 file: `CurrentStockPanel.jsx`, ~10 lines. |
