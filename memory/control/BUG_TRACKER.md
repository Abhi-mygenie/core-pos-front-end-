**Last Updated:** 2026-09-24 — **INTAKE: BUG-454 (P0 CRITICAL, order payload GST ungated by gst_status) + BUG-455 (P2, ProductForm tax rule) registered from GST investigation.** Registry: 715 items.
...
| **BUG-452** | Stale draft cart restored under the old key after order-type / table switch mid-build (`OrderEntry.jsx` effect L378–512 W1 write + BUG-334 carry-forward; `DashboardPage.cartsByTable`) | **P2** | HIGH | **GATE 3 CLOSED 2026-02** — Option B always-clear · **S1 remount** (bump `orderEntryResetNonce` in `handleOrderTypeChange`/`handleTableClick` when OrderEntry open) · **silent clear, no confirm dialog** · **BUG-334 REVERSED annotation approved**. **VD-8 (modal open at remount) DEFERRED to Wave 2 Gate 4** (owner 2026-02). Awaiting Wave 1 Gate 4 GO → Wave 2 Gate 4 → implement. | R5 ×2 (`DashboardPage.jsx` +2–4 lines, `OrderEntry.jsx` comment only). Latent second defect (placed items snapshot on occupied-table switch) also fixed by removing W1. `cartsByTable` plumbing becomes dead → cleanup CR later (OPEN_GAPS entry pending owner OK). Intake `change_requests/BUG-452_STALE_CART_RESTORED_AFTER_TYPE_TABLE_SWITCH_INTAKE.md` · IA `impact/BUG-452_IMPACT_ANALYSIS.md`. **BUG-334** (CLOSED 2026-08-20): no tracker row exists in this checkout; reversal recorded in `registry.json` `status_history`. |
| **BUG-455** | Single-item `ProductForm` lacks the CR-036-FU-03 tax-required rule — `ProductForm.jsx` has 0 `gstStatus` refs while `BulkEditor.jsx:576-584` enforces it | **P2** | MEDIUM | **INTAKE — GATE 1 (2026-09-24)** | 1 | OD-455-01 open. Intake `change_requests/BUG-455_PRODUCTFORM_MISSING_TAX_REQUIRED_RULE_INTAKE.md`. INV `INV_GST_MENU_VALIDATION_INVESTIGATION_REPORT_2026_09_24.md` X3. |
| **BUG-454** | Order payload GST not gated by `gst_status` — `orderTransform.js:723-734` (`buildCartItem`) + `calcOrderTotals` ignore `restaurant.tax.gstStatus`; Collect Bill hides GST (BUG-336) but place/update payload still sends `tax_amount`/`gst_tax` | **P0** | **CRITICAL** | **INTAKE — GATE 1 (2026-09-24)** — RELATED BUG-336 (display-only fix) | 1 | R5 `orderTransform.js` + R6 financial → full gate flow. OD-454-01 open. Intake `change_requests/BUG-454_ORDER_PAYLOAD_GST_NOT_GATED_BY_GST_STATUS_INTAKE.md`. |
