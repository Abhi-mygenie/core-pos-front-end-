...
| **BUG-248** | Bulk Editor: 9 columns missing from `isDirty()` checks — Packaged Item, Inventory, Out of Stock, Hidden from POS, Tax Calc, Sold By Unit, Avail. Start/End, Portion Size. Edits to these columns never trigger "Save X Changes" button. Silently lost. Owner-reported via Packaged Item screenshot. | **P1** | LOW | **INTAKE COMPLETE** | 0-1 | 1 file: `BulkEditor.jsx`. ~9 lines added to `checks` object. |
