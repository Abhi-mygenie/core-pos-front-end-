...
| **BUG-305** | `orderTransform.js` — `discountRatio` uses full subtotal in `calcOrderTotals` (line 823) + `buildBillPrintPayload` (line 1896): wrong GST in backend payload and bill print | **P1** | **CRITICAL** | **GATE 3 COMPLETE — AWAITING GATE 4 GO** | 1-3 ✅ | **1 file, 3 edits.** E1: `buildCartItem` +`_giveDiscount` marker (line 747). E2: `calcOrderTotals` +discountable buckets +`discountableRatio` (lines 783-841). E3: `buildBillPrintPayload` +split in forEach +fix ratio (lines 1840-1904). Plan: `plans/BUG-305_IMPLEMENTATION_PLAN.md`. IA: `impact/BUG-305_IMPACT_ANALYSIS.md`. |
