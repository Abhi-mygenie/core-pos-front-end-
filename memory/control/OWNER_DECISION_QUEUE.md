...
## OD-10 — CR-133 follow-ups: Bill/KOT preview + Test Print (PENDING — 2026-08-08)

**Context:** CR-133 shipped with preview deferred (OD-9) and test print as disabled "Coming soon" (OD-8). Post-delivery, owner asked where preview and test print are — options presented, awaiting answer.

| Option | Scope | Notes |
|---|---|---|
| a | Build in-browser Bill/KOT preview now | Frontend-only follow-up CR; sample receipt rendered from live style_config; fully browser-testable |
| b | Start real Test Print (Phase 2) | Requires INVESTIGATION of printer-agent socket contract first; final validation needs a physical printer |
| c | Both — preview first, then test-print investigation | — |
| d | Leave deferred (per original OD-8/OD-9) | — |

**Status:** AWAITING OWNER ANSWER. Do not start either work stream until resolved.
