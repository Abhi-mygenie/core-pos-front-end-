...
| **CR-376-FU-B** | CategoryPanel: hide categories with 0 items for active menu + `Name (count)` labels + default tab All + Popular scoped to active menu. | P2 | MEDIUM | **GATE 2 — IMPACT ANALYSIS COMPLETE (2026-09-25). Blocker (categoryId vs categoryIds) RESOLVED → use `categoryId` (same as grid L561). 0 open owner decisions. Awaiting owner "Gate 3 GO".** | 2 | `CategoryPanel.jsx` (~15 lines) + `OrderEntry.jsx` (~4 additive lines, R5 hotspot — no Fast Lane). Impact: `impact/CR-376-FU-B_IMPACT_ANALYSIS.md`. Handover: `handover/SESSION_HANDOVER_2026_09_25_CR376_FU_B_GATE2_COMPLETE.md`. |
