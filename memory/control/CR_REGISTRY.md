...
| **CR-376-FU-A** | CustomerModal: hide/disable inert off-menu suggestion rows after CR-376 ships. | P2 | LOW | **GATE_5A_IMPLEMENTED (2026-09-25). E1 filteredCrossSell ID guard + E2 filteredTopItems const + E3 render swap. 1 file (CustomerModal.jsx), 6 lines, Fast Lane. Compile PASS. Awaiting QA Gate 5b.** | 3→5A | `CustomerModal.jsx` (1 file). Sprint `sep_bug_closure`. Impact: `impact/CR-376-FU-A_IMPACT_ANALYSIS.md`. Plan: `plans/CR-376-FU-A_IMPLEMENTATION_PLAN.md`. |
| **CR-376-FU-B** | CategoryPanel: hide categories with 0 items for active menu + `Name (count)` labels + default tab All + Popular scoped to active menu. | P2 | MEDIUM | **GATE 5B — QA PASS (2026-09-26). 13/13: 6 unit + 7 browser (QA_HYATT). Default tab All (86) · 12/12 rows count==tiles on FOOD MENU · Bar & Drinks switch → 16 rows, All (170) · no `(0)` rows · search + add-to-cart OK · console clean. Popular row absent on Hyatt (E5 covered by unit only — NOTE N1). Awaiting Gate 6 Owner Smoke.** | 5b | `CategoryPanel.jsx` (E1-E3) + `OrderEntry.jsx` (E4-E6, R5 hotspot additive) + NEW `__tests__/CategoryPanel.cr376fub.test.jsx`. Plan: `plans/CR-376-FU-B_IMPLEMENTATION_PLAN.md`. QA handover: `handover/QA_HANDOVER_CR376_FU_B_2026_09_26.md`. QA report: `test_reports/CR-376-FU-B_QA_REPORT_2026_09_26.md`. Handover: `handover/SESSION_HANDOVER_2026_09_26_CR376_FU_B_GATE5B_PASS.md`. |
