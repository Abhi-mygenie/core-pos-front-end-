**Last Updated:** 2026-09-26 (s2) — **CR-390 INTAKE brainstorm session 2 (owner walkthrough).** Modules OWNER-DEFINED → one PDF each: MM Menu Management · EM Expenses Management · IM Inventory Management · DC Day Closure & Settlement · DR Daily Report (contents TBD) · Insights Basic · Insights Advanced · PMS redo. **OD-390-04 LOCKED** (perfect user journey; per-module journey owner-approved before generation). **Audience LOCKED: for clients.** OD-390-08 DEFERRED. Doubts re-asked at Intake gate as OD-390-09…14. OD-01/02/03/05/06/07 OPEN. Gate stays 1; zero code.

**Last Updated:** 2026-09-26 — **INTAKE: CR-390 registered (P2 / LOW, NEW sprint `modules_pdf`, Gate 1 — BRAINSTORM MODE)** — Screen Reference PDFs for ALL modules, repeating the PMS v2.0 exercise (real routes + dummy data → PDF) through a **committed, reusable pipeline** (PMS generator scripts were never committed and are lost). Code reality PARTIAL (PMS PDFs exist, tooling absent). DISTINCT. Module inventory M1–M6 (~90–100 screens). **OD-390-01…08 OPEN** (data strategy · persona · granularity · ordering · packaging · public surface · sequencing · template) — Gate 2 blocked until locked. Registry 726 items. Zero code. Intake `change_requests/CR-390_MODULES_SCREEN_REFERENCE_PDF_ALL_MODULES_INTAKE.md`.
...
**Last Updated:** 2026-09-25 — **PLANNING Gate 3 COMPLETE: CR-387 Implementation Plan written** → `plans/CR-387_IMPLEMENTATION_PLAN.md`. 19 edits / 6 files (`purchasePlanner.js` +4 fields ×2 · `AutoShoppingList.jsx` 9 · `SmartPurchasePanel.jsx` 5 · `GroupedVendorPreview.jsx` 1 · `quantityBreakdown.js` +`rowQuantity` · NEW planner jest). V1–V13. **OD-387-06 probe BLOCKED** (QA_INV creds absent; script ready, read-only; plan default = base basis). Registry `GATE_3_PLAN_COMPLETE` 4/7. **Preconditions: BUG-455 5b + BUG-459 5a.** Zero code. **Next: owner "Gate 4 GO".**

**Last Updated:** 2026-09-25 — **PLANNING Gate 2 CLOSED: CR-387 — all ODs locked by owner** ("All recommended"). **OD-387-01 = (b) display unit + display qty** · OD-02 YES rate per display unit · OD-03 (a) table-2 read-only text · OD-04 YES · OD-05 YES · OD-06 (a) probe `vendor-item-list` in Gate 3 · OD-07 (a) empty · OD-08 (a) after BUG-455 5b + BUG-459 5a. Registry `GATE_2_CLOSED` 3/7. Zero code. **Next: owner verbatim "Gate 3 GO" → plan after BUG-459 plan.**

**Last Updated:** 2026-09-25 — **PLANNING: CR-387 Gate 2 COMPLETE, presented to owner (workspace synced with remote HEAD `0a33e06`).** Status `GATE_2_IMPACT_ANALYSIS` held; **Gate 3 BLOCKED** on OD-387-01…08 (06 = read-only `vendor-item-list` probe in Gate 3). Zero code. Next: owner locks ODs → verbatim "Gate 3 GO" → plan after BUG-459 plan.

**Last Updated:** 2026-09-25 — **PLANNING Gate 2 COMPLETE: CR-387 Impact Analysis written** → `impact/CR-387_IMPACT_ANALYSIS.md`. ~17 edit sites / 4 files: `purchasePlanner.js` (+`small_unit`, `conversion_factor`, `has_conversion`, `display_qty_parts`; math untouched) · `AutoShoppingList.jsx` (breakdown on Projected Need / Gap / Suggested, two-box Qty to Buy) · `SmartPurchasePanel.jsx` (payload `Unit: display_unit` + display qty, CR-348 rate divisor re-based) · `GroupedVendorPreview.jsx`. R11 PASS (probe B1–B5). **CONFLICT BUG-455 (after 5b) + DEPENDS BUG-459 util (after 5a).** ODs 01–08 OPEN (06 needs read-only `vendor-item-list` unit_price probe). Status → GATE_2_IMPACT_ANALYSIS. Zero code.

**Last Updated:** 2026-09-25 — **INTAKE: CR-387 registered (P1 / HIGH, sprint `sep_bug_closure`, Gate 1)** — Smart Purchase unit-consistent breakdown for Projected Need / Gap / Suggested / Qty to Buy + two-box qty input + `add-purchase` `Unit: display_unit` + display quantity (backend-accepted, probe B1–B5). Companion **BUG-459** (P0 Stock Audit inflation) in BUG_TRACKER. Registry 719 items. Zero code. Next: Planning Gate 2 after BUG-459 util shape; OD-387-01…05 open.
...
**Last Updated:** 2026-09-24 (sprint `sep_bug_closure` **GATE 2 CLOSED by owner** — Q1–Q6 answered: BUG-453 mute = sound + toast · BUG-451 constant + delete dead fn, **backend probe DONE** (limit=2000 → 200, 561/561; 500 cap reproduced) · BUG-452 S1 remount, silent clear, BUG-334 REVERSED annotation approved · CR-386 no open ODs. **Registry drift fixed** — BUG-451/452/453/CR-386 re-registered (712 items). **Gate 3 NOT started** (owner: "do not start gate 3"); consolidated Gate 3 strategy presented for reconfirmation. Handover `handover/SESSION_HANDOVER_2026_09_24_SEP_BUG_CLOSURE_GATE2_CLOSED.md`.)
**Last Updated:** 2026-09-24 (CR-385 **P5 REGRESSION PASSED — §5.7 registry closure executed, awaiting owner sign-off (§5.8)** — Role 4 report `test_reports/QA_REPORT_2026_09_24_CR385_P5_ROLE4.md`: 34/34 rows PASS, 0 FAIL, Phase 5.5 not triggered; Sessions A′/B/C it.30–34, D17 ALL_PASS, probe pack 5/5, guards 6/6 on origin HEAD `b2db5a0` (hotspots byte-identical by blob sha256; `642ccb8` obsolete). Zero code changes. FU-385-C row added, FU-385-D row updated (owner sentences 2026-09-23 verbatim). CR-385 flips to CLOSED only on the owner’s verbatim word.)
...
**Last Updated:** 2026-09-23 (CR-385 **P5 plan note written** `plans/CR-385_P5_PLAN_NOTE_2026_09_23.md` · OG-PMS-049 triage complete: 762/56/818 full jest; cr385+bug450=128 green; 12 pre-existing suites quarantine-proposed — awaiting owner approval · **Gate 6 combined owner smoke IN PROGRESS** (M2-S01…S13 → M3-S01…S10 → M4-S01…S09 → CM-S01) · **Awaiting owner: "Phase 2 + 3 + 4 smoke OK" + OG-PMS-049 decisions + QA rotation + "Phase 5 GO"**)
# Layer 3 — CR Registry
...
**Last Updated:** 2026-09-20 (PLANNING — **CR-385 HARD GATE 2.6 CLOSED by owner → GATE 3 OPEN.** Owner quote: "08 a 04 ok 05 yes 06 ok update docs and decision and close gate 2.6 follow agent promot and rules". Decisions D52–D54: N7/N8/N9 · O-4 BUG-384/404/413 closed by decision · O-5 BUG-418 folded into M6 · O-6 transform-field ack (OD-385-12 exception) · O-8 = (a) settings toggles on the existing PMS settings page (new module M7, OD-385-12 exception). Backend N7/N8 shipped as settings + verified (BQ-385-17/18, `evidence/CR-385/probes_2026_09_20_n7n8/`); no open backend ask; N11 slab question non-blocking. Closure evidence: `impact/CR-385_IMPACT_ANALYSIS_REV4_GATE_2_6_FINAL.md`. Next: Gate 3 = D5 spike → `plans/CR-385_IMPLEMENTATION_PLAN.md` → owner "close Gate 3". Handover `handover/SESSION_HANDOVER_2026_09_20_CR385_GATE_2_6_CLOSED.md` → superseded by `handover/SESSION_HANDOVER_2026_09_20_CR385_PLANNING_GATE_3.md` (N11 fixed, D14/BQ-19 P2, Gate 4 GO preconditions G4-01…10, D55/D56).)
**Last Updated:** 2026-09-20 — **CR-385: backend build 3 DELIVERED · VERIFIED end-to-end** (BQ-385-08/09/10/14/15/06/12/11 + BQ-16; defects D1–D13 all fixed; evidence `evidence/CR-385/probes_2026_09_20_gate4/`). Mockup v2.28 (B2B removed from Booking). Money contract frozen (DESIGN_DECISIONS D50). Backend blockers B-1/B-3/B-4/B-5/B-6 closed; remaining B-7 bug smoke · B-8 D5 spike · B-9 owner "close Gate 2.6". Owner questions N7/N8/N9 open. Trackers: `public/cr385-impact-questions.html`, `public/backend-briefs.html`.
**Last Updated:** 2026-09-17 — **CR-385 GATE 2 CLOSED by owner. Backend brief `backend_briefs/BACKEND_BRIEF_CR-385_2026-09-17.md` (BQ-385-01/02/03). Hard gate 2.6 (IA re-validation after design) added before Gate 3. Next: Gate 2.4 UX flow.** OD-385-12: NEW beta page, no existing page touched. IA: `impact/CR-385_IMPACT_ANALYSIS.md`. 13 NEW files + App.js +2. New ODs: OD-385-09/10/11 + SC-385-01. FU-385-B verified (Sync exists on OTA page). Probe: LR `balance_payment` unreliable → Q4 stays folio-based. Backend brief HELD until Gate 2 close. Handover: `handover/SESSION_HANDOVER_2026_09_17_CR385_GATE2_IA.md`.
**Last Updated:** 2026-09-16 — **CR-385 INTAKE CLOSED by owner (Gate 1 → next Gate 2). UX options → mockup variants.** PMS Front Desk Unified Tabbed Workstation UX Revamp (P1 HIGH). Owner selected Option B (tabbed workstation). 4 candidate tabs: Arrivals / In-House / Departures / Room Status. Code reality NONE. Blast: LARGE (~7 files). 7 ODs open for Gate 2. Gate 2.5 design freeze MANDATORY. Registry: 686 items. Intake doc: `change_requests/CR-385_PMS_FRONTDESK_UNIFIED_WORKSTATION_UX_REVAMP_INTAKE.md`.
...
|---|---|---|---|---|---|---|---|---|
| **CR-390** | Screen Reference PDFs for ALL modules — repeat the PMS "real routes + dummy data → screenshots → PDF" exercise for POS Core (M1), Menu & Setup (M2), Inventory & Recipes (M3), Expenses & Staff (M4), Reports ×3 (M5a/b/c), PMS regen (M6) via a committed, parameterised pipeline (`frontend/scripts/screen-reference/`, planned) | P2 | LOW (→ MEDIUM only if OD-390-01 = (c) in-app demo flag) | **GATE_1_INTAKE — BRAINSTORM 2026-09-26** | 1 | PARTIAL — `public/MyGenie_PMS_Screen_Reference.pdf` v2.0 (38 pp / 36 screens) + v1.0 exist; generator scripts `/app/pms_dummy_data.py`, `/app/generate_pms_pdf_v2.py` LOST (never committed) | SMALL code (0 runtime files) / LARGE surface (~90–100 screens) | Source OWNER-REPORTED. DISTINCT (in-app PDF *export* items BUG-041/180/302, CR-086/089 unrelated). **Brainstorm agenda = OD-390-01 data strategy (rec. (b) Playwright `/api/*` interception + fixtures) · 02 shared persona · 03 granularity (rec. primary + key states) · 04 ordering · 05 packaging (rec. master + per-module + PNG packs) · 06 output location vs CR-372 public surface · 07 sequencing (rec. pilot M4 Expenses & Staff) · 08 PMS v2.0 template as standard.** Gate 2 blocked until all locked. Intake `change_requests/CR-390_MODULES_SCREEN_REFERENCE_PDF_ALL_MODULES_INTAKE.md` · evidence `evidence/CR-390/EVIDENCE_POINTER.md`. |

---

## 2026-09-25 — SMART PURCHASE UNIT BREAKDOWN (sprint `sep_bug_closure` — GATE 1 INTAKE)

| CR ID | Title | Priority | Risk | Status | Gate | Code reality | Blast | Notes |
|---|---|---|---|---|---|---|---|---|
| **CR-387** | Smart Purchase (Stock Update): unit-consistent quantity breakdown (`whole displayUnit remainder smallUnit`) for Projected Need / Gap / Suggested / Qty to Buy in both tables + two-box purchase qty input + `add-purchase` payload `Unit: display_unit`, `quantity` in display units (CR-348 rate re-based per display unit) | P1 | HIGH | **GATE_5A_IMPLEMENTED 2026-09-25** | 3 | NONE — planner display values for On-Hand only (`purchasePlanner.js` L130–131); `SmartPurchasePanel.jsx` L216–217 sends `unit: r.unit` (smallUnit) + base qty | MEDIUM (3 files + shared util from BUG-459, no hotspot) | Source OWNER-REPORTED. Backend contract live-verified (probe B1–B5: `Unit:"pkt"` ×800, `Unit:"gm"` ×1, `"xyz"` → 422 `PURCHASE_UNIT_NOT_COMPATIBLE`, decimals OK, no display flip). RELATED BUG-240 (On-Hand only), CR-078, BUG-455 (same files, Gate 5A), CR-348, BUG-459 (util owner). OD-387-01 payload basis (owner leaning display) · OD-387-02 rate semantics · OD-387-03 bottom table · OD-387-04 vendor preview · OD-387-05 rounding — OPEN. Intake `change_requests/CR-387_SMART_PURCHASE_UNIT_BREAKDOWN_DISPLAY_UNIT_PAYLOAD_INTAKE.md` · evidence `evidence/CR-387/PROBE_POINTER.md` · mockups `frontend/public/inv-*proposal*.html`. 4 probe purchase rows remain on preprod (₹103.50). |

---


## 2026-09-24 — PWA INSTALL SUPPORT (sprint `sep_bug_closure` — GATE 2 CLOSED, Gate 3 not started)

| CR ID | Title | Priority | Risk | Status | Gate | Code reality | Blast | Notes |
|---|---|---|---|---|---|---|---|---|
| **CR-386** | PWA install support — `public/manifest.json` + `logo192/512(+maskable).png` + `index.html` manifest/apple-touch-icon links, `theme-color #329937` | P2 | LOW | **GATE 5A IMPLEMENTED 2026-09-24** (Wave 1, commit `3783ee8`; manifest + 3 PNGs served 200 on preview, head tags ×3 verified; testing_agent PASS). Owner smoke VA-3..5 + re-install shortcut pending → QA PASS → CLOSED. | 2 | NONE | SMALL | Zero `src/**`. Approved icons in `evidence/CR-386/approved_A_*.png` — copy to `public/` at Gate 5 only. Side effect: `firebase-messaging-sw.js` `/logo192.png` 404 → fixed for free. Post-deploy owner step: delete old shortcut, re-install. Intake `change_requests/CR-386_PWA_INSTALL_MANIFEST_ICONS_INTAKE.md` · IA `impact/CR-386_IMPACT_ANALYSIS.md` · design brief `design_briefs/DESIGN_BRIEF_CR-386_APP_ICON_2026_09_23.md`. Re-registered in `registry.json` 2026-09-24 (drift). |
...
| CR ID | Title | Priority | Risk | Status | Gate | Code reality | Blast | Notes |
|---|---|---|---|---|---|---|---|---|
| **CR-385** | PMS Front Desk — Unified Tabbed Workstation UX Revamp | **P1** | **HIGH** | **2026-09-24 P5 REGRESSION PASSED — awaiting owner sign-off (§5.8); NOT CLOSED yet.** Role 4 `test_reports/QA_REPORT_2026_09_24_CR385_P5_ROLE4.md` 34/34 PASS · 0 FAIL · Sessions A′/B/C (it.30–34) · D17 legs A/B/C ALL_PASS · probe pack 5/5 (+ held_fallback skipped-no-recipe) · guards 6/6 (origin HEAD `b2db5a0`, hotspots byte-identical by sha256) · zero code changes · sandbox restored (owner r1 #256 untouched) · findings F1–F13 MINOR/NOTE only. FU-385-C / FU-385-D separate CRs. Prior: **2026-09-21 Phase 0 IMPLEMENTED + Phase 0.5 CLOSED (BUG-434…438 QA-VERIFIED, rounds 1–2); Gate 5B (P0+P0.5) CLOSED by owner; BUG-439 MINOR registered, routing open (D72); awaiting owner "Phase 0 smoke OK" before Phase 1.** GATE 2 CLOSED by owner 2026-09-17. Backend brief sent. Gate 2.4 merged into 2.5; clickable mockup LIVE `public/cr385-frontdesk-mockup.html` (MV-01..09 + Q1/2/3/5 switches, Freeze→JSON) awaiting owner freeze → HARD GATE 2.6 IA re-validation → Gate 3. OD-385-12 LOCKED: NEW beta page, NO existing page touched.** 13 NEW files + App.js +2 lines. Check-In form + RoomTile are COPIES (source untouched; mirror rule R9). Cutover → FU-385-C. LOCKED 2026-09-17: route `/pms/front-desk-v2`, sidebar item 'Front Desk (Beta)', `?tab=` deep link, degrade-Rooms-tab-only. LOCKED OD-385-15: folio + checkout on ONE screen (`FolioCheckoutPanel` = folio cards + inline `CollectPaymentPanel` + Print Bill; Print Folio disabled until CR-364-PRINT). No drawer on new page. OPEN: MV-01 layout (mockup). FU-385-B VERIFIED. Backend brief HELD until Gate 2 close. | 2.5 CLOSED (v2.27; mockup v2.28) → **2.6 CLOSED by owner 2026-09-20 (D54) → 3 OPEN** — backend DELIVERED·VERIFIED (build 3 + N7/N8 settings BQ-17/18); O-1..O-8 answered; B-7 smoke gates M3–M6 only; B-8 spike **DONE** (D57, Q6 = a) · Implementation Plan written 2026-09-20 · **GATE 3 CLOSED 2026-09-21 (owner quote D69) — execution plan `plans/CR-385_IMPLEMENTATION_PLAN_PHASED.md` P0–P5 with owner smoke after each phase; awaiting owner "Gate 4 GO"** | NONE | LARGE | IA: `impact/CR-385_IMPACT_ANALYSIS.md`. Evidence: `evidence/CR-385/` (4 probes 200; LR balance_payment unreliable → Q4 folio-based). Handover: `handover/SESSION_HANDOVER_2026_09_17_CR385_GATE2_IA.md`. Intake: `change_requests/CR-385_PMS_FRONTDESK_UNIFIED_WORKSTATION_UX_REVAMP_INTAKE.md`. Sprint: pos_pms_2. |
| **FU-385-C** | Front Desk cutover — retire legacy PMS pages (New Booking, Check-In, Departures drawer, Arrivals, Room Status copies), re-point/redirect `/pms/front-desk`, drop "(Beta)" label, sidebar review (merges FU-385-A); diff copies vs sources (RoomTile, CheckInForm) | P2 | HIGH | **PLANNED — separate CR after CR-385 closes (owner 2026-09-23 verbatim: "separate CR after CR-385 closes. Gate 5 closes without it; BUG-431/432/443/444/446/449, X-06 and legacy rounding stay DEFERRED-TO-FU-385-C with that sentence in the sign-off.")** | — | Legacy pages + dialogs unchanged during CR-385 (OD-385-12); copies carry source line ranges in headers | LARGE (11 legacy files) | Carries: BUG-431/432/433 (legacy displays), BUG-443/444/446/449, X-06 legacy browser date, legacy rounding. Opens after the CR-385 sign-off word. |
| **FU-385-D** | Front Desk (Beta) Bill — Split (multi-method) settlement for room stays (hidden by D88 host CSS in CR-385 P4.5b) | P2 | MEDIUM | **PLANNED — separate follow-up CR (owner 2026-09-23 verbatim: "separate follow-up CR. D88 stays; `frontdesk.css` + `hideSectionRows` guard unchanged; OG-PMS-048 stays OPEN pending FU-385-D.")** · P5 closure 2026-09-24: OG-PMS-048 stays OPEN pending FU-385-D; D88 rule + guard unchanged (FINAL_GUARDS G3/G5). Prior: **PLANNED 2026-09-22 (D88, OG-PMS-048)** — not started; opens after CR-385 Phase 5 / with FU-385-C decision | — | Panel Split exists (`payment-split-btn`), hidden only under `.frontdesk-bill` | SMALL (1 CSS rule + guard test + smoke row M4-S07) | Decide UX (panel Split inside the 440×560 host vs Front Desk split form); re-enable = delete `frontdesk.css` D88 rule + `hideSectionRows` guard line; smoke M4-S07 with Split. Related: BUG-449 (legacy TAB) in FU-385-C. |
...
| CR ID | Title | Priority | Risk | Status | Gate | Code reality | Blast | Notes |
|---|---|---|---|---|---|---|---|---|
| **CR-361** | PMS — Room Assignment on Tape Chart | P1 | MEDIUM | **INTAKE — BACKEND-BLOCKED** | 1 | PARTIAL | MEDIUM | Needs `PATCH local-reservations/{id}/rooms/{line}` (all candidate routes 404) + OG-PMS-013 board soft-allocation ruling. `change_requests/CR-361_PMS_ROOM_ASSIGNMENT_TAPE_CHART_INTAKE.md` |
| **CR-362** | PMS — Booking Modification & Cancellation (cancel / modify / extend stay / no-show) | P1 | **CRITICAL** | **INTAKE — BACKEND-BLOCKED** | 1 | NONE | LARGE | 3 new write endpoints needed; only `mark-no-show` exists (booking.com/gommt). Money + inventory → owner approval at Gate 4. `change_requests/CR-362_PMS_BOOKING_MODIFICATION_CANCELLATION_INTAKE.md` |
| **CR-363** | PMS — Night Audit Report | P1 | HIGH | **GATE 5A — IMPLEMENTED 2026-09-14. NightAuditPage.jsx + nightAuditTransform.js NEW. Webpack 0 new warnings. Self-test 18/18 PASS. Live on preprod. Awaiting QA.** | 2 | NONE | MEDIUM | `change_requests/CR-363_PMS_NIGHT_AUDIT_REPORT_INTAKE.md`; `impact/CR-363_CR-366_JOINT_IMPACT_ANALYSIS.md`; Brief: `BACKEND_BRIEF_CR363_NIGHT_AUDIT_2026_09_16.md` |
| **CR-364** | PMS — Guest Folio Detail Page | P1 | HIGH | **GATE 5A — IMPLEMENTED 2026-09-14. GuestFolioPage + folioTransform NEW. 3 link re-points done. Live on preprod. Awaiting QA.** | 2 | PARTIAL | MEDIUM | `change_requests/CR-364_PMS_GUEST_FOLIO_DETAIL_PAGE_INTAKE.md` (updated 2026-09-14); Investigation `CR-364_INVESTIGATION_CIB_COMPARISON.md`; Backend Brief `backend_briefs/BACKEND_BRIEF_CR364_FOLIO_PRINT_2026_09_16.md` |
| **CR-364-PRINT** | PMS — Guest Folio Print Template (`rtype='RM'` branch, ~60-field payload) | P2 | HIGH | **INTAKE — BACKEND-BLOCKED (2026-09-14). Scope split from CR-364. Q-364P-01/08/09/10/13/15 unanswered. Parked until BE answers print questions. `orderTransform.js` R5 + R6 print semantics. Code reality: PARTIAL (`rtype` already emitted at L2216).** | 1 | PARTIAL | SMALL | `change_requests/CR-364-PRINT_PMS_FOLIO_PRINT_TEMPLATE_INTAKE.md`; Backend Brief `backend_briefs/BACKEND_BRIEF_CR364_FOLIO_PRINT_2026_09_16.md` |
| **CR-365** | PMS — Housekeeping Workflow | P2 | MEDIUM | **INTAKE — UNBLOCKED 2026-09-13** (registry flag drift corrected 2026-09-16). All HK endpoints live; B-365-04/07 answered; sockets deferred. Prerequisite BUG-397 (Gate 5b PASS, awaiting Gate 6). Gate 2 blocked only on OD-365-NEW-01 laundry scope (ties CR-381 OD-381-01). | 1 | PARTIAL | LARGE | `change_requests/CR-365_PMS_HOUSEKEEPING_WORKFLOW_INTAKE.md` (updated 2026-09-13) |
| **CR-366** | PMS — Revenue Dashboard / Analytics | P2 | MEDIUM | **GATE 5A — IMPLEMENTED 2026-09-14. RevenueDashboardPage.jsx + revenueTransform.js NEW. 3 charts + 4 tables live on preprod. BN-6 month suppression. Awaiting QA.** | 2 | NONE | MEDIUM | `change_requests/CR-366_PMS_REVENUE_DASHBOARD_ANALYTICS_INTAKE.md` · `impact/CR-363_CR-366_JOINT_IMPACT_ANALYSIS.md` · `backend_briefs/BACKEND_BRIEF_CR366_REVENUE_AGGREGATION_2026_09_15.md` |
| **CR-367** | PMS — WhatsApp / SMS Guest Notifications | P2 | HIGH | **INTAKE — BACKEND-BLOCKED** | 1 | NONE | MEDIUM | No guest-messaging endpoint (7 routes 404); only order-bound payment link. Provider/DLT compliance decisions. `change_requests/CR-367_PMS_WHATSAPP_SMS_GUEST_NOTIFICATIONS_INTAKE.md` |
| **CR-358-P5** | PMS Phase 5 — Rate Grid + Inventory Restrictions + Rate Restrictions (S8-C full) + Mark No-Show (S8-D) | P1 | HIGH | **GATE_5B_QA_PASS (2026-09-09). 8/8 PASS, 13 NOTE deferred. Core: Rate grid✅ popover✅ Inv Restrictions✅ No-Show OTA✅ regression✅. Ready for Gate 6 owner smoke.** `test_reports/QA_REPORT_CR358P5_2026_09_09.md` | 3 | PARTIAL | MEDIUM | `impact/CR-358-P5_IMPACT_ANALYSIS.md` · `plans/CR-358-P5_IMPLEMENTATION_PLAN.md` · `design_guidelines.json` |
| **CR-358-P5.1** | PMS Phase 5.1 — Inventory + Rate Restrictions (deferred, RETIRED) | — | — | **RETIRED 2026-09-08** — B-P5-01 resolved; merged back into CR-358-P5 | — | — | — | Retired |
...
## 2026-09-03 — CR-358-P3 (GATE 2 — IMPACT ANALYSIS)
...
## 2026-09-03 — CR-360 (GATE 3 COMPLETE)
...
||---|---|---|---|
||| **CR-051** | **Customer Field Mandatoriness Override (Visibility Section).** 5 localStorage toggles: Walk-in Name/Phone, Dine-in Name/Phone, TakeAway Phone. Validation at Place Order + Collect Bill + QSR — same pattern as Delivery. Per-device, no backend sync. | **CLOSED — OWNER VERIFIED (retroactive 2026-08-22). Code investigation confirmed: `StatusConfigPage.jsx` CR-051 markers line 204, full toggle UI wired, `OrderEntry.jsx` + `CartPanel.jsx` validation live. Done under CR-051.** | `StatusConfigPage.jsx`, `OrderEntry.jsx`, `CartPanel.jsx` |
...
||||| **CR-060** | **Table/Room Management — Wire CRUD APIs to Existing UI.** `TableManagementView.jsx` (272 lines) has full UI shell with ALL CRUD mocked (toast-only, zero API calls). 8 endpoints mapped. REWRITE with real APIs, Dialog (Type toggle TB/RM + waiter assignment), `TableBulkEditor.jsx` (NEW, spreadsheet grid), Excel import/export, `comingSoon` flag removed. **SUBSUMES BUG-148.** Phase 2: QR codes + waiter permissions (parked). | **CLOSED — OWNER VERIFIED (retroactive 2026-08-22). Code investigation confirmed: `tableService.js` CR-060 markers, `storeTable()`/`deleteTable()` wired, `TableBulkEditor.jsx` exists, `TableManagementView.jsx` real CRUD (not mocked), `comingSoon` removed from Sidebar. Done under CR-060.** | `TableManagementView.jsx` (REWRITE), `TableBulkEditor.jsx` (NEW), `tableService.js`, `tableTransform.js`, `constants.js` (+TABLE_CONFIG_ENDPOINTS), `Sidebar.jsx` (comingSoon flag) |
...
| **CR-090** | Inventory Categories — Delete (trash icon, confirm, 3 error paths). Edit deferred (no backend PUT). | **P2** | MEDIUM | **IMPLEMENTED** | 5a | 3 files: constants.js, inventoryService.js, InventorySetupPanel.jsx. ~30 lines. |
...
| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-100** | Smart Purchase — Partial Payment, Reference ID, Notes & Unpaid Status | **P1** | HIGH | **IMPLEMENTED — awaiting QA** | 0-5a ✅ | Backend delivered. FE: 3 files changed (GroupedVendorPreview.jsx full rewrite → Paid/Partial/Unpaid tabs + split rows; SmartPurchasePanel.jsx validate()+handleSubmit(); inventoryTransform.js addPurchase()). Contract deltas applied: `payment_type` enum, `partial_payments[].payment_mode`, `.transaction_id`. `notes`+`paid_amount` dropped (owner 2026-08-08). Plan: `plans/CR-100_IMPLEMENTATION_PLAN.md`. |
...
| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-106** | Aggregator Integration Module (UrbanPiper / Swiggy / Zomato) — Full lifecycle: socket subscription, boot fetch, accept/reject/ready/dispatch, PopOut with auto-accept (CR-109), S/Z badges, platform filter, 8 post-delivery bugs fixed. | **P1** | HIGH | **CLOSED — OWNER VERIFIED** | 0-6 ✅ | 14 files, 51 code markers. Retroactive closure 2026-07-31. |

### 2026-08-06 CRM Customer Intelligence Beta Screens (CR-131)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-131** | Customer Intelligence (Beta) + Guest vs Registered (Beta) — CRM-enriched report screens | **P2** | **MEDIUM** | **IMPLEMENTED — Gate 5a 2026-08-06** | 0-2 ✅, Gate 3 ✅, Gate 4 GO ✅, 5a ✅ | 6 files: NEW `crmReportService.js`, NEW `CustomerIntelligenceBeta.jsx`, NEW `GuestVsRegisteredBeta.jsx`, MODIFY `constants.js`/`Sidebar.jsx`/`App.js`. CR-078 Phase 1 endpoints. Platinum tier, no date picker, data.count badges, no day-numbers in lifecycle labels, WhatsApp=wa.me/91{phone}. Old screens untouched ✅. Compile PASS. EXIT GATE 5/5. QA: `handover/QA_HANDOVER_CR131_2026_08_06.md`. |

---

### 2026-08-06 BILL Printer in Place Order (CR-130)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-130** | Add BILL Printer to `printer_agent` Array in Place Order Payload | **P1** | **HIGH** | **QA PASS (Gate 5b, 2026-08-06) — Awaiting Gate 6 Owner Smoke** | 0-5b ✅ | 1 file: `orderTransform.js` (R5). Edit 1: L4 import added `selectAgentsForBill`. Edit 2: placeOrder L1013-1016 `printerAgentForKot` + `[...kot, ...bill]`. Edit 3: placeOrderWithPayment L1277-1280 same. Cancel/update paths UNCHANGED. QA: T1 printer_agent=['KDS','BILL'] ✅, T4 KOT preserved ✅. Report: `test_reports/QA_REPORT_BUG300_CR130_2026_08_06.md` |

---

### 2026-08-05 Room Check-In Document Preview (CR-129)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-129** | Room Check-In — Docs on File + Phone +91 Fix + File Thumbnail Preview | **P2** | **HIGH** | **IMPLEMENTED — Gate 5a PASS 2026-08-05** | 0-5a ✅ | 12 edits, 3 files. A: `constants.js` +CUSTOMER_DOCUMENTS. B: `documentService.js` NEW (~40L, normaliser for object→array). C1-C12: `RoomCheckInModal.jsx` — ShieldCheck import, remove PhoneInput/isValidPhoneNumber, 3 state vars, doc-fetch useEffect, selectCrmCustomer hook-in, handleNameChange/handlePhoneChange reset, validate() fix, +91 prefix input, FileField thumbnail rewrite, CRM docs section, remove PhoneInput CSS. Compile PASS. EXIT GATE 5/5. QA: `handover/QA_HANDOVER_CR129_2026_08_05.md` |
...
| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-107** | Aggregator Auto-Accept (with Dynamic Prep Time) — When `auto_prep_time_ack` enabled, auto-accept new aggregator orders with computed prep time. Backend settings exist, FE feature NOT built. | **P2** | HIGH | **QA PASS (Gate 5b — 2026-07-31)** | 0-1 ✅ | 3+ files, ~100 lines. Needs Gate 2-3 planning. Related: CR-106, CR-109. |
| **CR-108** | Wire Auto-KOT on Aggregator Accept — `aggregator_auto_kot: Yes` in settings, Settings UI exists, but accept handler doesn't trigger KOT. Also: `aggregator_auto_bill` + `aggregator_auto_bill_stage`. | **P2** | MEDIUM | **CLOSED — OWNER VERIFIED (retroactive 2026-08-22). Code investigation confirmed: `AggregatorOrderPopOut.jsx:67` — `settings?.aggregatorAutoKot` wires KOT checkbox. `printerAgentConfigTransform.js:209` + `restaurantSettingsTransform.js:157` map the field. Done as part of CR-135 aggregator session.** | 0-6 ✅ | Done under CR-135/aggregator batch. |
| **CR-109** | Dynamic Prep Time Calculation — Backend has `default_prep_time`, `prep_time_count_method: quantity`, `prep_time_bonus_config`. FE shows static pills only. Pre-compute and pre-select optimal pill. | **P2** | LOW | **QA PASS (Gate 5b — 2026-07-31)** | 0-1 ✅ | 1-2 files, ~20 lines. Related: CR-107. |
| **CR-110** | MyGenie Brand Badge for Own Delivery Orders — Add "M" (green) badge on own delivery orders, matching S/Z pattern for Swiggy/Zomato. Staff can distinguish at a glance. | **P2** | LOW | **QA PASS (Gate 5b — 2026-07-31)** | 0-1 ✅ | 2 files (`TableCard.jsx`, `OrderCard.jsx`), ~15 lines each. OQs: badge design (M letter vs bell icon), color. |

### 2026-07-27 OrderCard Aggregator Enhancements — CLOSED retroactive 2026-08-22

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-111** | OrderCard item format: ● Qty× Name pattern (design mockup alignment) | **P3** | LOW | **CLOSED — OWNER VERIFIED (retroactive 2026-08-22)** | 0-6 ✅ | `OrderCard.jsx:749` CR-111 marker confirmed. Done in aggregator batch. |
| **CR-112** | OrderCard item price display (₹ per item line) | **P3** | LOW | **CLOSED — OWNER VERIFIED (retroactive 2026-08-22)** | 0-6 ✅ | `OrderCard.jsx:755` CR-112 marker confirmed. Done in aggregator batch. |
| **CR-113** | OrderCard customer+phone section for aggregator | **P3** | LOW | **CLOSED — OWNER VERIFIED (retroactive 2026-08-22)** | 0-6 ✅ | `OrderCard.jsx:912` CR-113 marker confirmed. Done in aggregator batch. |

### 2026-07-27 Smart Purchase UX Enhancements (CR-114, CR-115)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-114** | Smart Purchase — Default All Items Unselected (Opt-In). All planner items shown by default, overwhelming with 100+ items. Add opt-in checkbox, unchecked by default. | **P2** | MEDIUM | **QA PASS (Gate 5b — 2026-07-31)** | | 0-1 ✅ | 2 files (`SmartPurchasePanel.jsx`, `AutoShoppingList.jsx`), ~30-40 lines. Related: CR-103, CR-105. Intake: `change_requests/CR-114_SMART_PURCHASE_DEFAULT_UNSELECTED_INTAKE.md` |
| **CR-115** | Smart Purchase — Search Filter + Sort by Category. No search/filter/category sort in 100+ item list. Add filter bar: search input + category dropdown. | **P1** | MEDIUM | **QA PASS (Gate 5b — 2026-07-31)** | | 0-1 ✅ | 1-2 files, ~60 lines. Related: CR-114. Intake: `change_requests/CR-115_SMART_PURCHASE_SEARCH_SORT_CATEGORY_INTAKE.md` |
| **CR-116** | B2B Customer GST & Name Capture — New Input Fields in Collect Bill. `custGST` and `custGSTName` hardcoded empty in print payload. No UI exists. Need input fields + wire to settle/print. | **P1** | MEDIUM | **QA PASS (Gate 5b — 2026-07-31)** | 0-1 ✅ | 2 files: `CollectPaymentPanel.jsx` + `orderTransform.js`, ~50 lines. Owner decisions: where to show fields, always or B2B-toggle. Intake: `change_requests/CR-116_B2B_CUSTOMER_GST_CAPTURE_INTAKE.md` |

### 2026-07-30 Aggregator + Report Beta Intake Batch (CR-117 → CR-119)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-117** | Order Report Beta — Combined backend-aggregated daily order report. KPI strip, per-day collapsible sections, 11-column table, grand total footer, Excel export. Tabs: All + Aggregator (6 blocked pending backend). Filters: 6 active + 2 blocked. | **P1** | LOW | **QA PASS (Gate 5b — 2026-07-31)** | 5a ✅ | 5 files (1 new + 4 modified). ~460 lines. Backend brief filed for 9 missing fields. |
| **CR-118** | Aggregator KOT & Bill Manual Print — Manual KOT/Bill print for aggregator orders via `/urbanpiper/manually-print-aggregator`. PopOut KOT/Bill checkboxes (auto-setting defaults), card print buttons (fOS 1+2), `aggrigator_id` display fix, "Ready to Dispatch" label. | **P1** | MEDIUM | **QA PASS (Gate 5b — 2026-07-31)** | 5a ✅ | 7 files: `aggregatorTransform.js`, `profileTransform.js`, `constants.js`, `aggregatorService.js`, `AggregatorOrderPopOut.jsx`, `OrderCard.jsx`, `TableCard.jsx`. ~135 lines. | |
| **CR-119** | Aggregator Food Mapping — New panel to map POS menu items to aggregator platform items (Swiggy/Zomato ref_ids). | **P1** | MEDIUM | **IMPLEMENTED Gate 5a — QA PENDING (Gate 5b)** | 0-5a ✅ | 4 files: `constants.js` (+RECIPE_MAPPING_ENDPOINTS, 5 endpoints), `recipeService.js` (+getClients/getMapping/save/unlink/batch/export), `AggregatorInventoryTab.jsx` (NEW, 341 lines — brand selector, mapping table, combobox, batch save, xlsx export), `RecipeManagementPanel.jsx` (5th "Aggregator" tab wired). Related: CR-106. QA not yet run — needs Gate 5b before Gate 6 owner smoke. |
...
|---|---|---|---|---|---|---|
| **CR-120** | Aggregator: Split KOT/Bill Buttons by Order Status — preparing (fOS=1) shows KOT only, ready (fOS=2) shows Bill only. Currently both show at both statuses. | **P2** | LOW | **QA PASS (Gate 5b — 2026-07-31)** | 0-1 ✅ | 1 file: `OrderCard.jsx` — 2 line condition changes (lines 1013 + 1082). Related: CR-106, BUG-285. Intake: `change_requests/CR-120_AGGREGATOR_KOT_BILL_SPLIT_INTAKE.md`. |
| **CR-121** | Dashboard Quick-Start — Single-click order creation for Walk-in / Takeaway / Delivery from dashboard. Pre-selects order type, navigates to OrderEntry. | **P2** | MEDIUM | **QA PASS (Gate 5b — 2026-07-31)** | 0-1 ✅ | ~2-3 files (DashboardPage.jsx R5 hotspot). OQ-1: button placement. OQ-2: walk-in flow. OQ-3: customer prompt per CR-051. Intake: `change_requests/CR-121_DASHBOARD_QUICK_ORDER_INTAKE.md`. |
...
| Files | `InventoryTabBar.jsx`, `Sidebar.jsx`, `SmartPurchasePage.jsx`, `SmartPurchasePanel.jsx` |
| Related | CR-078 (original delivery), BUG-263 (sticky toolbar — separate) |
| Notes | 9 user-facing label occurrences across 4 files. Route paths + component names MUST NOT change. GroupedVendorPreview moves above item list. Owner decisions pending: OD-1 toolbar button rename, OD-2 API notes field. Intake: `change_requests/CR-122_INVENTORY_STOCK_UPDATE_RENAME_UX_INTAKE.md` |
...
| Risk | LOW |
| Parent | CR-122 |
| Area | Inventory → Stock Update |
| Files | `SmartPurchasePanel.jsx` only |
| Related | CR-122 (parent), BUG-263 (toolbar sticky — different element, already done) |
| Impact Doc | `impact/CR-123_IMPACT_ANALYSIS.md` |
| Plan Doc | `plans/CR-123_IMPLEMENTATION_PLAN.md` |
| Notes | Owner approved Option B. Gate on `activeRows.length > 0` (not `canSubmit`) so spinner visible during submit. `pb-20` on container. 2 edits, 1 file. Fast Lane eligible. Awaiting Gate 4 GO. |

### CR-125 — Aggregator Brand Name: Display + Print (2026-08-01)
| ID | CR-125 |
|---|---|
| Title | Aggregator Brand Name — Display on OrderCard + Include in Print Payload |
| Severity | P2 |
| Risk | HIGH (print payload) |
| Status | **CLOSED — OWNER VERIFIED (retroactive 2026-08-22)** |
| Related | CR-118 (aggregator print, RELATED) |
| Blast Radius | SMALL — 3 files, ~15 lines |
| Files changed | `aggregatorTransform.js`, `OrderCard.jsx`, `aggregatorService.js` |
| Notes | Code investigation 2026-08-22 confirmed: `aggregatorService.js:19` CR-125 marker. `brand_name` mapped in `aggregatorTransform.js:34` → `brandName`. `OrderCard.jsx:275` passes brandName to `manuallyPrintAggregator`. OQ-3 resolved — backend print endpoint includes brand_name. Done under CR-125. |

### CR-127 — Room Check-In: Add `cust_membership_id` to Payload (2026-08-04)
| ID | CR-127 |
|---|---|
| Title | Room Check-In: Add `cust_membership_id` to Check-In Payload |
| Severity | P1 |
| Risk | LOW |
| Status | **IMPLEMENTED** — Code complete 2026-08-04. QA: 11/11 PASS (iteration_12). Compile PASS. EXIT GATE 5/5 PASS. |
| Related | BUG-270 (update-place-order `cust_membership_id`, FIXED), BUG-065 (Corporate Room Check-in, CLOSED), BUG-092 (`customer_id` on room check-in) |
| Blast Radius | SMALL — 1 file, 1 line |
| Files WILL change | `roomService.js` |
| Owner Decisions | Q1=B (different fields), Q2=A (FE adds), Q3=A (backend forwards to CRM), Q9=B (no doc update) |
| CRM Validation | ✅ All endpoints live on `preprod-crm-deploy`. Zero CRM changes needed. |
| Impact Doc | `impact/CR-127_IMPACT_ANALYSIS.md` |
| Plan Doc | `plans/CR-127_IMPLEMENTATION_PLAN.md` |
| Notes | 1 edit: `fd.append('cust_membership_id', ...)` after existing `customer_id`. Verification: 3 checks (1 auto, 2 manual). |

### CR-128 — B2B Customer CRM Wiring: Read + Write + Auto-Populate (2026-08-04)
| ID | CR-128 |
|---|---|
| Title | B2B Customer CRM Wiring — Read + Write + Auto-Populate from CRM v2 Contract (CR-071/CR-072 CRM) |
| Severity | P1 |
| Risk | HIGH |
| Status | **IMPLEMENTED** — Code complete 2026-08-04. QA: 11/11 PASS (iteration_12). Compile PASS. EXIT GATE 5/5 PASS. |
| Related | CR-116 (B2B GST Capture in Collect Bill, QA PASS), BUG-065 (Corporate Room Check-in, CLOSED), BUG-281 (custGST forwarding, QA PASS) |
| Blast Radius | MEDIUM — 2 files, ~60-100 lines |
| Files WILL change | `customerTransform.js` (G1 read + G2 write), `RoomCheckInModal.jsx` (G3 CRM sync + G4 auto-populate) |
| Owner Decisions | Q5=A (auto-silent CRM sync), Q6=B (backend pulls GST, pending recheck), Q7=B (doc capture = separate CR), Q8=A (auto-populate), Q9=B (no doc update) |
| CRM Validation | ✅ All endpoints re-validated live on `preprod-crm-deploy`. customer_type auto-derives. |
| Impact Doc | `impact/CR-128_B2B_CUSTOMER_CRM_WIRING_IMPACT_ANALYSIS.md` |
| Plan Doc | `plans/CR-128_IMPLEMENTATION_PLAN.md` |
| Gaps | G1: 3 transforms (search/lookup/detail). G2: 2 transforms (create/update). G3: CRM sync in handleSubmit. G4: auto-populate in selectCrmCustomer. |
| Notes | 8 edits, 2 files. Verification: 11 checks (6 auto, 5 manual). Doc capture = separate CR. Order GST = backend. |


### 2026-08-07 Printer Agent Config Settings Screen (CR-133)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-133** | Printer Agent Config — Full Settings Screen (Complete Rewrite of PrintersView) | **P1** | **HIGH** | **IMPLEMENTED (2026-08-07)** | 5 | Full rewrite of `PrintersView` stub in `ListFormViews.jsx`. New endpoint `GET+POST /api/v2/vendoremployee/restaurant-settings/printer-agent-config` curl-verified (restaurant 478). 14 config sections: `printers[]` CRUD, `auto_printing` (bill/KOT/aggregator), `paper_settings`, `print_copies`, `qr_codes`, `windows_options`, `bill_display_options`, `restaurant_information`, `bill_footer`, `style_config` (bill+KOT font/size/bold). Owner: full scope, UX priority, UI decided during impact. 5 files: `ListFormViews.jsx` (rewrite), NEW `printerAgentConfigService.js`, NEW `printerAgentConfigTransform.js`, `constants.js`, possibly `SettingsPanel.jsx`. Hotspots NOT touched. OD-1..5 open. Intake: `change_requests/CR-133_PRINTER_AGENT_CONFIG_SETTINGS_INTAKE.md`. Evidence: `evidence/CR-133/get_response.json`. GATE 3 (2026-08-07): Implementation Plan at `plans/CR-133_IMPLEMENTATION_PLAN.md` — live probe re-verified all 9 audit corrections; found leftover Phase-2 test keys on preprod 478 (OD-6); transform = merge-onto-raw (zero data loss); 4-tab UI in new `printerConfig/` dir; 13-check verification matrix. Pending OD-6..9 + owner Gate 4 GO. GATE 4 GO (2026-08-07): OD-6..9 resolved. IMPLEMENTED (2026-08-07): E1-E7 per plan — `constants.js` +PRINTER_AGENT_CONFIG, NEW `printerAgentConfigTransform.js` (merge-onto-raw), NEW `printerAgentConfigService.js`, NEW `printerConfig/` 5 UI files (4-tab), `ListFormViews.jsx` L183-258 stub → thin re-export. 20/20 unit tests (V2-V7 + reinjection helper). QA via testing agent on live 478: V8/V9/V10/V11 + hidden-field integrity + OD-8 affordances + regression ALL PASS. **NEW CRITICAL BACKEND FINDING:** preprod deep-merges `printers[]` by id — ADD persists, DELETE silently re-injected (UI + direct curl confirmed). Frontend mitigation shipped: post-save reconciliation warns when deleted printer is kept. Backend delete support = follow-up. Residue on 478: `printer_new_1786121623349` 'QA Test Printer' — backend/DB cleanup needed (same class as OD-6). QA handover: `handover/QA_HANDOVER_2026_08_07_CR133.md`. |

### 2026-08-08 Restaurant Settings — New Backend Fields (CR-132)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-132** | Restaurant Settings — Wire New Backend Fields to FE (49 fields, 9-screen wizard) | **P1** | **MEDIUM** (3 fields HIGH) | **IMPLEMENTED — QA RE-TEST PENDING (schedule_order BUG-FIX applied 2026-08-11)** | 5 ✅ | IA revised 2026-08-09: 49 fields (was 16), 1 critical regression (room field advanced→basic). Screen architecture confirmed (9 screens). Screen 1 reviewed + approved. Comparison pages built for Screens 3-9 + printable PDF at `/cr132-print`. **Gate 3 blocked on: owner S3-9 feedback, CR-133 ODs (D1-D7, U1-U4), OD-GST-INCEXC.** Files WILL change: `restaurantSettingsTransform.js`, `RestaurantSettingsPage.jsx`. IA: `impact/CR-132_IMPACT_ANALYSIS.md`. S1 review: `handover/SESSION_HANDOVER_2026_08_09_CR132_DESIGN_REVIEW.md`. Comparison pages: `handover/SESSION_HANDOVER_2026_08_09_CR132_COMPARISON_PAGES.md`. |

### 2026-08-08 Settings Tiles — Wizard Mirror (CR-134)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-134** | Settings Tiles — Mirror All Restaurant Setup Wizard Fields | **P2** | **MEDIUM** | **⏸ HOLD — BACKEND PENDING** | 1 | Depends on CR-132. Backend adding more fields → CR-132 must complete first, then CR-134 tile mirror scope can be finalized. OD-1..OD-5 still open. Do NOT plan or implement until CR-132 is implemented + backend field freeze confirmed. Investigation: `investigation/INV-SETTINGS-COVERAGE_2026_08_08.md`. Intake: `change_requests/CR-134_SETTINGS_TILES_WIZARD_MIRROR_INTAKE.md`. |

### 2026-08-09 Aggregator Setup — New Settings Screen (CR-135)
...
| **CR-137** | **Optional `discount_for` Field in Order Payload.** When operator applies a discount, they can optionally enter a reason (e.g. "staff", "event"). Field sent as `discount_for` in place-order, update-order, placeOrderWithPayment, and collectBillExisting payloads. Field is currently always `null` in backend responses — never sent by FE. UI: optional free-text input in CollectPaymentPanel discount section. Order Ledger "Discount For" column will read actual API value. **Owner directive: optional field.** RELATED: CR-058 (mandatory comp note — different scope). **Intake:** `change_requests/CR-137_DISCOUNT_FOR_OPTIONAL_FIELD_INTAKE.md`. 4 files: 2 hotspot (`orderTransform.js` + `CollectPaymentPanel.jsx`), 2 non-hotspot. | **P2** | **HIGH** | **IMPLEMENTED → QA PASS (Gate 5b) 2026-08-12. 35/35 P0 PASS. Ready for Gate 6 owner smoke.** | pos_5_1 | `impact/CR-137_IMPACT_ANALYSIS.md` · `plans/CR-137_IMPLEMENTATION_PLAN.md` · `plans/CR-137_REGRESSION_TEST_LIST.md` · `test_reports/QA_REPORT_CR137_2026_08_12.md` |

| **ID** | CR-136 |
| **Sprint** | pos_5_1 |

| **CR-136** | **Item Sales Ledger + Variation & Addon Sales.** New report screens `/reports-module/item-sales` and `/reports-module/variation-addon-sales`. Powered by `top-food-sales-report` (item-level) and `order-logs-report` (variation/addon parsing). **Intake:** `change_requests/CR-136_ITEM_SALES_LEDGER_INTAKE.md`. **Impact:** `impact/CR-136_IMPACT_ANALYSIS.md`. **Plan:** `plans/CR-136_IMPLEMENTATION_PLAN.md`. **QA:** `test_reports/QA_REPORT_CR136_2026_08_12.md`. 7 files. EXIT GATE 5/5. **QA: 23/23 PASS — 100% coverage.** | **P2** | **MEDIUM** | **QA PASS — Gate 5b (2026-08-12) — Awaiting Gate 6 Owner Smoke** | pos_5_1 | `topFoodSalesService.js` · `variationAddonService.js` · `ItemSalesLedgerMockup.jsx` · `VariationAddonMockup.jsx` · `constants.js` · `App.js` · `Sidebar.jsx` |
...
### 2026-08-14 Addon Master Unified + BulkEditor Addon/Variation (CR-144, CR-145)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-144** | Addon Master — Unified Management in Menu Management Panel (All Menu Types) | **P1** | **MEDIUM** | **IMPLEMENTED ✅** | 5 ✅ | BATCHED with CR-142. Plan: `plans/CR-142_CR-144_COMBINED_IMPLEMENTATION_PLAN.md`. Design approved `/cr144-design.html`. Awaiting Gate 4 GO. |
| **CR-145** | BulkEditor — Addon & Variation Columns + Row Expand / Quick Edit | **P2** | **HIGH** | **IMPLEMENTED ✅** | 5 ✅ | Plan: `plans/CR-145_BULKEDITOR_ADDON_VARIATION_IMPLEMENTATION_PLAN.md`. editFood=POST FormData confirmed. addon_ids:[] clears addons. addons prop gap fixed in E8. Food image in E1a+E9/E10. Awaiting Gate 4 GO. |

---

### 2026-08-14 Addon Master V2 + Aggregator Leftover (CR-142, CR-143)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-142** | Addon Master V2 — Full CRUD Upgrade + Standalone Management UI | **P1** | **HIGH** | **IMPLEMENTED ✅** | 5 ✅ | BATCHED with CR-144. Plan: `plans/CR-142_CR-144_COMBINED_IMPLEMENTATION_PLAN.md`. Curl-confirmed: add-addon POST ✅ put-addon-update PUT ✅ status-change POST ✅ delete-addon DELETE ✅. 5 edits + 1 new file. Awaiting Gate 4 GO. |
| **CR-143** | Aggregator Leftover — Force-Swiggy + Addon Stock + Variation Stock | **P1** | **MEDIUM** | **IMPLEMENTED ✅** | 5 ✅ | Plan: `plans/CR-143_AGGREGATOR_LEFTOVER_IMPLEMENTATION_PLAN.md`. Curl-confirmed: bulk-addons GET ✅ bulk-apply POST ✅ variations GET ✅. force-swiggy 502 → graceful error. toggle-addon 404 → warning toast. 4 edits + 2 new files. Awaiting Gate 4 GO. |

---

### 2026-08-14 Aggregator Menu Management + Sync Operations (CR-140, CR-141)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-140** | Aggregator Menu Management — Food Add/Edit/StockToggle Fix | **P0** | **HIGH** | **INTAKE** | 1 ✅ | 7 gaps confirmed via live probe (2 restaurants). GAP-1: `add-food` creates ghost food — use `add-food-aggregator` (JSON). GAP-2: swiggy/zomato missing from transforms. GAP-3: client_id/brand selector absent. GAP-4: `fromAPI.food()` missing 5 fields (swiggy, zomato, client_id, food_stock, turn_on_at). GAP-5: stock-toggle absent (4 modes). GAP-6: BulkEditor missing fields. GAP-7: edit form can't change platform settings. 8 files (5 edit + 1 new). Code reality: NONE. DISTINCT from CR-135. Intake: `change_requests/CR-140_AGGREGATOR_MENU_FOOD_ADD_EDIT_STOCKTOGGLE_INTAKE.md` |
| **CR-141** | Aggregator Sync Operations — Category Timings + Sync/Clear Controls | **P1** | **MEDIUM** | **INTAKE** | 1 ✅ | 4 gaps: GAP-8 sync-catalog, GAP-9 clear-catalog (full_master_reset danger), GAP-10 clear-modifiers, GAP-11 category-timings (GET/POST/push). 2 new tabs in AggregatorSetupView. Unimplemented scope from CR-135 intake. Code reality: NONE. 5 files (3 edit + 2 new). OQ-1/2/3 open. Intake: `change_requests/CR-141_AGGREGATOR_SYNC_OPERATIONS_CATEGORY_TIMINGS_INTAKE.md` |
...
### 2026-08-18 Intake — CR-168, CR-169 (Printer Setup: Missing CRs)

| CR ID | Title | Priority | Risk | Status | Sprint | Intake Doc |
|---|---|---|---|---|---|---|
| **CR-168** | **Printer Agent: Test Print Job + Live Printer Status.** `PrintersTab.jsx:273-276` has two disabled stubs — "Status — Coming soon" (grey dot) + "Test Print — Coming soon" (disabled button). This CR wires them up: Part A = live connectivity status indicator per printer (green/red/grey dot). Part B = working Test Print button that sends a real test print job through the printer agent. No financial logic. API endpoint unknown — OQ-1 must be answered at Gate 2. Code reality: PARTIAL (stubs exist). Related: CR-133 (built stubs), CR-152 (absorbs its scope). Batch-09. | **P2** | **MEDIUM** | **INTAKE** | pos_5_x | `change_requests/CR-168_PRINTER_TEST_PRINT_AND_STATUS_INTAKE.md` |
| **CR-169** | **Live Bill/KOT Print Preview in Printer Config.** End-user feature to preview what the actual bill/KOT will look like using current saved style settings. `PrinterConfigPreviewPage.jsx` (372 lines, route `/printer-config-preview`) already exists but is a hardcoded dev design mock (CR-133 Gap Batch tool) with no nav link from UI — NOT an end-user preview. This CR builds a live panel wired to real `GET /printer-agent-config` data: sample items, real restaurant info, real font sizes, 58mm/80mm toggle. No API writes, read-only display. Code reality: PARTIAL. Related: CR-133, CR-167. Batch-09. | **P2** | **LOW** | **INTAKE** | pos_5_x | `change_requests/CR-169_LIVE_BILL_KOT_PRINT_PREVIEW_INTAKE.md` |

---

### 2026-08-20 CR-170 — Conditional Grand Total Round-Off

| Field | Value |
|---|---|
| **ID** | CR-170 |
| **Title** | Conditional Grand Total Round-Off — < 0.10 paise floor (negative round_up), ≥ 0.10 ceil |
| **Description** | Changes always-ceil rule (BUG-051/BUG-076) to threshold-based: if decimal < 0.10 → Math.floor (round DOWN, send negative round_up), if decimal ≥ 0.10 → Math.ceil (round UP). Toggle-gated (`totalRound = true` only). Backend confirmed accepts negative round_up. All 4 owner decisions locked (D1-D4). Related: BUG-051, BUG-076, BUG-119, CR-029-QSR. Distinct from all. |
| **Priority** | P2 |
| **Risk** | HIGH (R5 hotspots: orderTransform + CollectPaymentPanel + CartPanel; R6 financial: order_amount + round_up field) |
| **Status** | **IMPLEMENTED (2026-08-20) — 17/17 tests PASS. Awaiting QA (Gate 5b).** |
| **Sprint** | pos_5_0 |
| **Intake doc** | `change_requests/CR-170_CONDITIONAL_ROUND_OFF_THRESHOLD_INTAKE.md` |
| **Files** | `orderTransform.js` (L869, L872, L881, L1630), `CollectPaymentPanel.jsx` (L679), `CartPanel.jsx` (~L343), `round001.alwaysCeil.test.js` |

---

### 2026-08-21 CR-166 — Franchise / CS Multi-Restaurant Login (Common Login + Restaurant Picker)

| Field | Value |
|---|---|
| **ID** | CR-166 |
| **CR-166** | **CS/Franchise Multi-Restaurant Login.** CS agents and franchise operators get a single login that lists all assigned restaurants (card grid picker). Flow: POST `/common-login` → COMMON_TOKEN → GET `/assigned-restaurants` → picker page → POST `/login-as-restaurant` → AUTH_TOKEN → LoadingPage boot. Switch Restaurant sidebar button (admin sessions only). Logout clears both tokens. Code reality: NONE — entirely new flow, 2 new files + 7 modified. |
| **Priority** | P1 |
| **Risk** | CRITICAL (auth flow — two-token management, multi-tenant session, localStorage keys) |
| **Status** | **CLOSED — OWNER VERIFIED (retroactive 2026-08-22). Code investigation confirmed: `RestaurantPickerPage.jsx` (217 lines, CR-166 marker), route `/restaurant-picker` in `App.js:117`, `LoginPage.jsx` routes `loginType=admin` → picker, `AuthContext` clears common token on logout. Gate 5b QA PASS (2026-08-21) 15/15.** |
| **Sprint** | pos_5_0 |
| **Intake doc** | `change_requests/CR-166_COMMON_LOGIN_RESTAURANT_PICKER_INTAKE.md` |
| **Impact doc** | `impact/CR-166_IMPACT_ANALYSIS.md` |
| **Plan doc** | `plans/CR-166_IMPLEMENTATION_PLAN.md` |
| **Files** | `constants.js`, `authTransform.js`, `authService.js`, `commonAuthService.js` (NEW), `LoginPage.jsx`, `RestaurantPickerPage.jsx` (NEW), `App.js`, `AuthContext.jsx`, `Sidebar.jsx` |


---

## 2026-08-26 Batch Intake — CR-348 → CR-350 (from INVESTIGATION_REPORT_BATCH_2026_08_26.md)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-348** | Custom Item Modal — Add GST / Tax field | P2 | HIGH | **INTAKE** | 1 ✅ | `AddCustomItemModal.jsx` has no tax state/field. Custom items always submit 0% tax. OD-1: auto-populate from restaurant default? OD-2: taxCalc inclusive/exclusive? OD-3: backend field names? Intake: `change_requests/CR-348_CUSTOM_ITEM_GST_FIELD_MISSING_INTAKE.md` |
| **CR-349** | OrderReportBetaPage — Wire Change/Unpaid/Reprint on Settled tab | P2 | MEDIUM | **INTAKE — ODs LOCKED** | 1 ✅ | Option B: wire directly into Beta page's own custom table (no OrderTable reuse, layout unchanged). All 3 actions applicable. 1 file. Ready for Gate 2. Intake: `change_requests/CR-349_ORDER_REPORT_BETA_SETTLED_TAB_ACTION_BUTTONS_INTAKE.md` |
| **CR-350** | Room Check-In — ID upload mandatory/optional toggle | P2 | HIGH | **INTAKE — OD LOCKED** | 1 ✅ | Phase 1: FE-only via StatusConfigPage + localStorage (`mygenie_room_id_upload_required`). Identical to CR-051 pattern. No backend needed. Toggle in Dashboard Settings → new Room section. `RoomCheckInModal.jsx` reads key. Phase 2 (future): migrate to RestaurantSettingsPage Step 8 + backend key. Files: `StatusConfigPage.jsx` + `RoomCheckInModal.jsx`. Intake: `change_requests/CR-350_ROOM_CHECKIN_ID_UPLOAD_MANDATORY_TOGGLE_INTAKE.md` |


## 2026-08-27 BATCH-09 Intake — CR-351 (Local Printer Setup)
...
### 2026-09-02 CR-359 — Station Mapping Tab: Rewire to Correct `/station-printer-map` Endpoint + Fix Data Model
...
| **Gate** | 1 ✅ |
| **Sprint** | pos_5_x |
| **Related** | CR-353 (original build), POS2-003 (runtime print-agent), BUG-319 (printer backend-blocked) |
| **Description** | CR-353 built `StationMappingTab.jsx` reusing CR-160's service layer (`/printer-mapping` endpoint, old `printers[]/assignedEmployeeIds` model). Backend has since delivered the correct dedicated endpoint (`/station-printer-map`) with a completely different contract. 5 confirmed gaps: wrong endpoint (GAP-1), endpoint not in constants (GAP-2), `handleLoad()` uses old model / no per-employee GET (GAP-3), default-user dropdown shows all employees not `default_user_v2=Yes` only (GAP-4), save payload shape wrong (GAP-5). 1 suspected gap: runtime `print_agent` vs UI `station_printer_mappings` source disconnect (GAP-6 — needs backend probe). Code reality: PARTIAL. Blast radius: SMALL (3 files). Full gate flow required (CRITICAL risk). OD-1 (GAP-6 backend probe) + OD-2 (profile re-fetch strategy) must be answered before Gate 3. |
| **Intake doc** | `change_requests/CR-359_STATION_PRINTER_MAP_REWIRE_INTAKE.md` |
| **Investigation doc** | `handover/INVESTIGATION_STATION_PRINTER_MAP_2026_09_02.md` |
| **Backend brief** | `backend_briefs/BACKEND_BRIEF_STATION_PRINTER_MAP_GAP6_2026_09_02.md` |
| **Owner decisions** | OD-1: GAP-6 backend probe (does profile `print_agent` reflect `station_printer_mappings`?). OD-2: profile re-fetch after save if sources not linked. |

---

### 2026-09-02 CR-359 Gate 2 Update

| Field | Value |
|---|---|
| **Status** | **GATE 2 COMPLETE — GATE 3 BLOCKED (OD-3 + OD-4)** |
| **Impact Analysis** | `impact/CR-359_IMPACT_ANALYSIS.md` |
| **Code Reality** | PARTIAL (CR-353 shell exists, wrong endpoint/model) |
| **Conflict** | CLEAN (constants.js touched by CR-147 on different lines — parallel-safe) |
| **Scope locked** | 3 files: `constants.js`, `printerMappingService.js`, `StationMappingTab.jsx` |
| **OD-3** | UX: auto-load on employee change (Option A) vs keep Load button (Option B)? |
| **OD-4** | Include profile re-fetch + `setRestaurant` after save in this CR? (Recommended: YES — Option A) |

---

### 2026-09-02 CR-359 Implementation Complete

| Field | Value |
|---|---|
| **Status** | **IMPLEMENTED — AWAITING QA (Gate 5b)** |
| **Sprint** | pos_5_x |
| **Files changed** | `api/constants.js` (+1 line) · `api/services/printerMappingService.js` (+12 lines additive) · `components/panels/settings/printerConfig/StationMappingTab.jsx` (full logic rewrite) |
...
| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-368** | Test Suite Triage — Achieve Zero-Failure Baseline | P1 | MEDIUM | **GATE 2 — IMPACT ANALYSIS WRITTEN (2026-09-08). ODs locked: 02(c) 03(A) 04(c) 05i(register BUG). OPEN: 05(ii), 06. Gate 3 on hold per owner.** `impact/CR-368_IMPACT_ANALYSIS.md`. Code truth: 54 fails / 11 suites / 2 fake scripts (intake said 56/15/3 — OG-AUDIT-002). Phase 2 (axios mapper) already done by BUG-382. | 2 | 56 failing tests + 3 fake node scripts. Blocks CR-369 (D3-b). Source: F-QA-01 + F-QA-02. `change_requests/CR-368_TEST_SUITE_TRIAGE_CLEAN_BASELINE_INTAKE.md` |
...
| **CR-370** | Stale Doc Corrections — 5 Records | P2 | LOW | **IMPLEMENTED — 5 doc corrections applied (2026-09-08). Gate 5a. V1–V10 PASS. F-DRIFT-04/05/06 resolved.** | 5a | C1 reword v1-defunct, C2 PRD.md, C3 CONTROL_DASHBOARD, C4 ENV_REGISTRY.md PENDING REMOVAL, C5 test_credentials.md login path + RID. `change_requests/CR-370_STALE_DOC_CORRECTIONS_5_RECORDS_INTAKE.md` |
...
| **CR-372** | ~~Security Remediation (parent)~~ | — | — | **SPLIT → CR-372-A + CR-372-B (2026-09-08)** | — | Split on owner instruction: no-src/ fixes → CR-372-A; App.js routing → CR-372-B. Original intake retained for context. |
| **CR-372-A** | Security: File Moves + .env Cleanup (Zero src/ changes) | P1 | LOW | **IMPLEMENTED (Gate 5a, 2026-09-08) — 15 `__dev` files + 74 HTML + 3 non-HTML moved; −2 `.env` lines; V1–V17 PASS.** | 5a | F-SEC-01 (`public/__dev/` 15 files → `/app/memory/dev-dashboard/`), F-SEC-02 (**74** HTML → `memory/design_briefs/`, subfolders kept; `public/pms/` 13 + 4 carve-outs + PDF stay — OD-CR372A-02), F-SEC-02 ext (2 `.xlsx` + contract `.pdf` — OD-CR372A-03), F-SEC-07 (−2 `.env` lines; ENV_REGISTRY → REMOVED). 92 files total. OG-AUDIT-001 filed (v0.7 `__dev` refs → CR-369). Zero `frontend/src/`. `impact/CR-372-A_IMPACT_ANALYSIS.md` · `plans/CR-372-A_IMPLEMENTATION_PLAN.md` · `change_requests/CR-372-A_SECURITY_FILE_MOVES_ENV_CLEANUP_INTAKE.md` |
| **CR-372-B** | Security: Add ProtectedRoute to 23 Unguarded Routes (App.js only) | P1 | MEDIUM | **INTAKE — Ready for Gate 2** | 1 | F-SEC-03. Wraps 23 bare routes in `<ProtectedRoute>` in `src/App.js`. Keeps `/` + `/local-printer-setup` public. Only src/ file = App.js. Needs Gate 2 → Gate 4 GO → Impl → QA. `change_requests/CR-372-B_SECURITY_ROUTE_GUARDING_APP_JS_INTAKE.md` |
| **CR-373** | Aggregator Menu: "Use Item Image for Swiggy" toggle in ProductForm — pass `form.imageFile` for both image fields on save when toggled | P2 | MEDIUM | **INTAKE — NOT STARTED. Depends on BUG-390.** | 1 | ProductForm.jsx (Swiggy image section, ~20 lines). ODs locked: default to "Use same" for new items; default to "Upload different" when existing swiggyImagePreview present. `change_requests/CR-373_AGGREGATOR_USE_ITEM_IMAGE_FOR_SWIGGY_INTAKE.md` |
| **CR-374** | BulkEditor Filter Panel — Status / Type / Category always-visible strip. Adds status (All/Active/Inactive), food type (All/Veg/Non-Veg/Egg/Jain), searchable category dropdown. Fixes "Select All Inactive for bulk delete" flow. | P2 | MEDIUM | **INTAKE — DESIGN LOCKED (2026-09-11). Ready for Gate 3.** | 1 | `BulkEditor.jsx` only (~35–55 lines). Option A (always-visible) + searchable category dropdown locked by owner. `change_requests/CR-374_BULKEDITOR_FILTER_PANEL_STATUS_TYPE_CATEGORY_INTAKE.md` |
| **CR-375** | Aggregator Menu: Bulk Delete Capability in BulkEditor (currently hard-blocked by FE guard + backend `/delete-bulk` endpoint not supporting `food_for: Aggregator`) | P2 | MEDIUM | **INTAKE — BLOCKED. Backend brief filed.** | 1 | `BulkEditor.jsx` (FE guard removal, ~5 lines) + backend dependency. `change_requests/CR-375_AGGREGATOR_BULK_DELETE_CAPABILITY_INTAKE.md`. Backend brief: `backend_briefs/BACKEND_BRIEF_CR375_2026_09_11.md` |
| **CR-376** | POS Order Entry: Menu Switch — Local Settings per-station active menu switch (Design A). Excludes Aggregator. | P2 | MEDIUM | **GATE_5A_IMPLEMENTED (2026-09-25) — 5 files, 2 hotspots. Awaiting QA Gate 5b.** Zero code. | 3 | **5 files, 2 hotspots** (E6/E7 dropped OD-09=a): `productTransform.js`, `activeMenuPrefs.js` (NEW), `MenuContext.jsx`, `OrderEntry.jsx`, `StatusConfigPage.jsx`. Sprint `sep_bug_closure`. All ODs 01-10 locked. E5h card-row (OD-08=b). E4e firm (OD-07=a). Follow-ups: CR-376-FU-A/B/C. Hyatt probe done. `plans/CR-376_IMPLEMENTATION_PLAN.md` · `impact/CR-376_IMPACT_ANALYSIS.md` §12 |
| **CR-376-FU-A** | CustomerModal: hide/disable inert off-menu suggestion rows after CR-376 ships. | P2 | LOW | **GATE 1 — INTAKE COMPLETE (2026-09-25). BLOCKED: CR-376 Gate 5b required first.** | 1 | `CustomerModal.jsx` (1 file). Fast Lane potentially eligible (1 file ≤10 lines). Registered per owner P5. |
| **CR-376-FU-B** | CategoryPanel: hide categories with 0 items for active menu after CR-376 ships. | P2 | MEDIUM | **GATE 1 — INTAKE COMPLETE (2026-09-25). BLOCKED: CR-376 Gate 5b required first.** | 1 | `CategoryPanel.jsx` (1 file). Registered per owner OD-376-10=(a) + P5. |
| **CR-376-FU-C** | Per-restaurant server-side active menu default — backend API change required. | P3 | HIGH | **GATE 1 — INTAKE COMPLETE (2026-09-25). BLOCKED: CR-376 Gate 6 + backend team.** | 1 | `activeMenuPrefs.js` + `LoadingPage.jsx` + backend. Registered per owner P5. |

---

## 2026-09-10 — CR-377 (Sales Report Complete Redesign)

| Field | Value |
|---|---|
| **ID** | CR-377 |
| **Type** | CR |
| **Title** | Sales Report: Complete Redesign — surface all 82 API fields (Expense, Purchase, Galla/Cash Drawer) |
| **Priority** | P2 |
| **Risk** | MEDIUM |
| **Area** | Reports > Daily Summary (`OrderSummaryPage`) |
| **Status** | **INTAKE — Gate 1. 5 ODs open. Awaiting owner answers before Gate 2.** |
| **Sprint** | pos_7_0 |
| **Registered** | 2026-09-10 |
| **Source** | AGENT-DISCOVERED via live API probe (fresh token, owner@cafe103.com `***`) |
| **Confidence** | CONFIRMED — live response verified, 82 leaf fields catalogued |

**Description:** The daily sales API (`POST /api/v2/vendoremployee/daily-sales-revenue-report`) returns 82 leaf fields. `OrderSummaryPage` renders only 32 (39%). Three entire sections missing: **Expense** (10 fields), **Purchase** (10 fields), **Cash Drawer/Galla** (8 fields including `total_profit_loss`). Also: 3 existing sections partially incomplete (Payment Methods +3, Room +7, TAB +3).

**Original spec SUPERSEDED:** ~~Date range UI~~ — backend ignores `to` input (probes P4/P5 confirmed). Single-day only.

**Code Reality:** NONE (new sections) | PARTIAL (enhanced sections)

**Duplicate check:** DISTINCT. Related: CR-363, CR-366 (same endpoint, different report), CR-026 (closed)

**Blast radius:** MEDIUM — 2 files (`reportService.js` +50 lines, `OrderSummaryPage.jsx` +200 lines)

**Absorbs:** BUG-393 (implementing agent adds `to: dateStr` in transform rewrite)

**Owner Decisions Open:**
- OD-CR377-01: Page title "Sales Report" vs "Daily Summary"
- OD-CR377-02: Show Partial / Zomato Gold / Room Checkin in Payment section?
- OD-CR377-03: TAB Credit Cash/Card/UPI sub-breakdown visible?
- OD-CR377-04: Room Checkin Revenue — sub-section or standalone card?
- OD-CR377-05: Expense/Purchase — non-zero rows only or always all rows?

**Artifacts:** `change_requests/CR-377_SALES_REPORT_COMPLETE_REDESIGN_INTAKE.md` · `investigations/INV-SALES-REPORT-REINVESTIGATION_2026_09_10.md` · `evidence/INV-SALES-REPORT-REINV-001/live_response_2026_09_10.json`
...
|---|---|
| **ID** | CR-381 |
| **Title** | PMS — Laundry Management (linen queue, stock levels, dirty→clean processing, settings) |
| **Priority** | P2 |
| **Risk** | MEDIUM |
| **Area** | PMS → Housekeeping → Laundry sub-module |
| **Status** | **INTAKE — Gate 1. OD-381-01 open (standalone page vs sub-tab inside CR-365 HK). Awaiting owner answer + CR-365 Gate 2 completion before Gate 2 can start.** |
| **Sprint** | pos_pms_1 |
| **Registered** | 2026-09-13 |
| **Source** | AGENT-DISCOVERED — BE reply 2026-09-13 shipped laundry endpoints outside CR-365 scope |
| **Confidence** | CONFIRMED — all 4 endpoints live |

**Description:** Backend shipped a full laundry sub-module (`GET/PUT laundry/settings`, `GET laundry/queue`, `GET laundry/stock`, `POST laundry/process`). No FE implementation exists. HK checklist `laundry_issue`/`laundry_collect` actions push items into this queue — CR-381 closes the loop by letting staff see and process the laundry queue, check stock levels, and configure laundry settings.

**Code Reality:** NONE

**Duplicate check:** DISTINCT. Related: CR-365 (feeds the queue via HK checklist actions).

**Blast radius:** MEDIUM — 3 NEW files (`LaundryPage.jsx`, `laundryService.js`, `laundryTransform.js`) + 3 modified (`constants.js`, `Sidebar.jsx`, `App.js`)

**Backend blocked:** NO — all endpoints live.

**Execution dependency:** Plan after CR-365 Gate 2. BUG-397 → CR-365 → CR-381 recommended order.

**Owner Decisions Open:**
- OD-381-01: Standalone `/pms/laundry` page OR sub-tab inside CR-365 HK page?
- OD-381-02: Process action — all staff or manager/owner only?
