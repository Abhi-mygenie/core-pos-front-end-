...
| CR ID | Title | Priority | Risk | Status | Gate | Code reality | Blast | Notes |
|---|---|---|---|---|---|---|---|---|
| **CR-385** | PMS Front Desk — Unified Tabbed Workstation UX Revamp | **P1** | **HIGH** | **GATE 2 IN PROGRESS (2026-09-16). OD rounds 1+2 answered: KPI tiles ARE the tabs (one strip, 4 tiles); Check-In no navigation; Arrivals default; OD-02 deferred → FU-385-A. All 7 ODs answered: OD-05 = Hybrid density toggle (Comfortable default / Compact). OD-08 (Occupancy tile ↔ Room Status) → Gate 2.4. New sub-gate 2.4 UX flow before 2.5 mockup. Channel Sync card = removal candidate; Departures widget = rethink. Gate advances only on explicit owner close.** | 2 | NONE | LARGE | 4 tabs: Arrivals (default) / In-House / Departures / Room Status. Single URL `/pms/front-desk`. Check-In must NOT navigate away (OD-03) → CheckInPage.jsx form extraction now IN scope. Sidebar.jsx OUT (FU-385-A). GuestFolioPage.jsx out of scope. Hotspot: `App.js`. Sprint: pos_pms_2. Intake: `change_requests/CR-385_PMS_FRONTDESK_UNIFIED_WORKSTATION_UX_REVAMP_INTAKE.md`. Related: CR-358-P1/P3/P4, CR-362, CR-365. |
