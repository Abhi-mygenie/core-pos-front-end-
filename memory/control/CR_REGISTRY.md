...
|| **CR-017** | **WhatsApp Payment Link — Send Razorpay Payment Link to Customer via WhatsApp.** Single API call from POS (`POST /api/v1/razor-pay/payment-link`) — backend orchestrates Razorpay link creation + WhatsApp/SMS delivery. API contract confirmed live on preprod. Payload: `order_id`, `payment_amount`, `customer_phone`, `customer_name`, `restaurant_name`. Response: `payment_link` URL + `source` (razorpay\|db). | **Gate 2 COMPLETE (Impact Analysis). All 9 owner Qs answered. UI mockup approved. Gate 3 (Implementation Plan) next.** | — (no code yet) |
