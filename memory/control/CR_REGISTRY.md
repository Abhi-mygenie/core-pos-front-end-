**Last Updated:** 2026-09-24 — **CR-387 registered (GST/packaging validation alignment with `gst_status`, P1 HIGH, backend brief filed).** Registry: 715 items.
...
|---|---|---|---|---|---|---|---|---|
| **CR-387** | Align menu GST/packaging validation with outlet `gst_status` — Aggregator unconditional 5 % (BUG-391) vs packaged exemption (CR-036-FU-03) clash; `gst_status=false` still forces Aggregator GST; no packaging-GST key; settings wizard ignores new flat `basic.gst_status` | P1 | HIGH | **INTAKE — GATE 1 (2026-09-24)** — BQ-387-01..05 sent (`backend_briefs/BACKEND_BRIEF_CR-387_2026-09-24.md`), OD-387-01..04 open | 4 | PARTIAL | MEDIUM | Depends on BUG-454 for end-to-end correctness. Intake `change_requests/CR-387_ALIGN_MENU_GST_PACKAGING_VALIDATION_WITH_GST_STATUS_INTAKE.md` · INV `INV_GST_MENU_VALIDATION_INVESTIGATION_REPORT_2026_09_24.md`. |
...
| **CR-376** | POS Order Entry: Menu Switch — allow staff to toggle between Normal / Party / Premium menus when taking an order. Tab strip above item grid. Excludes Aggregator. | P2 | MEDIUM | **INTAKE — ODs 01–04 open** | 1 | `productTransform.js` (1 line), `MenuContext.jsx` (~15), `OrderEntry.jsx` (~20), `LoadingPage.jsx` (~5). ZERO backend changes. Phase 3 implementation already anticipated in codebase comment. `change_requests/CR-376_MENU_SWITCH_ORDER_ENTRY_INTAKE.md` |
