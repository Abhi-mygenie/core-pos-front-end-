...
| **CR-358-P5** | PMS Phase 5 — Rates & Restrictions (S8-C) + Mark No-Show (S8-D) + regression | P1 | HIGH | **INTAKE** (entry gate CLEARED) | 1 | PARTIAL | MEDIUM | All 5 endpoints live-verified 2026-09-04 (405/422/200). `rates[]` schema doc still needed (B-P5-01). `change_requests/CR-358-P5_PMS_RATES_RESTRICTIONS_NO_SHOW_INTAKE.md` |
...
| **CR-372-A** | Security: File Moves + .env Cleanup (Zero src/ changes) | P1 | LOW | **INTAKE — Ready for Gate 2** | 1 | F-SEC-01 (`public/__dev/` → `/app/memory/dev-dashboard/`), F-SEC-02 (~91 HTML briefs → `memory/design_briefs/`, 5 PMS files excluded), F-SEC-07 (remove CRM_API_KEYS + CORS_ORIGINS from .env). Zero `frontend/src/` changes. `change_requests/CR-372-A_SECURITY_FILE_MOVES_ENV_CLEANUP_INTAKE.md` |
