...
**Status:** POPULATED
**Last Updated:** 2026-05-31 (Owner-attested closures: BUG-097 25-row smoke PASSED → main VERIFIED; AUDIT-CLOSURE-DRIFT-001 G-2 smoke PASSED → CLOSED)
...
| **CR-013** | **Food Court Report — Station-wise Order Breakdown.** New custom report screen (route: `/reports-module/food-court`). Layout clones Order Ledger (S6): S7 header, flat table, KPI strip, TOTAL row, search, column chooser, side-sheet drill, Excel/PDF export. **One new top-level filter: Station dropdown** (one station at a time). Each order row shows only the selected station's items (name, qty, price), subtotal, GST, total. Data: `order-logs-report → order_details_table[].station_name`. Tabs: All / Settled. Sidebar entry under Insights. | **REGISTERED 2026-06-06** | — |
| **CR-013-AUDIT** | **Food Court Audit Tab — Station × Time Bucket Cross-Tabulation.** New "Audit" tab within Food Court report (CR-013). Pivot grid: rows = time buckets (auto: hourly for Today, daily for 7D/30D/MTD, monthly for 1Y/FY), columns = one per station (dynamic) + TOTAL column. Metric selector dropdown (5 views: Item Total, Discount, Sub Total, Tax, Total). Download menu extended with "Download Audit Report" (Excel: 5 sheets). Same shared header/date-range. | **REGISTERED 2026-06-07 — Gate ① mockup** | **Parent:** CR-013. **Files:** `FoodCourtMockup.jsx` |
| **CR-014** | **Menu Management API Migration — Switch from Product API to Menu Management API.** Current menu management (edit items, drag-and-drop reorder, category management, etc.) is wired to the Product API, which should only be used for order-taking/product display. A separate Menu Management API exists with a potentially different data structure. **Scope:** Keep existing UI intact, create a new service file, wire entirely new API endpoints with new data model mapping. Product API continues to be used ONLY for order-taking flows. | **REGISTERED 2026-06-07** | — |
| **CR-015** | **Settlement Module — Day-Closing / Cash Settlement.** Full settlement module: opening balance entry, closing balance calculation, per-waiter cash settlement (partial/full), self-settlement, pilferage tracking. 4 backend APIs already available on preprod (`get-settlement-report`, `get-waiter-list`, `opening-balance`, `settlement`, `self-settlement`). Key mapping documented. Related to BUG-105. | **REGISTERED 2026-06-07** | — |
