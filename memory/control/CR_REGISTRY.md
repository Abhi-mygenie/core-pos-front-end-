...
| CR ID | Title | Priority | Risk | Status | Gate | Code reality | Blast | Notes |
|---|---|---|---|---|---|---|---|---|
| **CR-385** | PMS Front Desk — Unified Tabbed Workstation UX Revamp | **P1** | **HIGH** | **GATE 1 INTAKE — OPEN (extended discovery, 2026-09-16). Owner has NOT closed intake. 5 decision rounds recorded (KPI tiles = tabs; Arrivals=pending only; Departures=due-out only; In-House gets Request HK + Mark Clean; Rooms hybrid density; Channel Sync card → header dot; Departures widget removed; 40-room mockup). Open Qs presented one-by-one next session. After close: Gate 2 IA → 2.4 UX → 2.5 HTML.** | 1 | NONE | LARGE | Handover: `handover/SESSION_HANDOVER_2026_09_16_CR385_FRONTDESK_INTAKE.md`. Data inventory: `impact/CR-385_DATA_INVENTORY.md`. Backend Qs: BQ-385-01 (PMS socket events — FE has none), BQ-385-02 (Phase 2 aggregation). UI rule: never show "Channel Manager". Sprint: pos_pms_2. Intake: `change_requests/CR-385_PMS_FRONTDESK_UNIFIED_WORKSTATION_UX_REVAMP_INTAKE.md`. |
