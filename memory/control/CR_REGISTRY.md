...
| **CR-380** | PMS Check-In — Guest ID Documents | P1 | HIGH | **GATE 3 COMPLETE (2026-09-14). IA at `impact/CR-380_IMPACT_ANALYSIS.md`. Plan at `plans/CR-380_IMPLEMENTATION_PLAN.md`. 11 edit sites / 3 files (GuestDocsSection NEW, CheckInPage MAJOR, pmsService MINOR). 15 verification checks V-01..V-15. Awaiting Gate 4 GO.** | 3 | NONE | MEDIUM | `pmsCheckIn` JSON→FormData (OD-3-B). Docs-on-file viewer + upload to CRM. Reuse CR-350 mandatory-doc toggle (OD-4-A). Extra adult/child IDs (OD-5-A). Files: `CheckInPage.jsx` + `pmsService.js` + new `GuestDocsSection.jsx`. Related: CR-379 (dependency), CR-129, BUG-351, CR-350. `change_requests/CR-380_PMS_CHECKIN_GUEST_ID_DOCUMENTS_INTAKE.md` |
