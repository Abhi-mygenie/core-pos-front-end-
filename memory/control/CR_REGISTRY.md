...
| **CR-372** | Security Remediation — Remove Internal Files from Public Surface | P1 | HIGH | **INTAKE — ALL ODs LOCKED ✅ — Ready for Gate 2** | 1 | OD-01: protect 23 routes (keep `/` + `/local-printer-setup` public; PMS HTML files carved out until PMS closes). OD-02: CRM key not in src/ — safe to remove. OD-03: Option A (move `__dev/` → `/app/memory/dev-dashboard/`). F-SEC-02 scope ~91 HTML briefs (5 PMS files excluded). RELATED: DEV-DASHBOARD-001. `change_requests/CR-372_SECURITY_REMEDIATION_PUBLIC_SURFACE_INTAKE.md` |
