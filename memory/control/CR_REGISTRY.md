...
| CR ID | Title | Priority | Risk | Status | Gate | Code reality | Blast | Notes |
|---|---|---|---|---|---|---|---|---|
| **CR-385** | PMS Front Desk — Unified Tabbed Workstation UX Revamp | **P1** | **HIGH** | **GATE 1 INTAKE CLOSED by owner 2026-09-16 (6 rounds). Next: Gate 2 IA (PLANNING). Directions locked: KPI tiles = tabs (Arrivals default · Departures · In-House · Rooms); no page navigation; Arrivals = pending only; Departures = due-out only w/ true balance; In-House rows Request HK + Mark Clean; Rooms hybrid density; Channel Sync card → header dot; Departures widget removed; alert bar wanted; 40-room mockup; Q10 refresh-on-focus provisional. All remaining UX options → switchable variants MV-01..MV-09 in Gate 2.5 mockup.** | 1→2 | NONE | LARGE | Handover: `handover/SESSION_HANDOVER_2026_09_16_CR385_FRONTDESK_INTAKE.md`. Data inventory: `impact/CR-385_DATA_INVENTORY.md`. Backend Qs: BQ-385-01 (PMS socket events — FE has none), BQ-385-02 (Phase 2 aggregation). UI rule: never show "Channel Manager". Process: Gate 2 IA → 2.4 UX flow → 2.5 HTML variants → 3. Sprint: pos_pms_2. Intake: `change_requests/CR-385_PMS_FRONTDESK_UNIFIED_WORKSTATION_UX_REVAMP_INTAKE.md`. |
