...
| **CR-363** | PMS — Night Audit Report | P1 | HIGH | **INTAKE** (🟡 PARTIALLY UNBLOCKED per INV 2026-09-06: no no-show field, balance needs client join) | 1 | NONE | MEDIUM | Composes dashboard-kpis + daily-sales-revenue-report + reservation buckets + balances. OD-363-01/02 (day boundary, revenue basis) before Gate 2. `change_requests/CR-363_PMS_NIGHT_AUDIT_REPORT_INTAKE.md` |
| **CR-364** | PMS — Guest Folio Detail Page | P1 | HIGH | **INTAKE** (🟡 PARTIALLY UNBLOCKED per INV 2026-09-06: read-only OK; `pos/room-payment` 403 → OG-PMS-014) | 1 | PARTIAL | MEDIUM | Reuses PmsCheckoutDrawer fetch + orderTransform.roomInfo; display-only money (D3). Payment history endpoint optional. `change_requests/CR-364_PMS_GUEST_FOLIO_DETAIL_PAGE_INTAKE.md` |
...
| **CR-366** | PMS — Revenue Dashboard / Analytics | P2 | MEDIUM | **INTAKE** (🟢 MOSTLY UNBLOCKED per INV 2026-09-06; daily-sales N-day loop) | 1 | NONE | MEDIUM | dashboard-kpis ≤31d chunks + LR channel split; OD-366-01 revenue basis (R6). `change_requests/CR-366_PMS_REVENUE_DASHBOARD_ANALYTICS_INTAKE.md` |
