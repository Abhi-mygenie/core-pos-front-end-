...
| **CR-376** | POS Order Entry: Menu Switch — allow staff to toggle between Normal / Party / Premium menus when taking an order. Tab strip above item grid. Excludes Aggregator. | P2 | MEDIUM | **GATE 3 FINAL (2026-09-25) — R11 probe done, HTML mockup, OD-376-07/08/09/10 + P1–P5 OPEN — awaiting owner review → Gate 4 GO** | 3 | 7 files: `productTransform.js`, `activeMenuPrefs.js` (NEW), `MenuContext.jsx`, `OrderEntry.jsx`, `StatusConfigPage.jsx`, `LoadingPage.jsx`, `useRefreshAllData.js`. Sprint `sep_bug_closure` (from `pos_7_0`). Design A (Local Settings). Plan re-verified at HEAD 2026-09-25; PLAN GAP → OD-376-07 (CustomerModal `menuItems` L2844). Zero code. `plans/CR-376_IMPLEMENTATION_PLAN.md` · `impact/CR-376_IMPACT_ANALYSIS.md` §11 |
