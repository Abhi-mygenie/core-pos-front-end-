...
|---|---|---|---|---|---|---|---|---|
| **CR-379** | PMS Check-In — CRM Customer Link (Core) | P1 | HIGH | **GATE 3 GO (2026-09-12). Design frozen by owner. 8 design decisions locked (DD-1..DD-8). Backend contract live-probe confirmed. BUG-090 stale label corrected. 2 files, 0 backend. Documents fetch (getDocuments) in CR-379 scope. Awaiting Gate 3 Implementation Plan.** | 3 | NONE | MEDIUM | Files: `CheckInPage.jsx` + `pmsService.js`. Badge: 4-col (Stays/LastStay/LoyaltyPts/StoreCredit) + Doc-type cards. Auto-lookup on 10 digits. Extra adults inline. Children per-input. Corporate checkbox. Amber non-blocking error. `impact/CR-379_IMPACT_ANALYSIS.md` · `plans/CR-379_DESIGN_DECISIONS.md` · `public/cr379-design-mockup.html` |
