...
|| **CR-041** | **Navigation Consistency: Investigation catalogue complete.** 3 patterns, 4 inconsistencies, 3 owner decisions pending (D-1/D-2/D-3). | **INVESTIGATION COMPLETE 2026-06-12. NO CODE — owner decisions pending.** | — |
...
||| **CR-045** | **Insights Module: Suppress/Ignore Unused API Response Fields.** | **CLOSED — OWNER VERIFIED (2026-06-13). TEMPORARY FE-side strip.** | `orderPayloadStripper.js` (NEW), `insightsService.js`, `orderLedgerService.js`, `roomOrdersService.js`, `foodCourtService.js`, `prepServeService.js`, `CancellationsMockup.jsx` |
...
|||||| **CR-047** | **AGENT_PROMPT_ALPHA v0.6 — Role Hardening (Intake, QA, Bug Fix, Investigation).** Harden 4 weakest roles: INTAKE (+severity rubric, +duplicate detection, +evidence storage, +blast radius). QA (+4-tier finding severity, +environment check, +re-test protocol, +coverage check). BUG FIX (full rewrite: +reproduce-first, +mandatory RCA, +fix report, +5 escalation paths). INVESTIGATION (+10-step time-box, +hypothesis method, +structured template, +data persistence, +exit criteria). Doc-only CR — single file `AGENT_PROMPT_ALPHA.md`. ~350 lines added. | **IMPLEMENTED 2026-06-15. v0.6 applied. 8 edits, 1481 lines total. All 38 verification items pass (grep-verified).** | `AGENT_PROMPT_ALPHA.md` |
