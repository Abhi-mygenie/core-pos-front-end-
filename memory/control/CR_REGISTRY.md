...
| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-114** | Smart Purchase — Default All Items Unselected (Opt-In). All planner items shown by default, overwhelming with 100+ items. Add opt-in checkbox, unchecked by default. | **P2** | MEDIUM | **IMPLEMENTED** | | 0-1 ✅ | 2 files (`SmartPurchasePanel.jsx`, `AutoShoppingList.jsx`), ~30-40 lines. Related: CR-103, CR-105. Intake: `change_requests/CR-114_SMART_PURCHASE_DEFAULT_UNSELECTED_INTAKE.md` |
| **CR-115** | Smart Purchase — Search Filter + Sort by Category. No search/filter/category sort in 100+ item list. Add filter bar: search input + category dropdown. | **P1** | MEDIUM | **IMPLEMENTED** | | 0-1 ✅ | 1-2 files, ~60 lines. Related: CR-114. Intake: `change_requests/CR-115_SMART_PURCHASE_SEARCH_SORT_CATEGORY_INTAKE.md` |
| **CR-116** | B2B Customer GST & Name Capture — New Input Fields in Collect Bill. `custGST` and `custGSTName` hardcoded empty in print payload. No UI exists. Need input fields + wire to settle/print. | **P1** | MEDIUM | **INTAKE** | 0-1 ✅ | 2 files: `CollectPaymentPanel.jsx` + `orderTransform.js`, ~50 lines. Owner decisions: where to show fields, always or B2B-toggle. Intake: `change_requests/CR-116_B2B_CUSTOMER_GST_CAPTURE_INTAKE.md` |

### 2026-07-30 Aggregator + Report Beta Intake Batch (CR-117 → CR-119)

| CR ID | Title | Priority | Risk | Status | Gate | Notes |
|---|---|---|---|---|---|---|
| **CR-117** | Order Report Beta — Combined backend-aggregated daily order report. KPI strip, per-day collapsible sections, 11-column table, grand total footer, Excel export. Tabs: All + Aggregator (6 blocked pending backend). Filters: 6 active + 2 blocked. | **P1** | LOW | **IMPLEMENTED** | 5a ✅ | 5 files (1 new + 4 modified). ~460 lines. Backend brief filed for 9 missing fields. |
