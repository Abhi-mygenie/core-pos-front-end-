...
| **CR-014** | **Menu Management API Migration — Switch from Product API to Menu Management API.** Existing UI kept intact. New `menuManagementService.js` (17 functions) + `menuManagementTransform.js` wired to 20 endpoints. Food CRUD (#1-7,#11), Category CRUD (#12-15), Station list (#16), Addon CRUD (#17-20). `item_type` mapping (0=NonVeg,1=Veg,2=Egg,3=Jain). Fields removed per backend: stock_out, is_disable, tax_calc, is_inventory, packed_food. Station at category level. Order-taking untouched. | **IMPLEMENTED — AWAITING OWNER SMOKE (2026-06-08). Gate 5 QA: 100% pass. Phase 2 (bulk ops) deferred.** | `menuManagementService.js` (NEW), `menuManagementTransform.js` (NEW), `MenuManagementPanel.jsx`, `CategoryList.jsx`, `ProductList.jsx`, `ProductCard.jsx`, `ProductForm.jsx` |
