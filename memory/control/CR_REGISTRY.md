...
| Files WILL change | `aggregatorTransform.js`, `OrderCard.jsx`, `aggregatorService.js` (cond. OQ-3) |
| Open Questions | OQ-3: does backend print endpoint need `brand_name` from FE? |
| Notes | `brand_name` present in `get-order-list` top-level (sibling of rider_info). Code Reality: NONE. Distinct from CR-110. Intake: `change_requests/CR-125_AGGREGATOR_BRAND_NAME_DISPLAY_PRINT_INTAKE.md` |
