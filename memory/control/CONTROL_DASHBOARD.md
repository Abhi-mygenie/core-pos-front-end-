...
|| **CR-047 AGENT_PROMPT_ALPHA v0.6 Role Hardening** | **IMPLEMENTED (2026-06-15)** | **v0.6 applied to AGENT_PROMPT_ALPHA.md. 8 edits: STEP -1 session start (+handover reading, +stale batch, +multi-batch, +fallback), NEW STEP -1.5 conditional env check, PLANNING stage dispatch, INTAKE (severity+duplicate+evidence+blast), QA (4-tier severity+coverage+re-test+8-step), BUG FIX full rewrite (reproduce+RCA+5 escalation paths), INVESTIGATION (10-step+hypothesis+persistence+exit criteria). File: 1149→1481 lines. Zero app code changes.** | **Awaiting owner smoke (Gate 6)** |
