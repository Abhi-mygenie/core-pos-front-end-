...
|| **CR-047 AGENT_PROMPT_ALPHA v0.6 Role Hardening** | **INTAKE COMPLETE + IMPACT ANALYSIS DONE (2026-06-15)** | **Harden 4 roles: Intake (+severity rubric, +duplicate detection, +evidence storage), QA (+4-tier severity, +env check, +re-test protocol, +coverage), Bug Fix (full rewrite: +reproduce-first, +RCA, +5 escalation paths), Investigation (+10-step time-box, +hypothesis method, +exit criteria). Doc-only CR — sole target: `AGENT_PROMPT_ALPHA.md`. Impact analysis + flow recheck (5 scenarios) complete. Zero application code changes.** | **Awaiting Gate 3 (Implementation Plan) → Gate 4 (Owner GO)** |
