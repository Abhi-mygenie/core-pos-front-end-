...
| `REACT_APP_TRAINING_ENABLED` | `false` | **CR-053-ENV-01 (2026-06-18):** Master switch for Training Academy SDK. Build-time substituted into `frontend/public/index.html` via `%REACT_APP_TRAINING_ENABLED%`. Only the literal lowercase string `"true"` enables. Anything else (missing, `"True"`, `"1"`, `"yes"`, `"false"`) keeps SDK off — safe-fail default. Requires `sudo supervisorctl restart frontend` after change (CRA env-var convention). Currently `false` in this pod. |
...
## Planned Additions (awaiting CR Gate-4 GO — NOT yet in `.env`)

_(none currently planned)_

### localStorage keys planned (CR-053 family)

| Key | CR | Purpose |
|---|---|---|
| `training_now_try_it_dismissed` | CR-053-UX-01 | User has dismissed the "Now try it" completion checklist. Already in code (`overlay/CompletionScreen.jsx`), persists across missions for that user only. Auto-set on click of "Don't show again". |
