...
| OD-385-18 | Split at advance points (M1/M3/M4, D47-h) — payload field for split legs not yet in MASTER for `direct-reservation.advance` / check-in / extend `payment` | (a) probe backend; if unsupported, ship single-method advance now and park Split-at-advance to a follow-up · (b) block M1/M3/M4 until answered | **OPEN — owner asked for a walk-through (2026-09-20).** Fact: `split_payments[]` exists in MASTER as *our proposed* shape (L133–134 advance, L261 extend payment) but no backend reply mentions it and no probe (`run_gate4.py`, `run_n7n8.py`, `run_n11.py`) ever sent it — BQ-10/14 were verified with a single-method advance only. Default if silent: (a). |
