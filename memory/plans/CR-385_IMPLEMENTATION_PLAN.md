...
| OD-385-18 | Split at advance points (M1/M3/M4, D47-h) — payload field for split legs not yet in MASTER for `direct-reservation.advance` / check-in / extend `payment` | (a) probe backend; if unsupported, ship single-method advance now and park Split-at-advance to a follow-up · (b) block M1/M3/M4 until answered | **PROBED 2026-09-20 (`x1_split_advance.json`):** server accepts `method:"split" + split_payments[]` with 201 but stores/echoes only the lump `advance_payment 1000` — legs, method and txn id are not echoed. → **BQ-385-20** raised. **Plan default (a):** M1/M3/M4 ship Cash/Card/UPI single-method advance; Split tile at advance points parked until BQ-20 answers. Owner may still pick (b) block. |
