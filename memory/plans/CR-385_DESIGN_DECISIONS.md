...
### D81 — Phase 2 plan-note round: OD-385-19 closed, BUG-446 routed, New Booking placement locked (owner, 2026-09-22)
| Field | Value |
|---|---|
| Owner words (verbatim) | "Answers: a=flat, c=defer to FU-385-C, d=top-of-tab." · "drawer is out of scope — top-of-tab, F1 locked" |
| **OD-385-19 = (a) flat buttons — CLOSED** | Arrivals row keeps the P1 flat layout Check In · Modify · Cancel/No-Show. Phase 2 only makes the already-rendered `fd-row-<id>-checkin-btn` live; no kebab, no new row testids, X-10 unaffected. Mockup v2.22 kebab is superseded for the row-action layout. |
| **BUG-446 → DEFERRED-TO-FU-385-C** | Legacy header browser-date fix retires with the legacy pages. No code in P2 / P2.5. BUG_TRACKER + intake footer updated. |
| **New Booking placement = top-of-tab expansion (F1 locked)** | `+ New Booking` (header `fd-new-booking-btn`) toggles a `NewBookingForm` rendered between the KPI tile strip and the alert/search row of the current tab. Opening it collapses any open row/room expansion (`closeAll()`); opening a row/room expansion closes the booking form. A right-side drawer is **out of scope**. Plan §2.1 E12c/E12d. |
| Sandbox | Owner restores `extend_rate_mode` held → `calendar` via Channel Manager › Front Desk Rules and confirms before GO (entry probe found `held`, `s1_settings_list.json`). |

### D82 — New Booking RIGHT pane pre-save under D50: **design deviation vs mockup v2.18** (recorded 2026-09-22, owner-requested)
| Field | Value |
|---|---|
| Mockup v2.18 (D35/D36) | RIGHT pane shows a **live bill before Save**: room + meal-plan itemised, SGST / CGST as two lines, total, advance paid, balance — all computed client-side from the selected grid cell × nights. |
| D50 rule | The FE never computes GST or a total; every rupee shown comes from the server `charge{}`. `direct-reservation` has **no preview endpoint** — `charge` exists only in the 201 response. |
| **Deviation (what M1 renders pre-save)** | RIGHT pane = **"Stay summary"**, not a bill: selected room type + rate plan · **rate / night** as returned by `getRatesData` for those dates (server figure, display only, labelled "rate-table price · per night") · **N nights** (date arithmetic, not money) · advance block (amount · Cash/Card/UPI · reference) · readiness strip → "Ready to book" pill (D30 style) · one line **"Total, SGST, CGST and balance are priced by the server when you save"** (`booking-bill-pending`). **No** subtotal, **no** GST lines, **no** total, **no** balance before the 201. The advance input `max` is the grid cell × nights **for input validation only** (companion §4 M1, documented exception; never displayed as a figure). Sold-out type rows grey out from `room-availability`. |
| **Post-201** | Confirmation strip from `data.reservation.charge`: `booking-bill-total` (`total_with_gst`), `booking-bill-sgst`, `booking-bill-cgst`, advance (`charge.advance_payment`), balance (`charge.balance_due`) — the testids from companion §4 M1 keep their names but are populated **after** save. Then Save → `afterAction('Booking saved — <booking_id>')`, or Save & check in → CheckInForm for the new row. |
| Error path | 422 `"no rate configured"` → `booking-error` verbatim; HTTP 200 `skipped:true` / missing `data.reservation` → hard `booking-error` "Booking not created — <message>" (BQ-385-26), no toast, no refetch. Unit-tested. |
| Owner status | Recorded as requested ("state exactly what the New Booking RIGHT pane shows pre-save under D50 … record it as a design deviation vs mockup v2.18"). Booking design lock v2.18/v2.28 otherwise intact; mockup not edited. |

### D83 — Phase 2 QA gate (5B) CLOSED with acknowledged gaps (owner, 2026-09-22)
| Field | Value |
|---|---|
| Owner words (verbatim) | "please note these gaps , update docs and decision and close gate" — after the QA gap list (0 defects; upgrade path BLOCKED-BY-SANDBOX; T3/T7 live owed; end-state API read-back pending preprod 404; QA stay settled by the main agent; N-01/N-03 notes; BQ-385-26). |
| Decision | **Gate 5B (Phase 2 QA) CLOSED.** Evidence: `QA_REPORT_2026_09_22_CR385_P2_ROLE4.md` (it.17 smoke 13/13 · it.18 live T1/T4/T6 PASS · wire shapes verified · 0 defects). No Phase 2.5 needed. |
| Gaps carried (not waived — re-tested at Gate 6) | OG-PMS-038 upgrade path → owner smoke **M2-S08** · OG-PMS-039 early-check-in toggle cycle → **M2-S09/S10** · OG-PMS-040 F1/C7 live counts → **M2-S01** + next QA · OG-PMS-041 API end-state read-back → rerun when preprod login returns (tell owner first) · OG-PMS-042 Departures settle recipe for QA · OG-PMS-043 header-date flash (cosmetic) · OG-PMS-044 = BQ-385-26 (backend). |
| Gate 6 (Phase 2 owner smoke) | Open. Script = tracker `#m2s` M2-S01…S13 (HTML) = handover §Phase 2 owner smoke. Closes on **"Phase 2 smoke OK"**; any FAIL → Intake → Phase 2.5. |
| Phase 3 | Plan note may be prepared after Gate 5B close; **implementation only after "Phase 2 smoke OK" + "Phase 3 GO"** (phased plan §0 gate sequence unchanged). Entry condition BUG-433 (Extend pricing). |

### D84 — Phase 2 owner smoke DEFERRED and merged into the Phase 3 owner smoke (owner, 2026-09-22)
| Field | Value |
|---|---|
| Owner words (verbatim) | "Phase 2 owner smoke is DEFERRED and merged with the Phase 3 owner smoke — one combined smoke M2-S01…S13 + M3-S01…Sxx after Phase 3 QA. Gate 5B (P2) stays closed as-is." |
| Gate effect | Gate 5B (P2 QA) remains CLOSED (D83). Gate 6 for Phase 2 is **not** closed — it merges with Gate 6 for Phase 3: one combined session M2-S01…S13 + M3-S01…S10 after Phase 3 QA. Phase 3 planning may start now; **Phase 3 code only after "Phase 3 GO"** (no "Phase 2 smoke OK" prerequisite any more, by this decision). |
| Conditions (owner) | 1) OG-PMS-038 (paid upgrade), OG-PMS-039 (early check-in toggle), OG-PMS-040 (F1/C7 live) are **NOT waived** — re-tested inside the **Phase 3 QA (live)** AND in the combined smoke. 2) Phase 3 QA **must include a Phase 2 regression**: T1, T4, T6 re-run + T3 and T7 executed live + the upgrade path if an Executive room is free. 3) Any Phase 2 defect found later → **Phase 3.5** (§0-bis), never silently absorbed. |
| Records updated | phased plan §0 gate note + §2.5 status · registry `current_gate` · smoke batch header (M2 rows kept, run with M3 rows) · OPEN_GAPS 038/039/040 re-test target "P3 QA + combined smoke". |

### D85 — M5 row balance: explicit, temporary D50 exception for the balance cell only (owner, 2026-09-22, "Phase 3 GO")
| Field | Value |
|---|---|
| Owner words (verbatim) | "M5 row balance raw figure is FE-summed via the existing pmsService.getInHouseGuests (CR-358 pattern), display rounded with the shared applyGrandTotalRoundOff. This is an explicit, temporary D50 exception for the balance cell only; no new FE money math is added. File BQ-385-27: backend to expose a folio-level balance_due (room + room orders + transferred F&B, GST-inclusive) so P4/FU-385-C can replace the FE sum." |
| Implementation | `frontDeskService.getRowBalances` → one `getInHouseGuests({ roomGstApplicable })` call per snapshot (no maths copied) → `joinRowBalances` by `parentOrderId` → `roundBalance(raw, restaurant.totalRound !== false)` = `applyGrandTotalRoundOff` (CR-170, same helper as `CollectPaymentPanel` / `orderTransform`) → `{ raw, display, roundOff }`. Cell shows `display`, tooltip shows raw + round-off. Loading → "…", folio path failure → `charge.balance_due` + footer note. |
| Retire path | **BQ-385-27** (MASTER v2.4): folio-level `balance_due` on `local-reservations` rows → P4 / FU-385-C replaces the FE sum; D85 exception then closes. |
| Owner answers at GO | Q1 a (legacy Folio residual → FU-385-C, OD-385-20 = a) · Q2 a ("…" placeholder) · Q3 a (collect-now soft hint, server verdict verbatim) · Q4 a (409 → same-type free rooms only) · Q5: template left unresolved in the GO message → resolved empirically at QA time (upgrade leg runs iff an Executive room is free on the sandbox). |

### D86 — Combined P2+P3 owner smoke DEFERRED again and merged into the Phase 4 owner smoke (owner, 2026-09-22)
| Field | Value |
|---|---|
| Owner words (verbatim) | "The Gate 6 combined smoke (M2-S01…S13 + M3-S01…S10) is DEFERRED again and merged with the Phase 4 owner smoke. One combined smoke after Phase 4 QA: M2 + M3 + M4 rows. Gate 5B for P2 and P3 stays closed as-is." |
| Gate effect | Gate 5B P2 (D83) and P3 remain CLOSED. Gate 6 for P2 and P3 is **not** closed — it merges with Gate 6 for P4: one combined session M2-S01…S13 + M3-S01…S10 + M4-S01…Sxx after Phase 4 QA. Phase 4 planning starts now (plan note `plans/CR-385_P4_PLAN_NOTE_2026_09_22.md`); **Phase 4 code only after "Phase 4 GO"**. |
| Conditions (owner, mandatory) | **1)** Phase 4 QA (Gate 5B P4) = INDEPENDENT testing_agent, MUTATING, full-chain regression on the live sandbox: booking (advance) → check-in (collect-now; paid upgrade if an Executive room is free) → room-service order → extend +1 night (collect) → shorten → balance == folio → BILL & CHECKOUT via the new M6 panel → settle → LR/room read-back. Plus early-check-in toggle cycle ending OFF, F1/C7 live counts, BUG-433 paise case, BUG-447 live. If one testing_agent session cannot afford it, split into 2–3 sessions (each with its own cleanup) — do NOT substitute a self-driven main-agent run; a main-agent run may be added as extra evidence but does not close the gate. **2)** OG-PMS-038/039/040/041 re-test target = "P4 QA (live, independent) + combined smoke"; marked "deferred 4th time — must close in P4 QA". OG-PMS-041: retry `common-login` at P4 entry and P4 exit. **3)** Any P2/P3 defect found during P4 QA or the smoke → Phase 4.5 (§0-bis) with its origin phase tagged (P2/P3/P4). **4)** Smoke batch: rows M2-S01…S13 and M3-S01…S10 untouched; M4-S01…Sxx appended later; header "combined run after Phase 4 — ≈60 min, money — needs one Executive room free (stay 174 checked out or another) and a Departures settle path". **5)** No gate closes on summary words: every row gets the owner's verdict verbatim. |
| Records updated | phased plan §0 gate note + §2.5/§3.5 status lines · registry `current_gate` / `status` / `phase` · smoke batch headers (P2, P3, result line) · OPEN_GAPS 038/039/040/041 · master checklist status · PRD. |

### D87 — M6 checkout body: unchanged POS `collectBillExisting` + `room_gst_tax` passthrough — explicit D50 exception (owner, 2026-09-22, "Phase 4 GO")
| Field | Value |
|---|---|
| Owner words (verbatim) | "checkout body = unchanged POS collectBillExisting + room_gst_tax passthrough (same as PmsCheckoutDrawer L138–161); explicit D50 exception, host adds nothing else, pinned by unit test" |
| Implementation | `FolioCheckoutPanel.handlePaid` = drawer L128–172 copied: `orderToAPI.collectBillExisting(effectiveTable, placedItems, customer, paymentData, { autoBill, waiterId, restaurantName })` + `room_gst_tax = order.roomInfo.gstTax` (server folio value, BUG-386) → `frontDeskService.payBill`. Unit test `tests/cr385/phase4.cr385.test.jsx` "D87: … host adds nothing else" asserts the posted body keys == collectBillExisting output keys + `room_gst_tax` only. Panel room figures fed from LR `charge.*` (`roomInfoFromCharge`, M6-10) — BUG-425 hand formula not copied. |
| Retire path | **BQ-385-29** (MASTER v2.5, non-blocking): charge-only room checkout body (server prices the room leg from the reservation; FE sends intent + tendered amount) → D87 exception closes with FU-385-C. |
| Owner answers at GO | Q1 a (D87) · Q2 a (LEFT lines only, no FE subtotals) · Q3 a + b (upgrade line from the folio "Room upgrade: <reason>" line; **BQ-385-28** `upgrade_reason` on `charge`, non-blocking) · Q4 a (panel's own Split, `onOpenSplitBill={null}`) · **Q5 left as a template in the GO message → unresolved; resolved empirically at QA: the upgrade leg runs iff an Executive room is free; r2 is never touched without the owner's word; if still blocked → "not available — OG-PMS-038 open, 5th deferral, must close in combined smoke"** · Q6 a (every login announced before it happens) · Q7 a (sessions A/B/C). |

### OD-385-21 — Front Desk Bill: Credit/TAB block "visible + prefilled" accepted as a design deviation vs D47-c (owner, 2026-09-22, "Phase 4.5 GO — BUG-448")
| Field | Value |
|---|---|
| Owner words | "accept 'TAB block visible + prefilled' as a NOTE, record as OD-385-21 (design deviation vs D47-c; hiding needs a panel edit = hotspot, not allowed). In 4.5 check only whether the TAB name/phone inputs carry data-testids — if yes, hide them with a `.frontdesk-bill` host CSS rule (same pattern as Q6/E46); if no, leave visible." |
| Finding | `CollectPaymentPanel` L3124 wraps the block in `data-testid="tab-customer-section"` (inputs `tab-customer-name-input` / `tab-customer-phone-input`). |
| Implementation | Host prefill `billCustomer(order,row)` = `{ …drawer shape, name: row.guestName ‖ order.customerName, phone: digits-only last 10 of row.phone ‖ order.phone }` (`// CR-385 BUG-448`). CSS `.frontdesk-bill.fd-bill-tab-prefilled [data-testid="tab-customer-section"] { display:none }` — the class is set **only when the prefill satisfies the panel rule** (name + 10 digits); otherwise the block stays visible so staff can type (panel rule never bypassed). `handlePaid` still uses the drawer's `buildCustomer(order)` (D87 parity). |
| Legacy | `PmsCheckoutDrawer.jsx` L268 passes the same `{customerName, phone}` shape → same latent TAB behaviour on `/pms/departures` (staff must type name + phone) → **BUG-449** DEFERRED-TO-FU-385-C; smoke pre-read: "legacy Departures TAB: use Cash". |

### D88 — Front Desk Bill: Split tile hidden by host CSS (Phase 4.5b); Split for room stays → FU-385-D (owner, 2026-09-22)
|---|---|
| Owner words | "D88 / Phase 4.5b: Split hidden in Front Desk (Beta) Bill via host CSS under .frontdesk-bill (no CollectPaymentPanel edit; if no stable testid, STOP and tell me). Test: split not visible inside .frontdesk-bill, still present on POS. D88 recorded; plan §4 'Split → FU-385-D'; M4-S07 → 'Split tile NOT shown; second stay settled with Cash'; M4-S08 keeps POS split with a PRICED item; OG-PMS-048; FU-385-D in CR_REGISTRY." |
| Finding | `CollectPaymentPanel.jsx` L2740 `data-testid="payment-split-btn"` — stable testid exists → CSS route allowed (same pattern as Q6/E46 and OD-385-21). |
| Implementation | `frontdesk.css` `.frontdesk-bill [data-testid="payment-split-btn"] { display: none; }` (`/* CR-385 D88 */`). Guard `tests/cr385/hideSectionRows.cr385.test.js`: panel still renders the testid; the only CSS line mentioning `payment-split-btn` is the `.frontdesk-bill`-scoped one → `/dashboard` keeps Split. No panel edit; no hotspot touched. |
| Consequence | Front Desk (Beta) Bill = Cash / Card / UPI / Credit(TAB) single-method only. Room-stay split (multi-method, two ledger rows) → **FU-385-D** (CR_REGISTRY). Smoke rows: M4-S07 re-worded (Split tile NOT shown; Cash settle), M4-S08 keeps the POS split with a **priced** item (it.26 F-6: ₹0 probe never renders the tile). Gap **OG-PMS-048**. Superseded Q4 a ("panel's own Split") from D87 owner answers. |

### D89 — Gate 6 (combined P2 + P3 + P4 + CM-S01 owner smoke) CLOSED by owner word; Phase 5 entry conditions EC-1 + EC-3 met (owner, 2026-09-23)
|---|---|
| Owner words | "all smoke test passed" (2026-09-23) — read as the D86 closing word "Phase 2 + 3 + 4 smoke OK". Same day, on the P5 plan note questions: Q-1 "separate CR after CR-385 closes. Gate 5 closes without it; BUG-431/432/443/444/446/449, X-06 and legacy rounding stay DEFERRED-TO-FU-385-C with that sentence in the sign-off." · Q-2 "separate follow-up CR. D88 stays; `frontdesk.css` + `hideSectionRows` guard unchanged; OG-PMS-048 stays OPEN pending FU-385-D." · Q-3 "I rotate QA_TGK and update `memory/test_credentials.md` after the smoke, before your first mutating session; I'll say 'QA_TGK rotated'. Then scrub the historical files on my word." · Q-5 "approved — quarantine all 12 pre-existing suites + the 2 `process.exit` scripts to their owning CRs (record the mapping in OPEN_GAPS_REGISTER); P5 jest gate = `cr385|bug450` 128 green; no code in those suites." · Session 0 correction: "D17 / BUG-412 re-verify is mutating … remove it from Session 0 and put it in Session A. Session 0 = read-only guards only … You may run Session 0 now." |
| Consequence | Gate 6 CLOSED for P2/P3/P4/CM (smoke sheet Result block 13/13 · 10/10 · 9/9 · 1/1; M2-S08 recorded "not available — OG-PMS-038 open" unless the owner states PASS; M2-S11/M3-S06 optional). EC-1 ✅, EC-3 ✅ (OG-PMS-049 TRIAGED, mapping in OPEN_GAPS_REGISTER). FU-385-C and FU-385-D are separate CRs; the CR-385 sign-off must quote the DEFERRED-TO-FU-385-C sentence. D17 re-verify belongs to Session A (mutating). Session 0 executed 2026-09-23: Guards 1–6 PASS (`evidence/CR-385/probes_2026_09_23_release/SESSION0_GUARDS.md`); entry read-back deferred to "QA_TGK rotated". Sandbox stays FROZEN for agent mutations until "QA_TGK rotated" + "Phase 5 GO" (EC-4, EC-5 open). Q-4 (OG-PMS-038 room) default = path (c): owner writes "OG-PMS-038 deferred to FU-385-C — accepted" verbatim at sign-off. |
| Addendum 2026-09-23 | Owner on M2-S08: "1 pass as sign off this was checked in smoke" → **OG-PMS-038 CLOSED** (R-P5-2 path a). No "OG-PMS-038 deferred to FU-385-C — accepted" sentence needed at sign-off. Q-4 moot. |
| Addendum 2 2026-09-23 | Owner on M2-S11 / M3-S06 (both optional rows): "skip will check later" → recorded **skipped**; Gate 6 row table complete (12/13+skip · 9/10+skip · 9/9 · 1/1). No P5 dependency. |
