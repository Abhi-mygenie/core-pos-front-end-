...
**Verification (v2.14):** iteration_6 PASS on structure, room-in-LEFT → RIGHT live update (upgrade Paid raises total), collapsible cards + auto-collapse-when-complete (manual-expanded stays open), F1 lightbox, F3 pills (selected pill pure white), F4 2-col facts, a5/a1 states, mobile 390 stack, checkout 103/107 regression. iteration_6's one HIGH (RIGHT still scrolled / Confirm clipped at 1920×800) was fixed after the report: `fitCheckin()` rewritten to be viewport-aware (reveal expansion to panel top, then cap `--ci-height` to panel bottom − layout top − disclaimer) and the disclaimer compacted; Confirm now sits well within the 1920×800 viewport with the RIGHT column scroll-free (main-agent browser verified). A confirmatory re-test of that single containment fix is advisable next session.
