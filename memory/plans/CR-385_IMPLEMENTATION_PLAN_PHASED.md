...
| **P4** | M6 bill / checkout | CRITICAL (settlement) | Bill expansion (Layout B) with the real POS payment panel; TAB/Cash/Card/UPI (**Split → FU-385-D, D88 — hidden by host CSS in P4.5b**); SGST+CGST two lines (BUG-418). | none |
| **P5** | closure | — | Full regression (matrix §6 all rows), POS F&B regression, registry closure, FU-385-C decision (retire old pages) | — |

**Gate-sequence exception D84 (owner 2026-09-22):** the Phase 2 owner smoke is deferred and merged into the Phase 3 owner smoke (one combined session M2-S01…S13 + M3-S01…S10 after Phase 3 QA). Gate 5B (P2) stays closed (D83). Phase 3 QA must carry a Phase 2 regression (T1/T4/T6 re-run, T3/T7 live, upgrade path if Executive free); OG-PMS-038/039/040 not waived; late P2 defects → Phase 3.5.

**Gate-sequence exception D86 (owner 2026-09-22):** the combined P2+P3 owner smoke is deferred again and merged into the Phase 4 owner smoke (one combined session M2-S01…S13 + M3-S01…S10 + M4-S01…Sxx after Phase 4 QA). Gate 5B P2/P3 stay closed. Phase 4 QA = INDEPENDENT testing_agent, MUTATING, full-chain (booking → check-in → room-service → extend → shorten → balance == folio → BILL & CHECKOUT via M6 → settle → read-back) + toggle cycle ending OFF + F1/C7 counts + BUG-433 paise + BUG-447 live; 2–3 sessions allowed, no main-agent substitute. OG-PMS-038/039/040/041 deferred 4th time — must close in P4 QA (041: `common-login` retry at P4 entry + exit). P2/P3 defects found in P4 QA/smoke → Phase 4.5 tagged with origin phase. Every smoke row gets the owner's verdict verbatim. Plan note: `plans/CR-385_P4_PLAN_NOTE_2026_09_22.md`.
...
### 2.1 Existing-file edits — ~~none~~ **AMENDED 2026-09-22 (Phase 2 Entry Verification, handover §3 C1)** — line anchors verified by `grep -n` on the current tree; re-anchor by content if drifted (cross-phase checklist).
| Edit | File · line (current) | New (marker `// CR-385 M1` / `M3`) |
|---|---|---|
| **E11a** | `components/pms/frontdesk/ArrivalsPanel.jsx` L74 `<PhaseButton testId={\`fd-row-${r.id}-${variant}checkin-btn\`} label="Check In" phase={2} />` | add `onClick={() => onOpen?.(r.id, 'checkin')}`; **early-check-in guard (N7/D53, C2)**: when `!rules.allowEarlyCheckin && r.checkin > bd` render it disabled with `title="Arrives {fmtDate(r.checkin)} — modify the booking dates to check in today"` (`data-testid` `fd-row-<id>-checkin-btn` unchanged; tooltip id `checkin-early-tooltip` via `title`). |
| **E11b** | `ArrivalsPanel.jsx` L63 props `({ rows, meta, kpis, expandedId, expandedKind = 'detail', onToggle, onOpen, onDone, cancelledBy, chip, onChip })` | `+ rules, rooms, checkedInBy` (rules = `getFrontDeskRules()` result; rooms = `snapshot.rooms` for the booked-type chooser; `checkedInBy = useAuth().user.fullName` — C4). |
| **E11c** | `ArrivalsPanel.jsx` L89–106 `renderExpansion` switch (`modify` · `cancel`/`noshow` · default) | insert `case 'checkin': return <CheckInForm row={row} meta={meta} rooms={rooms} rules={rules} onDone={onDone} onClose={close} />;` before `default` (C6: success → `onDone('Checked in — Room rX')`; form root `stopPropagation` on click/keydown like L96). |
| **E11d** | `ArrivalsPanel.jsx` L2 header comment "Check In stays phase-gated (P2)" | reword "Check In live (P2, M3)". |
| **E12a** | `pages/pms/FrontDeskWorkstationPage.jsx` L74 `kind = detail \| modify \| cancel \| noshow` | `+ checkin` (comment only; `openExpansion` L80 already generic). |
| **E12b** | `FrontDeskWorkstationPage.jsx` L35–56 `useFrontDeskSnapshot` (or a sibling `useFrontDeskRules`) | read `getFrontDeskRules()` once per snapshot → `{ allowEarlyCheckin, extendRateMode, autoPrintCheckinReceipt }` (C2). Failure → defaults `false / calendar / false` + no page error. |
| **E12c** | `FrontDeskWorkstationPage.jsx` L125 `onNewBooking={null}` | `onNewBooking={() => setBookingOpen(v => !v)}` + new state `bookingOpen`; opening the booking form calls `closeAll()` (F1 one-open-at-a-time). |
| **E12d** | `FrontDeskWorkstationPage.jsx` L140–141 (between `<KpiTabStrip …/>` and the AlertBar/GlobalSearch row) | render `{bookingOpen && <NewBookingForm meta={snap.meta} onDone={afterAction} onCheckInNow={(res) => { setBookingOpen(false); navigateTo({ tab:'arrivals', rowId: res.id, kind:'checkin' }); }} onClose={() => setBookingOpen(false)} />}` — top-of-tab expansion (handover C1). |
| **E12e** | `FrontDeskWorkstationPage.jsx` L116 `<RoomsPanel …/>` · L117 `<ArrivalsPanel …/>` | RoomsPanel `+ onCheckIn={(rowId) => navigateTo({ tab:'arrivals', rowId, kind:'checkin' })}`; ArrivalsPanel `+ rules={rules} rooms={snap.rooms} checkedInBy={user?.fullName \|\| 'staff'}`. |
| **E13** | `components/pms/frontdesk/RoomDetail.jsx` L73 `<PhaseButton testId={\`fd-room-action-checkin-${room.id}\`} label="Check In" phase={2} />` | `+ onClick={room.reservation ? () => onCheckIn?.(room.reservation.id) : undefined}` (booked tile → Arrivals row expanded as `checkin`; no linked reservation → stays disabled, D17 "missing link shows a block"). Prop `onCheckIn` added to the signature; L1 header reworded. |
| **E14** | `components/pms/frontdesk/RoomsPanel.jsx` L81 `<RoomDetail … onPatch={onPatch} onClose=…/>` | pass-through `onCheckIn={onCheckIn}` (+ prop on the panel signature). |
| **E15** | `api/services/frontDeskService.js` — **append after L40** (`previewModifyReservation`) | `createBooking(body)` → `api.post(AIOSELL_ENDPOINTS.DIRECT_RESERVATION, body)`; `getAvailability({checkin, checkout})` → `api.get(AIOSELL_ENDPOINTS.ROOM_AVAILABILITY, { params })`; `checkIn(p)` = the `pmsCheckIn` FormData builder **copied** from `pmsService.js` L218–283 + `aiosell_reservation_id`, `upgrade_type/amount/reason`, `room_price=0 / order_amount=0 / gst_tax=0` (server prices), sent with explicit `multipart/form-data` (C10); `getRatesData` re-exported from `pmsService` L440 (no edit there). **Never** `rate_per_night`, `room_price>0`, `amount_after_tax` (X-01 grep guard extended). |
| **E16** | `api/constants.js` L581 `LOCAL_CHECKIN: '/api/v1/vendoremployee/pos/user-group-check-in',` | **insert after**: `ROOM_AVAILABILITY:   '/api/v2/vendoremployee/aiosell/room-availability', // CR-385 M1 BQ-385-06` — **not in the handover list; found at Entry Verification** (`grep room-availability src/` = 0 hits). |
| **E17** | `api/services/restaurantSettingsService.js` L36–43 `getFrontDeskRules` returns `{ allowEarlyCheckin, extendRateMode }` | `+ autoPrintCheckinReceipt: Boolean(basic.auto_print_checkin_receipt ?? basic['pms.auto_print_checkin_receipt'] ?? false)` (D32, C2). Read-only; `updateFrontDeskRules` untouched (M7 keeps writing two keys). Key confirmed present in `settings-list` (probe `probes_2026_09_22_p2_entry/s1_settings_list.json`). |
| **E18** | `components/pms/frontdesk/GuestTable.jsx` L147–148 `PhaseButton = ({ testId, label, phase, danger, onClick })` · `title={onClick ? undefined : \`Available in Phase ${phase}\`}` | `+ title` prop override (`title={title ?? (onClick ? undefined : …)}`) so the early-check-in tooltip can reuse the component. Handover C1 said "PhaseButton phase 2 → live" — verified: the button is already live whenever `onClick` is passed; only the tooltip override is needed. |
| — | `WorkstationHeader.jsx` L38 `fd-new-booking-btn disabled={!onNewBooking}` · `KpiTabStrip.jsx` | **no edit** (already wired to the prop; handover "possibly header/KpiTabStrip" → verified not needed). |

**Not touched (hard):** `pmsService.js` (hotspot — `createDirectReservation` L195 keeps OD-P2-07 for the legacy page; `pmsCheckIn` L218 copied, not edited), `pages/pms/CheckInPage.jsx`, `pages/pms/NewBookingPage.jsx` (copy sources; BUG-431/432 retire with FU-385-C), `CollectPaymentPanel.jsx`, `orderTransform.js`, `updateFrontDeskRules`.

**OD-P2-07 vs BQ-385-10 (plan note, 2026-09-22):** `pmsService.js` L195 `/** S3 … OD-P2-07: never sends advance. */` is the CR-358-P2 decision for the **legacy** `NewBookingPage` S3 flow (`rooms:[{restaurant_table_id}]` + `order_amount`, no advance). MASTER **BQ-385-10 explicitly replaces OD-P2-07** for the Front Desk path: `advance{amount, method, reference}` on `direct-reservation` is VERIFIED build 2 (`probes_2026_09_20_final/s2_create_advance.json`: advance 1000 → `charge.advance_payment 1000`, `balance_due 9148`; D2 fixed → carried to check-in as "booking carry"). **Resolution: no conflict — two callers, two decisions.** M1 sends `advance` only through the **new** `frontDeskService.createBooking` (payload BQ-10/16: `guest{name,phone}, checkin, checkout, adults, children, rooms:[{room_code, rateplan_code, rooms_count:1}], advance?`). `createDirectReservation` stays byte-identical (legacy keeps OD-P2-07 + BUG-432 until FU-385-C). Grep guard: `advance` must not appear in `NewBookingPage.jsx` (V8b of CR-358-P2 still holds).

**Owner decisions locked at the plan-note round (D81/D82):** OD-385-19 = (a) flat row buttons · BUG-446 → DEFERRED-TO-FU-385-C · New Booking = **top-of-tab expansion, F1 locked, drawer out of scope** · RIGHT pane pre-save = "Stay summary" without GST/total (D82 deviation vs v2.18) · `skipped:true` / missing `data.reservation` = hard `booking-error` (BQ-385-26).

**Fixtures (P2, existing convention `src/__fixtures__/cr385/*.json` = saved probe bodies, imported by relative path in the tests):** `direct_reservation_201.json` ← `probes_2026_09_20_final/s2_create_advance.json` (body) · `direct_reservation_skipped.json` ← `probes_2026_09_22_p2_entry/s7_direct_reservation_skipped.json` · `direct_reservation_422_norate.json` ← `probes_2026_09_20_gate4/s1b_bq16_unknown_plan.json` · `room_availability.json` ← `probes_2026_09_22_p2_entry/s3_room_availability.json` · `checkin_200_upgrade.json` ← `probes_2026_09_20_final/s3_checkin_upgrade.json` (body) · `settings_list_basic.json` ← `probes_2026_09_22_p2_entry/s1_settings_list.json` (redact nothing money-related; no tokens inside). Existing `local_reservations_view_all.json` / `room_status_board.json` reused for rows + rooms.

**Contract facts from the read-only Entry probes (2026-09-22, `evidence/CR-385/probes_2026_09_22_p2_entry/`):** `direct-reservation` with incomplete dates returns **HTTP 200 `{status:true, skipped:true, message:"Direct reservation skipped: incomplete stay dates.", data:null}`** — the FE must treat `skipped === true || !data?.reservation` as a failure (never toast success on a bare 200); unknown plan → **422 `{status:false, message:"no rate configured"}`** (gate4 `s1b_bq16_unknown_plan.json`); `room-availability` → `data.rooms[{restaurant_table_id, table_no, title, aiosell_room_code, available, blocked_by, booking_id}]`; `settings-list` `basic.*` carries `allow_early_checkin`, `extend_rate_mode`, `auto_print_checkin_receipt` (+ `pms.*` aliases).
**Entry conditions BUG-431 / BUG-432 — SATISFIED 2026-09-22:** fixed by construction in M1/M3 and QA-verified (V-M1-02 no `rate_per_night`; V-M3-01 zero money fields; it.18). Legacy pages DEFERRED-TO-FU-385-C.

**Gate status (2026-09-22):** Phase 2 IMPLEMENTED (Gate 5A) → QA it.17/it.18, 0 defects → **Gate 5B CLOSED by owner (D83) with acknowledged gaps OG-PMS-038…044** (upgrade path sandbox-blocked, toggle cycle / F1-C7 live counts owed, API end-state read-back pending preprod login 404). Gate 6 = owner smoke `frontend/public/cr385-master-checklist.html#m2s` M2-S01…S13 → "Phase 2 smoke OK". Phase 3 implementation only after that + "Phase 3 GO".
...
4. 201 → `data.reservation.charge` → confirmation strip (total, SGST, CGST, advance, balance from `charge`) → `onDone` (refetch) or `onCheckInNow(reservation)`; 422 "no rate configured" → `booking-error` verbatim; **HTTP 200 `skipped:true` or missing `data.reservation` → hard `booking-error` "Booking not created — <message>", no toast/refetch (BQ-385-26, unit-tested)**. **Pre-save RIGHT pane = "Stay summary" (D82 deviation vs v2.18): type + plan, server rate/night (display), N nights, advance block, readiness pill, `booking-bill-pending` note — no client GST/total/balance.**
...
**Status 2026-09-22 (D84): smoke DEFERRED → combined with §3.5.** Rows M2-S01…S13 live in `control/POS_PMS_2_OWNER_SMOKE_BATCH_2026_09_21.md` and run together with M3-S01…S10 after Phase 3 QA. **Status 2026-09-22 (D86): DEFERRED AGAIN → combined with §3.5 + §4.5 after Phase 4 QA (one session M2 + M3 + M4 rows).**
...
### 3.1 Existing-file edits — ~~none~~ **AMENDED 2026-09-22 (Phase 3 plan note): E31–E38, see `plans/CR-385_P3_PLAN_NOTE_2026_09_22.md` §3** (`frontDeskService` +extendStay/getRowBalances · `DeparturesPanel` · `InHousePanel` · `GuestTable.commonColumns` amountOf · `FrontDeskWorkstationPage` balances + kind extend · `RoomDetail`/`RoomsPanel` onExtend · `frontDeskTransform` nightsDetail/orderId · P1/BUG-439 test flips). Entry condition **BUG-433** fixed inside M5 (CR-170 display round-off shared with the bill; RCA in the plan note §2).
...
**Status 2026-09-22 (D86): DEFERRED → combined with §2.5 + §4.5 after Phase 4 QA (one session M2-S01…S13 + M3-S01…S10 + M4-S01…Sxx). Rows M3-S01…S10 live in the smoke batch, untouched.**
...
### 4.1 Existing-file edits — ~~none~~ **AMENDED 2026-09-22 (Phase 4 plan note): E41–E47, see `plans/CR-385_P4_PLAN_NOTE_2026_09_22.md` §3** (`DeparturesPanel`/`InHousePanel` Bill live + `bill` expansion · `RoomDetail`/`RoomsPanel` onBill · `FrontDeskWorkstationPage` kind bill · `frontDeskService` +getFolio/payBill · `frontdesk.css` Q6 rules · P1/P3/BUG-439 test flips). `CollectPaymentPanel.jsx` imported only; `PmsCheckoutDrawer.jsx` untouched (copy source). Entry condition **BUG-418** fixed inside M6 (SGST/CGST two lines from `charge`). **Phase 4 GO 2026-09-22 → IMPLEMENTED (jest 116) → Phase 4.5 BUG-448 (jest 120) → Gate 5B P4 QA PASSED (it.23/25/26, QA_TGK) → Phase 4.5b D88 Split hidden (`frontdesk.css` +1 rule, guard test) → Split → FU-385-D. Next: combined owner smoke §2.5 + §3.5 + §4.5 rows + CM-S01 (D86).**
