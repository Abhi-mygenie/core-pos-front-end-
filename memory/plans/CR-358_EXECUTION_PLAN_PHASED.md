...
| **MISSING-01** — `GET /aiosell/dashboard-kpis` | ✅ **DELIVERED — agent-verified 200 (2026-09-03)**, requires start_date/end_date (≤31 days). Shape in `impact/CR-358-P3_IMPACT_ANALYSIS.md` | S1 KPI strip | Phase 3 (non-blocking skeleton per NS-02) |
| **MISSING-02** — `GET /aiosell/room-status-board` + `PATCH /aiosell/room-status/{table_id}` | ✅ **DELIVERED 2026-09-02** — full spec in handover_3.md. `room-status-board` returns ALL RM rooms with pre-computed `display_status` (5 states). PATCH accepts hk/ooo/available; 422 if occupied; `inventory_push_warning` on push fail. Auto-HK on RM checkout confirmed. Migration: `2026_09_02_140000_add_room_operational_status.php` | S7 Room Status Board | Phase 4 — **ENTRY GATE CLEARED** |
...
5. ~~**Before Phase 4 QA:** Build `PATCH room-status` + `room_status` column.~~ ✅ **DONE 2026-09-02** — `GET room-status-board` + `PATCH room-status` both delivered via handover_3.md. Migration `2026_09_02_140000_add_room_operational_status.php` to be confirmed run on preprod before Phase 4 QA.
...
**Entry gate:** Phase 3 closed (local-reservations stable). ✅ **MISSING-02 CLEARED 2026-09-02** — both Phase 4 endpoints delivered: `GET /aiosell/room-status-board` + `PATCH /aiosell/room-status/{table_id}`. **Migration `2026_09_02_140000_add_room_operational_status.php` must be confirmed run on preprod before Phase 4 QA starts.**
...
| `api/transforms/roomStatusTransform.js` | NEW — ⚠ **APPROACH CHANGED 2026-09-02**: original plan was two-source merge (GET_ROOM_LIST + GET /aiosell/rooms). Now: single `GET /aiosell/room-status-board` returns all 5 states pre-computed server-side (available/occupied/booked/hk/ooo). `fromRoomStatusBoard(data)` maps `rooms[]` → S7 tile models; extracts `guest{}` + `reservation{}` blocks. See NS-B/NS-C in investigation report. |
...
| NS-B | `roomStatusTransform.js` scope changed — two-source merge superseded by single `room-status-board` endpoint | P4 | Phase 4 Gate 3 plan must redesign transform around single endpoint. Do NOT implement old two-source approach. |
| NS-C | "booked" is a 5th `display_status` — not in original plan | P4 | S7 board needs Booked tile with `reservation.channel`, `checkin`, `guest_name`. Precedence: ooo → occupied → hk → booked → available. |
| NS-D | 422 when marking occupied room HK/OOO | P4 | FE must handle 422 from PATCH — show error toast: "Cannot mark — room has an active order" |
| NS-E | `inventory_push_warning` in PATCH response | P4 | After 200, check `data.inventory_push_warning`. If non-null → secondary warning toast: "Status saved. Inventory sync failed — will retry" |
...
- P4: ~~does backend add `room_status` to `GET /aiosell/rooms` response or a separate endpoint?~~ ✅ ANSWERED 2026-09-02 — dedicated `GET /aiosell/room-status-board` endpoint. No changes to `/aiosell/rooms`.
