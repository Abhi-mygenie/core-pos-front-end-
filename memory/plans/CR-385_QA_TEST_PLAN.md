...
### 2C. New Booking — v2.28 (LOCKED; was v2.18/v2.19)  ·  hook `?booking=1`
- **Type-only** (reserves a room TYPE from inventory, not a room number; room number assigned at
  ...
  cols=plans, each cell = all-in per-night price; one click selects both type & plan). **No B2B/GST
  at booking** (v2.28, owner D48-c): a note under the Guest block says company/GST billing is
  captured at Check-In. Optional **advance** → passed to Check-In as already paid. RIGHT bill shows
  advance. `Save` and `Save & Check in now`.
- Verify: grid cell selects type+plan and updates price; advance shows in bill; row re-click
  collapse; Save & Check in now carries type/guest/dates/advance into Check-In; `booking-b2b-toggle`
  / `booking-gst-name` / `booking-progress-gst` / `booking-bill-gst` are ABSENT; Check-In still has
  `checkin-b2b-toggle`.
...
## 6b. v2.27 SETTLEMENT REVISION (D47) — self-verified 2026-09-19 at 1366×768, hook `?bill=102`, 0 JS errors
| # | Check | Result |
|---|---|---|
| S-01 | Tiles = Cash · Card · UPI · Split · Credit (`payment-method-*`) | PASS |
| S-02 | Cash: tendered = grand, Checkout enabled, "Settled" | PASS (panel 250 px) |
| S-03 | Card without Txn/UTR → Checkout disabled, reason "Txn / UTR no. is required for Card"; with ref → enabled | PASS |
| S-04 | Split empty → "₹0 tendered · ₹3,785 remaining", reason "Collect the full amount or select Credit", Checkout disabled | PASS |
| S-05 | Split Card 1000 → Remaining ₹2,785; UPI 99999 blurred → clamped to 2785 (POS parity) | PASS |
| S-06 | Card row amount > 0 without 4-digit Txn ID → disabled, reason "Card row needs Txn ID · last 4"; with 1234 → enabled, "Settled" | PASS |
| S-07 | Credit → "Billed to guest's TAB · <name> · <masked phone>", ₹0 collected · on TAB, Checkout enabled | PASS |
| S-08 | Two-step: first click → "Confirm checkout · ₹3,785" (3 s arm) | PASS |
| S-09 | AC-19 no right-pane scroll at 1366×768 in all states (aside scrollHeight == clientHeight; Split state 438 px inside 560 px row) | PASS |
| S-10 | No-Show refund arithmetic shows "PHASE 2" ribbon (`noshow-phase2-ribbon`) | PASS |
| S-11 | Rooms › Area titles = Ground floor · First floor · 2nd floor · 3rd floor · Patal Lok (D47-f) | PASS |
| S-12 | `payment-received-input` removed (0 occurrences); no under-payment path | PASS |
Pending: owner visual acceptance of v2.27; Safari/Edge manual. Backup of v2.26: `evidence/CR-385/cr385-frontdesk-mockup_v2.26_backup.html`.

## 6c. v2.27 — Split at advance / collect-now points (D47-h) — self-verified 2026-09-19 at 1366×768, 0 JS errors
| # | Surface · hook | Check | Result |
|---|---|---|---|
| A-01 | Check-In `?checkin=a2` | Tiles Cash·Card·UPI·Split disabled until amount; 1000 + Split → 3 rows + Card Txn row; Card 600, UPI 900 → clamped 400; "Split matches ₹1,000"; readiness lists "Card row Txn ID · last 4" until 4 digits | PASS |
| A-02 | Check-In pane fit | right pane scrollHeight == clientHeight (342/342) in Split state | PASS |
| A-03 | New Booking `?booking=1` | advance 500 + Split → rows; Card 200, UPI 999 → clamped 300; readiness adds/removes Txn ID rule; pane 358/358 | PASS |
| A-04 | Extend Stay (In-House › Extend) | collect 500 + Split, UPI 500 → "Ready · collect ₹500 now · ₹5,595 stays on folio" | PASS |
| A-05 | Modify (Arrivals › Modify, checkout +2) | collect 300 + Split → readiness "Split rows must total ₹300" | PASS |
| A-06 | Regression Bill `?bill=102` | §6b S-01…S-12 unchanged (same helpers not used there; bill keeps its own settlement block) | PASS (re-run S-01/S-04/S-07 spot) |
| A-07 | Blur re-render | deferred redraw (setTimeout 0) — no "outerHTML … no longer a child" error | PASS |
Not run: testing-agent QA audit across all four tabs (owner asked for it; deferred with the build hold — schedule with owner visual acceptance of v2.27). Safari/Edge manual pending.

## 6d. iteration_28 — testing-agent four-tab QA audit of v2.27 (2026-09-19) → 29/30 PASS, 0 console errors; 2 findings fixed same day
| # | Finding | Fix | Re-measure |
|---|---|---|---|
| P1 | Check-In right pane overflow in Split state at 1366×768 (354/336). Root cause: the advance "Remaining" line carried class `due`, which collides with the generic boxed `.due` rule (34 px box instead of a 12 px line) + readiness wrapping to two lines. | Class renamed `rem-due` (bill + advance); split grid compacted (rows 24 px, gap 2 px, margin 0, line-height 12); shorter shared validation copy "Split must total ₹X" / "Card Txn ID (4 digits)". | `.ci-pane-body[1]` = 332/332 in empty / card / sum / done states; Booking 348/348; 0 JS errors |
| P3 | Room Detail header title-cased the real board title ("2nd Floor") vs SECTIONS "2nd floor" (D47-f). | `secLabel()` now keeps the title verbatim (whitespace-collapsed, first letter capitalised). | `?room=223` → "· 2nd floor" |
Evidence: `/app/test_reports/iteration_28.json`. Owner accepted v2.27 before the audit; fixes are cosmetic/layout only — no rule change. **Design Gate 2.5: v2.27 CLOSED.**

## 6e. v2.28 — B2B removed from New Booking (D48-c) — testing agent iteration_30, 2026-09-20, ALL PASS
- `?booking=1`: `booking-b2b-toggle` / `booking-gst-name` / `booking-gst-number` / `booking-progress-gst` / `booking-bill-gst` ABSENT; `booking-b2b-note` present ("captured at Check-In"); header + disclaimer say v2.28.
- Booking flow intact (CRM autofill, grid cell → bill, advance 500 → balance = total − 500, RIGHT pane no inner scroll, save button < 800 px). Check-In `?checkin=a2` still has `checkin-b2b-toggle` → GST fields + readiness. `?bill=103` renders (grand total ₹6,152 since v2.26 — the ₹2,677 in older notes is stale). 0 JS errors on 4 URLs. Report `/app/test_reports/iteration_30.json`.
- Backend-side acceptance now governed by AC-22 (D50 money contract); live verification evidence `evidence/CR-385/probes_2026_09_20_gate4/`.
