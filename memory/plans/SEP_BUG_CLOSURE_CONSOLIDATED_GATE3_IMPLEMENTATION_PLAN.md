...
```
**Gate 3 closed ☑ 2026-09-24 (owner "close gate 3") · Plan validated ☑ 2026-09-24 (code-reality re-check at HEAD `a4c9196f`) · Gate 4 GO ☑ 2026-09-24 (owner verbatim "gate 4 go", VD-8 = **Option C**) · IMPLEMENTED ☑ 2026-09-24 (D-1/D-2/D-3/D-4; jest 69/69; `yarn build` exit 0) · Gate 5B combined QA ☐**

**VD-8 DECISION (owner 2026-09-24) — Option C (OD-452-06):** code-level probe: `OrderEntry.jsx` L152/L154 `useEffect(() => { if (initialShowMerge) setShowMergeModal(true) }, [initialShowMerge])` + L166 `useState(initialShowPayment)` run on every mount, and the flags are only cleared in `handleCloseOrderEntry` → a dismissed modal WOULD re-open on remount (Option A rejected: owner "when staff doesn't decide to merge … it should not show"). Option B rejected (keeps BUG-334 carry-forward alive in Merge/Shift/Payment sessions). **Implemented:** D-1 and D-2 guarded block is `if (orderEntryType !== null) { setOrderEntryResetNonce(n => n + 1); setInitialShowMerge(false); setInitialShowShift(false); setInitialShowPayment(false); setInitialTransferItem(null); }` (+12 additive lines total instead of +4; same 2 handlers, same scope lock). Dashboard-entry paths (`setInitialShowMerge(true); handleTableClick(t)` at L1808/L1812/L1988/L1991, payment L1573, transfer L1579) are unaffected because `orderEntryType === null` at that moment. **D-4 fallback recorded:** structural source test (`DashboardPage` 5-context + router + socket-hook fan-in makes a mounted RTL harness impractical) — behavioural VD-1..VD-11 belong to the preprod QA run.

**Line drift annotation (2026-09-24):** BUG-453 commit `3783ee8` added 2 lines before `handleTableClick` (import at L24 + toggleSnooze line at L1282). D-1 insert point shifted from plan's L1485 → **current L1487**; D-2 from plan's L1504 → **current L1506**. D-3 `OrderEntry.jsx` BUG-334 block from plan's L504–508 → **current L505–508**. Plan is still valid — insert by code content, not line number.

**VD-8 probe requirement before Gate 4 GO:** Run 2–3 preprod probes (open merge/shift/payment modal inside OrderEntry → trigger in-OrderEntry table switch via header → observe modal state). Save evidence to `evidence/BUG-452/VD8_probes/`. Present owner with A (fine as-is) / B (guard bump) / C (other). Record decision here and in registry. Only after owner A/B/C + verbatim "Gate 4 GO" may implementation begin.
