...
**Gate 2 status:** **CLOSED 2026-09-24 (owner)** — shape **S1 (remount via nonce) LOCKED** (Q4 "ok as suggested"); **BUG-334 REVERSED annotation APPROVED** (Q5 "yes"); **discard-confirm dialog REJECTED → silent clear** (Q6 "Silent clear"). Gate 3 NOT started (owner: "do not start gate 3").
**Registry drift found 2026-09-24:** `impact/BATCH-04_IMPACT_ANALYSIS.md` (BUG-334 IA referenced by the registry) does **not exist** in this checkout and `BUG_TRACKER.md` has no BUG-334 row → the BUG-334 reversal annotation lands in `registry.json` (`status_history`/`notes`) and in the `OrderEntry.jsx:504–508` code comment only.
...
**→ OWNER DECISION 2026-09-24: S1 LOCKED (D-452-S = a).** Owner walkthrough given: S2 requires a hand-maintained per-field reset list inside the R5 effect (miss one → half-cleared screen; every future draft field must remember to join the list); S1 lets React reset every `useState`/`useRef` by construction via the existing `key={orderEntryResetNonce}` mechanism already used by PROD-004 at `:1541`.

### BUG-334 reversal bookkeeping — **APPROVED by owner 2026-09-24 (Q5 "yes")**
- `registry.json` BUG-334: append `status_history` "REVERSED BY BUG-452 (OD-452-01 Option B, owner 2026-09-23; annotation approved 2026-09-24)"; status text stays CLOSED with the annotation. **Done 2026-09-24 (docs session).**
- `BUG_TRACKER.md` BUG-334 row: **no row exists in this checkout** → note recorded in the 2026-09-24 tracker section instead.
- `impact/BATCH-04_IMPACT_ANALYSIS.md` (BUG-334 IA): **file absent in this checkout** → cannot annotate; registry note carries the pointer.
- Code: `OrderEntry.jsx:504–508` comment rewritten to `// BUG-334 carry-forward REVERSED by BUG-452 (owner 2026-09-23)…` so §F code↔registry cross-check stays truthful. **Code edit happens at Gate 5 only.**
...
| Risk | L | Mitigation |
|---|---|---|
| Cashier loses a half-built cart on an accidental type click | **Accepted by owner (Option B)** | **D-452-04 "Discard N items?" confirm dialog — REJECTED by owner 2026-09-24 ("Silent clear").** No dialog; clean slate exactly as locked. |
...
Owner decisions (ALL LOCKED 2026-09-24): D-452-S = S1 remount · BUG-334 REVERSED annotation APPROVED · D-452-04 discard-confirm REJECTED (silent clear)
