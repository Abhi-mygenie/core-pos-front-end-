...
### Owner-approved v2.8 feedback amendment (continuation after v2.7)
| Feedback | Approved change | Verification |
|---|---|---|
| FB-1 / D9 | Right opens at Bill Summary, with Adjustments still above and reachable by scrolling up. Preserve scroll and focused input on edits. | Open Room 103, scroll up, edit, verify no jump back to Summary. |
| FB-2 / D10 | Checkout visible outside scroll body; viewport-bound columns and bring the opened expansion into view. | Checkout rectangle stays within workstation/browser while either column scrolls, at both desktop width presets. |
| FB-3 / D11 | Left ROOM heading → Adjustments (room discount disabled) → Room Summary; guest/stay and transfers remain left. | Shared section styling/order; no second payment block or active room discounts. |
| FB-4 / D12 | Neutral white settlement; exact order **Room orders (F&B) → Transferred orders → Room balance → Grand Total**. | Owner: `room transfer room balance grand total`, confirmed `yes`. Verify text order and Room 103 ₹986 + ₹741 + ₹950 = ₹2,677. |

### Owner-approved v2.9 amendment (after visual feedback on v2.8)
- **D13:** hide prototype toolbar and artificial frame margins; use full available page width/height. User wants the actual page appearance, not variant/test controls.
- **D14:** all Room/F&B/Transferred details and applicable adjustments on LEFT in three collapsed-by-default sections. Each heading total must equal its matching RHS row. RIGHT only neutral final figures plus single payment/Checkout. Supersedes D9 Summary-first RHS scrolling and the old equal fixed-height bill layout; bounded left scroll remains for long expanded details.
- **D15:** Room ledger = Booking amount(total all nights), Advance, pre-GST Balance, SGST, CGST, final payable Total. Preserve original tax base, paid amounts and final totals. Other existing room payments get a conditional deduction before final Total.
- **Impact / risk:** moving F&B controls outside the existing right-side panel means Q6 is broader than a hide-three-rows switch. Future spike must prove one shared financial state supplies LHS controls, LHS heading totals and RHS settlement, without copying formulas or creating a second payment path. Original b/c hide-only options and old line estimates are historical, NOT an approved solution for v2.9. Record this under OG-PMS-021 and consolidate at Rev3.2 after visual approval.
- No API, auth, integration or application-source changes in this amendment; D6 production eligibility and D7 CRM loyalty requirement remain accepted.

**Current gate state:** Q1–Q5 and D-1/D-2 answered; Q6 implementation mechanism remains OPEN. Owner approved v2.9 design amendments D13–D15; revised rendered mockup still needs visual review. Gate 2.6 remains OPEN; no registry advancement. BQ-385-07 brief and Rev 3.2 consolidation remain pending after design review.

### Rev 3.2 notes (post Layout-B / Adjustments decision)
- `CheckOutExpansion.jsx` (#10): target left = ROOM Adjustments → Room Summary + Transferred Orders (D11); target right = existing F&B adjustment/summary content with viewport-bound **pinned settlement** (D9/D10/D12). **Source correction (OG-PMS-021): `CollectPaymentPanel` scroll body starts at L1321; Payment Method at L2681 is inside that body until L3297; only Pay Button L3299–3326 is outside.** Therefore the prior statement that Payment Method was already pinned was incorrect. Hiding the three section rows alone does NOT deliver the target layout. Gate 2.6/Q6 must account for scroll anchoring, settlement placement and row order; Gate 3 spike must prove it without changing financial formulas. No hotspot edit or final existing-line estimate is approved by this mockup update. Room discount remains **disabled UI-only stub in v1**; no payload or arithmetic changes.
...
- Q6 mechanism affects §2 existing-edits count: the old ~4-line estimate applies to hiding section rows ONLY, not the full pinned settlement/scroll/reorder design. A wrapper/prop versus panel layout decision remains owner-gated; exact scope must be verified in the approved Gate 3 spike before any source plan is frozen.
