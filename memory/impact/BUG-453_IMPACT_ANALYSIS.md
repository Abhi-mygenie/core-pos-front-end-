...
**Intake:** `change_requests/BUG-453_MUTE_DOES_NOT_STOP_RINGER_INTAKE.md` · ODs all LOCKED (Fix A+B one bug · anti-rule override approved · per-order manual mute toggle, no timer)
...
- Any later FCM whose `data.order_id` (or `orderId`) === X → `play()` skipped (toast/list entry still added — visual path unchanged). Log line `[Notification] Sound muted for order X`.
...
| 2 | `src/contexts/NotificationContext.jsx` | Before `soundManager.play(resolvedSound)` (:132–135): derive `notifOrderId = String(data.order_id \|\| data.orderId \|\| '')`; skip play when `soundManager.isOrderMuted(notifOrderId)`; logout effect `soundManager.clearMutes()` | ~5 | NO |
...
| `notifications[]` list / toasts | Still populated for muted orders (owner semantics: mute = sound only). If owner wants toast suppression too → new OD (not assumed) |
...
## 6. Owner decisions
All LOCKED at Gate 1. **One clarification for Gate 3 (default assumed if silent):** muted order still shows toast + notification-list entry (sound-only mute). Confirm or say "suppress toast too".
...
3. Browser (preprod, alias `cafe103_no_rooms_postpaid_gst`): trigger two YTC FCMs quickly (or DevTools → dispatch two `processNotification`) → Mute → silence; simulate retry → still silent; Mute again → rings.
4. Sidebar Silent Mode ON → nothing plays regardless of mute set.
5. Logout → login → mute set empty.
6. Regression: OrderCard bell dim toggles; pop-out re-pops after 2 min (visual) without sound; normal (non-muted) orders still ring; `yarn build` 0 new warnings.
...
Owner decisions: none blocking; 1 clarification (toast stays visible for muted order — default YES)
