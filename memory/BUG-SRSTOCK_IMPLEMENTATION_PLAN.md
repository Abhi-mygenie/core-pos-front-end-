# BUG-SRSTOCK — Implementation Plan (Gate 3)
# Sub-Recipe Stock Panel: Produce vs Recount Semantic Fix

**Date:** 2026-08-13
**Builds on:** BUG-SRSTOCK_IMPACT_ANALYSIS.md
**Risk:** MEDIUM
**Scope lock:**
  - Files WILL change: SubRecipeStockPanel.jsx · inventoryTransform.js · StockAuditPanel.jsx
  - Files will NOT touch: inventoryService.js · SubRecipeStockPage.jsx · InventoryTabBar.jsx · any hotspot file
...
```
Edit 1 → inventoryTransform.js        (foundation — no UI dep)
Edit 2 → StockAuditPanel.jsx          (depends on Edit 1 being correct)
Edit 3 → SubRecipeStockPanel.jsx      (depends on Edit 1 for correct payload)
```

---

## Edit 1 — `src/api/transforms/inventoryTransform.js`
**Lines 226–237 (addSubRecipeStock function)**

### Current (lines 226–237)
```js
...
**Verification:** `data.physicalQty != null` is true for 0, 5, 100 (any number), false only for `undefined` and `null`.

---

## Edit 2 — `src/components/inventory/StockAuditPanel.jsx`
**Lines 66–73 (sub-recipe branch inside handleSaveAll)**
...
**Why:** Stock Audit records what the user sees on the shelf (physical count). For sub-recipes that means:
- `physical_qty = shelf_count` (recount triggers wastage row for the diff)
- `quantity = 0` (no production credit — user is not producing, just counting)
- Before this fix: `quantity = shelf_count` → stock becomes shelf+shelf (double-counted)

---

## Edit 3 — `src/components/inventory/SubRecipeStockPanel.jsx`
**Full component rewrite — 324 lines → ~330 lines**

### Design
- Default mode: **Produce** (matches the label "Add production quantities")
- Toggle to: **Recount** (enter what you count on the shelf)
- Entries object cleared on mode switch
- Produce mode: 6 columns — no Drift, no Wastage Reason
- Recount mode: 8 columns — Drift + Wastage Reason shown when |drift| > 0.01
...
|---|---|---|---|---|
| 1 | `inventoryTransform.js` | `physical_qty` conditional on `physicalQty != null` | curl: `quantity:5, no physicalQty` → no `physical_qty` in request body; `physicalQty:8` → `physical_qty:8` appears | NO (Network tab) |
| 2 | `StockAuditPanel.jsx` | `quantity:0, physicalQty:shelf` for sub-recipes | Open Stock Audit, enter shelf count for churmura toe → network tab shows `quantity:0, physical_qty:X` | NO (Network tab) |
| 3a | `SubRecipeStockPanel.jsx` | Produce mode (default) sends `quantity:X`, no `physical_qty` | Enter 5 in Produce mode → network tab: `{quantity:5, unit:"gm"}` only | NO (Network tab) |
| 3b | `SubRecipeStockPanel.jsx` | Recount mode sends `quantity:0, physical_qty:shelf` | Switch to Recount, enter shelf=5 (current=10) → network: `{quantity:0, physical_qty:5}` | NO (Network tab) |
| 3c | `SubRecipeStockPanel.jsx` | Recount: shelf == book → skip row (no request fired) | Enter shelf=current exactly → Save → no HTTP request for that row | NO (Network tab) |
| 3d | `SubRecipeStockPanel.jsx` | Recount: drift shown as book−shelf (not shelf−book) | current=10, shelf=5 → drift shows "−5 loss" (positive diff = loss) | NO (Visual) |
| 3e | `SubRecipeStockPanel.jsx` | Produce mode: negative qty blocked (min=0.01) | Enter −5 → input constraint prevents; Save button disabled | NO (UI) |
| 3f | `SubRecipeStockPanel.jsx` | Mode switch clears entries | Enter qty in Produce → switch to Recount → fields empty | NO (UI) |
| 3g | `SubRecipeStockPanel.jsx` | Wastage reason required only in Recount when mismatch | Recount mode, mismatch, no reason → toast error | NO (UI) |
...
```
- [ ] registry.json: BUG-SRSTOCK → status: IMPLEMENTED
- [ ] BUG_TRACKER.md: row added for BUG-SRSTOCK with IMPLEMENTED status
- [ ] FILE_OWNERSHIP.md: add SubRecipeStockPanel.jsx + inventoryTransform.js + StockAuditPanel.jsx with BUG-SRSTOCK + date
- [ ] Code markers: // BUG-SRSTOCK in every modified file
- [ ] Compile check: webpack 0 new warnings
```

---

## Key Contract Rules Enforced by This Plan

| Rule | Enforcement |
|---|---|
| Never send `physical_qty: 0` as placeholder | `data.physicalQty != null` guard in transform (0 IS included if passed, but UI never passes 0 unless user actually enters 0) |
| Never default `physicalQty` to `quantity` | Produce mode payload has NO physicalQty field; Recount mode takes `entry.shelf` (a separate field) |
| Omit `physical_qty` when shelf == book | `Math.abs(currentQty - shelfCount) > 0.01` guard before calling service |
| `waste_reason` = text string, not ID | `wastageReasons.find(r => r.id === Number(entry.reasonId))?.reason` resolves text before sending |
| Produce mode: no drift UI | `isDrift` column rendered only when `!isProduce` |
| Recount mode: `quantity: 0` | Hardcoded `quantity: 0` in recount branch of handleSaveAll |

---

```
Planning complete: BUG-SRSTOCK
Stage: Impact Analysis (Gate 2) + Implementation Plan (Gate 3) — both in this session
Code reality: PARTIAL (wrong semantics)
Risk: MEDIUM
Files WILL change: SubRecipeStockPanel.jsx · inventoryTransform.js · StockAuditPanel.jsx
Files will NOT touch: inventoryService.js · SubRecipeStockPage.jsx · InventoryTabBar.jsx · all hotspot files
Owner decisions needed: NONE
Verification matrix: 9 checks, all manual (Network tab / UI visual)
