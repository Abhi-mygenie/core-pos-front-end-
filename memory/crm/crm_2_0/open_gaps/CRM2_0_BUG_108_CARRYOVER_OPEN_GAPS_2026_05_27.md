...
| **BC-02** | G-11 (P1) | Loyalty idempotency key | POS BE owns generation of `"order_{id}_loyalty"` per Owner 2026-05-25 Option B. Unverified end-to-end (order 869016 had `loyalty_idempotency_key = null`). | BACKEND_BLOCKED | POS BE team | Same R689 commit captures in CRM 2.0 CR-002 live-QA window can confirm BE now emits the key. |
