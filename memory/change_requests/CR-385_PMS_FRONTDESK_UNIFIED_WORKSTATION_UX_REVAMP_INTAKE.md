...
## Gate 2 — CLOSED 2026-09-17 (PLANNING session)
- IA: `impact/CR-385_IMPACT_ANALYSIS.md` (Rev 2.2). Backend brief: `backend_briefs/BACKEND_BRIEF_CR-385_2026-09-17.md`.
- **OD-385-12 (owner):** NEW beta page `/pms/front-desk-v2`; **no existing page touched** (App.js +2, Sidebar.jsx +1 "Front Desk (Beta)"). Check-In form, RoomTile, folio cards, checkout host are **copies**; cutover → **FU-385-C**.
- **OD-385-15 (owner):** folio + checkout on ONE screen (`FolioCheckoutPanel`, `CollectPaymentPanel` inline, no drawer/pop-up, Print Bill live, Print Folio waits CR-364-PRINT).
- Locked: OD-385-13 route, OD-385-14 sidebar item, OD-385-10 `?tab=`, OD-385-11 degrade Rooms tab only.
- **Process (owner): HARD GATE 2.6 — Impact Analysis must be re-validated after the final design (Gate 2.5) and explicitly closed before Gate 3 planning.**

## Addendum 2026-09-18 — SCOPE CHANGE (owner, during Gate 2.5)
- **New Booking is IN scope**: creating a direct/walk-in booking happens inside the workstation as an expand-in-place form (`+ New Booking` header / `Book Room` tile). `NewBookingPage.jsx` itself stays untouched (OD-385-12); the form is copied to `components/pms/frontdesk/NewBookingForm.jsx`. Walk-in = New Booking + "Check in now" (chains into the Check-In expansion). Details: `plans/CR-385_DESIGN_GAP_CHECKLIST.md` §11 NB1–NB6. Backend question BQ-385-06 (availability by date range).
- Design-round decisions also in scope: expand-in-place everywhere, Check Out = Folio, common guest row, global search, compact tab strip, M1–M6/M9/M11. Parked: FU-385-E..H.
- Impact Analysis must be re-validated for this at Hard Gate 2.6.
...
*2026-09-20: Backend build 3 DELIVERED · VERIFIED end-to-end (MASTER v1.6 §10). Mockup v2.28. Money contract D50.*
*2026-09-20 (later): N7/N8/N9 decided (D52) and shipped by backend as settings (D53, BQ-17/18 verified). **HARD GATE 2.6 CLOSED by owner** (D54) → Gate 3 open. O-4..O-8 answered; settings toggles = module M7 (OD-385-12 exception).*
