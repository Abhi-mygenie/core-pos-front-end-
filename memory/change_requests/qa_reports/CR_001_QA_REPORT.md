...
- Commands run:
  ...
  - `curl` on `https://bug-104-credit.preview.emergentagent.com/reports/audit` → **200 OK** (protected route falls back to login, app mounts cleanly)
...
- Route: `curl -I https://bug-104-credit.preview.emergentagent.com/reports/audit` → `200 OK` (protected route serves login; app mounts).
