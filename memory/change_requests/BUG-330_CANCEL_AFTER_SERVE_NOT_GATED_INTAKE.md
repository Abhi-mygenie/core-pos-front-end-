...
### OQs Resolved
- **Which flag:** Use `restaurant.cancellation.allowPostServeCancel` (from `cancle_post_serve` via profileTransform:222). The second field `allowPostServeCancel2` (from `allow_cancel_post_server`) is a separate API field — not controlled by the Settings toggle.
- **What counts as post-serve:** `item.status !== 'preparing'` = post-serve. Confirmed from `orderTransform.js:953`: `cancel_type: item.status === 'preparing' ? 'Pre-Serve' : 'Post-Serve'`

### Exact Fix Location
```js
// OrderEntry.jsx lines 322–324 (current):
const isItemCancelAllowed = useCallback((item) => {
...
const isItemCancelAllowed = useCallback((item) => {
  if (!canCancelItem) return false;
  // BUG-330: gate on allowPostServeCancel when item is post-serve
  if (item.status !== 'preparing' && !cancellation?.allowPostServeCancel) return false;
  return true;
}, [canCancelItem, cancellation?.allowPostServeCancel]);
```
