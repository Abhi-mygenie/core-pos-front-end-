...
| Ask | Status | Contract |
|---|---|---|
| B-365-01 OG-PMS-010 prerequisite | ✅ **RESOLVED** — `occupied_hk` new status handles stayover HK. Auto-task creation confirmed server-side. |
| B-365-02 HK task CRUD | ✅ **SHIPPED** | `GET/POST /aiosell/hk-tasks` + `PATCH /aiosell/hk-tasks/{id}` + `POST /aiosell/hk-assign/{tableId}` |
| B-365-03 Auto-task on room→hk | ✅ **CONFIRMED** — server auto-creates task when room PATCH → `hk` or `ooo` |
| B-365-04 Employee list path | ✅ **ANSWERED** → `GET /employee/employees-list`, filter HK role client-side |
| B-365-05 Checklist templates | ✅ **SHIPPED** | `GET/PUT /aiosell/hk-checklist/template` + `GET /aiosell/hk-checklist/{tableId}` + `POST .../complete` |
| B-365-06 Sockets/FCM | ⏳ **DEFERRED** — poll/manual refresh for now. Non-blocker. |
| B-365-07 Task complete → room state | ✅ **ANSWERED** → server-side atomic. Do NOT PATCH `available` separately. |

### New scope from this reply (owner decision needed):
- **Laundry module** — backend shipped full laundry sub-module: `GET/PUT /aiosell/laundry/settings`, `GET /aiosell/laundry/queue?status=`, `GET /aiosell/laundry/stock`, `POST /aiosell/laundry/process`. NOT in original CR-365 scope.
  - **OD-365-NEW-01:** Include laundry in CR-365 scope? → a) YES — expand CR-365 · b) NO — register as CR-365-B

### New prerequisite:
- **BUG-397** (`occupied_hk` not handled in roomStatusTransform.js / RoomStatusPage.jsx) must be planned before or alongside CR-365 Gate 2.

**Status: BACKEND-BLOCKED → UNBLOCKED (2026-09-13). Ready for Gate 2 pending OD-365-NEW-01 laundry decision.**
