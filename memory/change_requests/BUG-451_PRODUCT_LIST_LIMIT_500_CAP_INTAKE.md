...
| **OD-451-03** (Gate 2 D1) | One constant `PAGINATION.DEFAULT_LIMIT = 2000` at all 3 call sites vs 3 literals | **LOCKED 2026-09-24 — CONSTANT** (owner "ok") |
| **OD-451-04** (Gate 2 D2) | Dead `productService.getAllProducts()` (0 callers) | **LOCKED 2026-09-24 — DELETE** (owner "ok") |
| **R11 probe** (Gate 2 Q3) | Backend accepts `limit=2000`? | **DONE 2026-09-24 — YES.** Owner supplied the yabyum owner alias (credential not stored, R20). Restaurant has 561 products: `limit=500` → 500 returned (bug reproduced); `limit=1000/2000/5000` → 561/561, HTTP 200, no cap. Evidence `evidence/BUG-451/probe_limit_{500,1000,2000,5000}.json` (token masked). |

**GATE 2 CLOSED — 2026-09-24 (owner).** IA `impact/BUG-451_IMPACT_ANALYSIS.md` patched. Gate 3 NOT started (owner: "do not start gate 3").
