...
$ EXT_HASH=$(curl -s https://bug-104-credit.preview.emergentagent.com/static/js/bundle.js | md5sum | awk '{print $1}')
...
```bash
$ curl -s https://bug-104-credit.preview.emergentagent.com/static/js/bundle.js | grep -c "GAP-A\|tabIsCustomerSelected"
...
1. In the browser, click the **"Wake up servers"** button on the "Frontend Preview Only" overlay at `https://bug-104-credit.preview.emergentagent.com`.
...
3. **If the URL is the Emergent preview pod (`https://bug-104-credit.preview.emergentagent.com`):**
