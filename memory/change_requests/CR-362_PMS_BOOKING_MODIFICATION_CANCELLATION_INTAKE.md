...
| OD | Question | Options | **DECISION** |
|---|---|---|---|
| OD-362-01 | Refund policy on cancel: advance received (cash/UPI) — no refund API exists except Razorpay online | a) Display "refund manually" note only · b) Record refund via `pos/room-payment` negative? (needs backend) · c) No refunds in v1 | **LOCKED A — FE shows "Refund ₹{advance} to guest manually" note. Backend handles status + inventory. Physical refund is staff's responsibility. (2026-09-13)** |
| OD-362-02 | Allow local cancel of OTA bookings? | a) No — show "cancel in OTA extranet" · b) Yes, marked local-only · c) Yes + notify CM (needs B-362-01 notify_cm) | **LOCKED B — Local cancel allowed for all booking types. `notify_cm: true` pushes inventory back to AioSell. OTA booking on guest side NOT auto-cancelled (backend inbound/webhook only). FE shows extranet warning: "To notify the guest, also cancel on Booking.com / GoMMT extranet." (2026-09-13)** |
| OD-362-03 | Extend-stay pricing | a) Existing nightly rate × extra nights (from `amount_after_tax / nights`) · b) Live `fetch-rates` for the new dates · c) Manual entry with reason | **LOCKED B — Live `fetch-rates` endpoint. FE fetches rates for new dates, shows rate to staff, staff confirms before saving. (2026-09-13)** |
| OD-362-04 | Who may cancel/modify — permission key? | owner/manager only vs all POS users | **LOCKED — No role gating in v1. All POS users can cancel/modify. Gate added in future sprint. (2026-09-13)** |
| OD-362-05 | Cancellation reasons — reuse `CANCELLATION_REASONS` endpoint list or PMS-specific list? | reuse / new | **LOCKED — Reuse existing `CANCELLATION_REASONS` endpoint. PMS-specific list deferred. (2026-09-13)** |
| OD-362-06 | Modify dates on `in_house` (shorten stay = early checkout) → is that just Checkout via PmsCheckoutDrawer? | yes / needs refund logic (ties to enhancement 13 in handover list) | **LOCKED — No new early checkout feature in CR-362 scope. Early checkout = existing PmsCheckoutDrawer. `room-extend-stay` is extend-only. (2026-09-13)** |
...
- [x] **Gate 1 CLOSED — All 6 ODs LOCKED (2026-09-13)**
- [ ] Gate 2 — Impact Analysis → **READY TO START**
...
## Backend Reply — 2026-09-13 ✅ ALL BLOCKERS RESOLVED

**Source:** `evidence/INV-BE-REPLY-2026-09-13/be_reply_raw_2026_09_13.md`

| Ask | Status | Contract |
|---|---|---|
| B-362-01 Cancel endpoint | ✅ **SHIPPED** | `POST /api/v2/vendoremployee/aiosell/local-reservations/{id}/cancel` · body: `{reason, cancelled_by, notify_cm}` · 409 if checked in |
| B-362-02 Modify endpoint | ✅ **SHIPPED** | `PATCH /api/v2/vendoremployee/aiosell/local-reservations/{id}` · 409 if checked in |
| B-362-03 Extend stay endpoint | ✅ **SHIPPED** | `POST /api/v2/vendoremployee/pos/room-extend-stay` · body: `{order_id, new_checkout_date, new_room_price?, reason}` |
| B-362-04 view=cancelled | ✅ **SHIPPED** | Both `?status=cancelled` and `?view=cancelled` now work |
| B-362-05 Sandbox seed | ⏳ NOT YET — non-blocking for FE Gate 2 |
| B-362-06 OTA push on Direct cancel | ✅ **ANSWERED** → OD-E2-02 LOCKED: local DB + inventory only. No Aiosell push. |

**Status: BACKEND-BLOCKED → UNBLOCKED (2026-09-13). Ready for Gate 2.**

### ODs still needing owner answers before Gate 2 completes:
OD-362-01 (refund policy), OD-362-02 (OTA cancel → local-only confirmed now: LOCKED A), OD-362-03 (extend-stay pricing), OD-362-04 (permissions), OD-362-05 (cancellation reasons), OD-362-06 (early checkout path)
