...
| Part | Bucket | Count |
|---|---|---|
| A | Rejected rules — code bug, awaiting fix | 2 |
| B | Approved-with-amendment — awaiting code alignment & verification | 15 |
| C | Deferred rules — awaiting owner clarification | 3 |
| D | Pending runtime / live-print / backend verification gates | 9 |
| E | Linked implementation bugs (cross-reference) | 12 |

---

## Part A — Rejected Rules (Code Fix Required)

These rules were documented incorrectly. The owner has rejected the original wording and stated the correct behaviour. Frontend code must be fixed before the corrected rule can be frozen.

### A1. TIP-003 — Tip on Takeaway / Delivery
...
- **What's blocking freeze:** Implementation fix (see BUG-001) and a re-test confirming tip = ₹0 in Takeaway/Delivery payloads.
- **Known files:** `orderTransform.js:632,647`, `CollectPaymentPanel.jsx`.

### A2. ROUND-001 — Conditional Ceiling/Floor Round-off
...
- **What's blocking freeze:** Implementation fix (see BUG-002) and a re-test confirming both ₹105.05 and ₹105.15 round to ₹106.
- **Known files:** `orderTransform.js:657-661`, `CollectPaymentPanel.jsx:579-583`.
...
### C1. TOTALS-004 — Room Order Grand Total Composition
- **Why deferred:** For room orders, grand total (and `payment_amount` / `grant_amount`) likely includes room balance, but this needs backend + runtime verification.
- **What's needed to unblock:** Capture runtime payload for a room order and confirm whether `payment_amount` includes room balance. Verify with backend.

### C2. PAY-006 — Transfer to Room Payload
- **Why deferred:** The Transfer to Room payload content is unclear; owner could not confirm without a runtime check.
- **What's needed to unblock:** Capture the actual Transfer to Room API payload at runtime. Owner to review and confirm correct fields.
...
| 5 | TOTALS-004 | Backend + runtime | Capture room order payload to confirm `payment_amount` and `grant_amount` include room balance. Confirm behaviour with backend. |
...
| 8 | PAY-007 | Backend confirmation | Confirm with backend team: is the `'sucess'` (misspelled) status string permanent? If backend plans to fix the typo, frontend must coordinate to avoid silent PayLater breakage. *(Note: PAY-007 is also in the frozen baseline as a current-state freeze; this gate tracks future-coordination only.)* |
| 9 | POLL-003 | Backend confirmation | If explicit status-8/9 filter code exists in the polling hook → confirm backend enforces the same exclusion (i.e., these orders are never returned in polling response). |
