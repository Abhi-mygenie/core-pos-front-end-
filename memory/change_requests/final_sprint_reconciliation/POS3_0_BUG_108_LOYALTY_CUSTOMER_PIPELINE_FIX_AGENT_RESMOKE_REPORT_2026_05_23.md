...
Browser attempt log:
- Navigated to `https://react-pos-build-11.preview.emergentagent.com/` → login page renders.
...
- Even if login succeeded, CRM data calls (`https://mygenie-crm-build-3.preview.emergentagent.com/api/pos/...`) require the external backend to be alive; the "Wake up servers" pill at page bottom is the app's own indicator that external services are not currently reachable.
