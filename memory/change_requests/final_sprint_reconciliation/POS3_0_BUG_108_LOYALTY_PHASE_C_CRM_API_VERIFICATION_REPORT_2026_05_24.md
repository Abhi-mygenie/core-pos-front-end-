...
CRM backend code is NOT available locally — CRM is a remote service at `https://crm-may-branch.preview.emergentagent.com/api`. All verification performed via live API calls.
...
CRM backend is a remote service — no local source code. Verification performed via live HTTP calls against `https://crm-may-branch.preview.emergentagent.com/api` with valid `X-API-Key` from restaurant 689 (kunafamahal) login.
