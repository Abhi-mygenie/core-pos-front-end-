...
| `crmAxios` config | `baseURL = REACT_APP_CRM_BASE_URL`, currently `https://mongo-crm-preview-1.preview.emergentagent.com/api`. Header: `X-API-Key` from login response `crm_token`. Paths used today: `/pos/customers`, `/pos/customer-lookup`, etc. (relative — baseURL prefix supplies `/api`). |
