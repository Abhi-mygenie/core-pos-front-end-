...
### API Probe — insights-discounts (confirmed 2026-08-19)
Backend has added `orders_table` key to `POST /api/v2/vendoremployee/report/insights-discounts`:
```
New response shape:
  ...
  coupons: []
  orders_table: []  ← NEW — per-order rows with discount_for field
```
`orders_table` rows contain `discount_for` (the reason text from CR-137).
Currently empty on test account (no discounts applied in test data — confirmed zeros throughout).

### Confirmed via Orders Beta endpoint
`daily-order-report-details-combined` order rows have `discount_for: null` field — same key, same data.

### Fix Path
Add `orders_table` section to `DiscountReportMockup.jsx` — table showing per-order discount reasons.
Show `discount_for` value or "—" when null. 1 file, ~20-25 lines.
