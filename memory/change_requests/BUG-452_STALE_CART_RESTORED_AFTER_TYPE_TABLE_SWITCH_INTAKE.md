...
| OD-452-02 | Fix location: `DashboardPage.jsx:2070` handler vs `OrderEntry.jsx:383–386 / 504–508`. **Note:** INV Option A (clear inside `handleOrderTypeChange` *before* `setOrderEntryType`) does **not** work — the OrderEntry effect fires after the type change and re-writes the old key. With Option B the natural place is the OrderEntry effect (skip the save + `setCartItems([])` on real switch) — R5. | OPEN (Planning) |
| OD-452-03 (new) | Occupied table → other table: items already **placed** (`placed: true`) belong to the source order and must never be cleared from that order or carried — only the unplaced draft is cleared. Planning to confirm the existing guard. | OPEN (Planning) |
