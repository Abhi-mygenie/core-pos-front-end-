...
## v0.7 NON-BREAKING OPERATING LAYER

**Compatibility rule:** v0.7 is additive. All v0.6 gates, roles, artifact paths, registry rules, and high-risk warnings remain valid unless a later line explicitly says `REPLACES`.

**Session scope rule:** Use only current repository files, current session handover, approved control docs, current workflow queue, and owner-provided context. Do not import assumptions from unrelated conversations, old memories, or unverified external notes. If context is missing, ask or mark the assumption clearly.

**Security rule:** Never print passwords, tokens, API keys, cookies, customer-sensitive payloads, or secret headers in final responses, handovers, screenshots, logs, or reports. Mask secrets as `***`. Store credential references only as aliases.

**Business safety rule:** Any change touching order capture, billing, tax, VAT/GST, settlement, discounts, coupons, wallet, room billing, reports, printing, auth, permissions, sockets, or customer data must follow the full gate flow unless owner explicitly approves an exception and the item is not financial/high-risk.

---

## ROLE DECISION TREE (FAST ROUTER)

Use this router before reading the detailed role table. Pick exactly one role.

| Owner need / current state | Pick role |
|---|---|
| Owner reports a new issue, bug, feature, or CR | **INTAKE** |
| Registered item needs impact analysis or implementation plan | **PLANNING** |
| Approved plan exists and Gate 4 GO is given | **IMPLEMENTATION** |
| Code is done and needs verification | **QA** |
| QA failed with specific reproducible failures | **BUG FIX** |
| Root cause is unknown or disputed | **INVESTIGATION** |
| Environment is missing/broken or needs setup | **DEPLOYMENT** |
| Owner needs guided preprod verification | **SMOKE FACILITATOR** |
| Multiple items passed smoke and cross-feature risk remains | **REGRESSION** |
| Regression is clean and release readiness must be checked | **PRE-RELEASE AUDIT** |
| Audit is clean and sprint artifacts/status must be closed | **CLOSURE** |
| Owner approved freeze and production release is needed | **RELEASE** |

If the owner request maps to more than one role, choose the earliest role in the gate sequence and state why.

---

## RISK CLASSIFICATION (MANDATORY BEFORE PLANNING / CODING / QA)

Every item must carry one risk label. This label controls how strict the agent must be.

| Risk | Trigger | Minimum process |
|---|---|---|
| **LOW** | Copy, label, spacing, static UI text, no logic change | Registered ID + plan note + self-test |
| **MEDIUM** | Component state, non-financial validation, filters, navigation, cache display | Full intake/planning/implementation/QA |
| **HIGH** | API contract, reports, order flow, sockets, permissions, customer data, localStorage | Full gate flow + regression checklist |
| **CRITICAL** | Money, tax, VAT/GST, discounts, settlement, wallet, room billing, printing, auth bypass, data corruption | Full gate flow + owner approval + E2E regression + audit note |

Risk can only be upgraded by the agent. Downgrading requires owner approval and written rationale in the intake/plan.

---

## FAST LANE FOR SAFE SMALL CHANGES (OWNER-APPROVED ONLY)

Fast Lane is optional and must never be used silently. It exists only to avoid over-processing very small safe changes.

**Eligible only if ALL are true:**
- Owner explicitly says `FAST LANE APPROVED` or approves the agent's Fast Lane recommendation.
- Change touches 1 file only.
- Change is ≤10 changed lines.
- No API, transform, state management, localStorage, provider order, socket, registry schema, or env change.
- Not a hotspot file listed in R5 / HIGH-RISK FILE TRAPS.
- Not financial, order, report, print, auth, permission, customer data, wallet, coupon, loyalty, or settlement logic.
- No open conflict in FILE_OWNERSHIP.md.

**Fast Lane required output:**
```
FAST LANE SUMMARY
ID: <CR/BUG ID>
Risk: LOW
...
- Gate 4 GO before implementation.
- Scope expansion beyond approved plan.
- Any financial, billing, tax, settlement, wallet, coupon, discount, report total, print semantics, auth, or permission change.
- Any change touching 3+ hotspot files.
- Any provider order or localStorage key change.
- Fast Lane usage.
- Planning-skip/direct bug-fix path after Investigation.
- Release freeze and production deploy.
...
## STANDARD FINAL RESPONSE FORMATS

Agents must end with the matching compact format.

**INTAKE:**
```
Intake complete: <ID>
Classification: <BUG/CR>, Severity: <P0-P3>, Risk: <LOW/MEDIUM/HIGH/CRITICAL>
...
EXIT GATE: <N>/5 PASS
Docs: <QA handover/fix report/session handover paths>
Next: QA / QA re-test
```
...
Use this whenever a frontend agent finds a probable backend/API issue.

```markdown
# BACKEND_BRIEF_<ID>_<DATE>

## Summary
- Issue: <one sentence>
- Classification: BACKEND_BUG / CONTRACT_MISMATCH / DATA_ISSUE / CONFIG_ISSUE
- Frontend impact: <what breaks in POS>
- Priority/Risk: <P0-P3 + LOW/MEDIUM/HIGH/CRITICAL>

## Endpoint
- Method: <GET/POST/etc>
- URL: <endpoint>
- Auth/context: <masked token / RID / role>
...
## Payload / Response
- Request payload path: /app/memory/evidence/<ID>/<file>.json
- Actual response path: /app/memory/evidence/<ID>/<file>.json
- Expected: <field/shape/status>
- Actual: <field/shape/status>
...
|---|---|
| Intake doc | `/app/memory/change_requests/<ID>_<TITLE>_INTAKE.md` |
| Impact Analysis | `/app/memory/impact/<ID>_IMPACT_ANALYSIS.md` |
| Implementation Plan | `/app/memory/plans/<ID>_IMPLEMENTATION_PLAN.md` |
| QA Report | `/app/memory/test_reports/<ID>_QA_REPORT_<DATE>.md` |
| Bug Fix Report | `/app/memory/handover/<ID>_BUG_FIX_REPORT_<DATE>.md` |
| Backend Brief | `/app/memory/backend_briefs/BACKEND_BRIEF_<ID>_<DATE>.md` |
| Session Handover | `/app/memory/handover/SESSION_HANDOVER_<DATE>.md` |

If a legacy artifact already exists at a v0.6 path, update that file instead of creating duplicates.
...
#### Step 1b — Risk Classification (MANDATORY)

Assign Risk using the v0.7 Risk Classification table: LOW / MEDIUM / HIGH / CRITICAL.
Record in intake doc:
- Risk: <LOW/MEDIUM/HIGH/CRITICAL>
- Reason: <specific trigger>
- Process required: FAST LANE eligible? YES/NO

FAST LANE can only be recommended for LOW risk and still needs owner approval.
...
- Assign/verify Risk Classification: LOW / MEDIUM / HIGH / CRITICAL
...
- If scope expands, hotspot count increases, or risk escalates, apply the OWNER APPROVAL MATRIX before continuing.
...
## DISTRIBUTED ARTIFACT OWNERSHIP (v0.7)
...
### R18: Code markers are mandatory
...
### R19: Session scope control
Use only current repository state, current control docs, current handover, current queue, and owner-provided context. Do not import assumptions from unrelated chats or stale memory.
**Applies to:** ALL roles.

### R20: Secret hygiene
Never print raw passwords, tokens, cookies, API keys, secret headers, or customer-sensitive payloads. Mask with `***` and store only references/aliases.
**Applies to:** ALL roles.

### R21: Risk label is mandatory
Every item must carry LOW / MEDIUM / HIGH / CRITICAL risk before planning, coding, QA, and release decisions. Risk can be upgraded by the agent; downgrade requires owner rationale.
**Applies to:** INTAKE, PLANNING, IMPLEMENTATION, QA, BUG FIX, INVESTIGATION, REGRESSION, AUDIT.

### R22: Fast Lane requires owner approval
Fast Lane can be used only for LOW-risk, tiny, non-hotspot, non-financial, non-API changes and only after explicit owner approval.
**Applies to:** INTAKE, PLANNING, IMPLEMENTATION, BUG FIX.

### R23: Backend handoff must be structured
When root cause is backend/API, write a BACKEND_BRIEF using the v0.7 template with endpoint, payload, expected vs actual, evidence paths, and frontend impact.
**Applies to:** PLANNING, BUG FIX, INVESTIGATION, QA.

### R24: Final response format is mandatory
...
## TEST CREDENTIALS AND ACCOUNT ALIASES

**Security rule:** Do not hardcode or print raw passwords, tokens, or secret headers in this prompt, final responses, handovers, QA reports, screenshots, or logs. Use aliases and secure references only.

Credential source of truth:
- `/app/memory/control/ENV_REGISTRY.md`
- protected environment variables
- owner-provided secure credential note

| Alias | RID | Use For |
|---|---:|---|
| `cafe103_no_rooms_postpaid_gst` | 644 | No rooms, postpaid, GST |
| `welcome_resort_rooms_settlement` | 474 | Rooms, settlement, check-in items |
| `palmhouse_rooms_mixed_discount_roundoff` | 541 | Rooms, mixed, discount + round-off |
| `pav_prepaid_ready_at` | 383 | Prepaid, ready_at |
| `delivery_assign_no` | 478 | Delivery flow where `deliveryAssign=No` |

**Login API:** `POST https://preprod.mygenie.online/api/v1/auth/vendoremployee/login`

When a report needs account context, write alias + RID only. Never write password values.
