...
- dynamic-price (₹1) catalog item handling during add-to-cart
### Future change rules
...
- Catalog items priced exactly at ₹1 are treated as dynamic-price items in the ordering flow. `OrderEntry.addToCart` must intercept these items and require the cashier to enter the actual runtime price via the dynamic-price modal before the item is added to the cart. The cashier-entered price becomes the authoritative unit price and flows through cart, totals, payment, and print without any transform/contract change. The dynamic-price item carries `_isDynamicPrice: true`. No backend/catalog mutation occurs.
### Open decisions, if any
- room billing/print lifecycle policy remains deferred until the next room billing / room print related change.

### Changelog
- 2026-04 (BUG-035): Documented dynamic-price (₹1) ordering rule. Frontend-only behavior; cart price is authoritative for downstream payment/print flows.
