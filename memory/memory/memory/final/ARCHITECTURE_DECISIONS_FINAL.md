...
### Rule API-07 — Catalog price ₹1 means dynamic-price item at ordering time
Catalog items with price exactly ₹1 are treated as dynamic-price items by the frontend ordering flow:
- `OrderEntry.addToCart` intercepts items where `Number(item.price) === 1` and opens a price-entry modal instead of adding the item to the cart at ₹1
- the cashier must enter a runtime price > 0; invalid entries (0, blank, negative, NaN) are rejected with an inline error
- the cart item is then created with `price` set to the cashier-entered value and a `_isDynamicPrice: true` flag
- `orderTransform.buildCartItem`, `CollectPaymentPanel`, `CartPanel` and the print payload builders are unchanged — they consume `item.price` from the cart item, which is now the cashier-entered value
- non-₹1 items are unaffected; customizable items continue to route through `ItemCustomizationModal`; the custom-item creation flow (`AddCustomItemModal`) remains a separate path
- this is a frontend-only convention; no backend/catalog mutation occurs and no API/socket payload contract changes
...
10. Dynamic-price (₹1) interception in `OrderEntry.addToCart` and the price-entry modal
