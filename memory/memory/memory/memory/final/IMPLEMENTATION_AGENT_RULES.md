...
2. **REGISTRATION GATE (POS 4.0+):** Verify the item has a registered ID in `registry.json`. If not, run the Intake Flow (`/app/memory/control/INTAKE_WORKFLOW.md`) first. See `/app/memory/control/REGISTRATION_GATE_POLICY.md`.
3. Map the request to affected module(s) before touching code.
4. Check unresolved owner decisions before assuming behavior.
5. Use code as current implementation truth when docs and code differ.
6. Do not implement without impact analysis.
7. Do not casually change financial, room, socket, bootstrap, or localStorage behavior.
8. Strict rule: no implementation without clear affected-module mapping.
...
## Closure Gate — 6-Artifact Rule (added 2026-05-12)

A bug shall not be marked Closed in `BUG_TEMPLATE.md` until all six artifacts
exist on disk under `/app/memory/bugs/`:
...
The tracker keeper must verify all six exist before flipping the row.

For legacy bugs predating this rule, see `BUG_LEGACY_BULK_CLOSURE_2026_05_12.md`
for the one-time grandfathering exception.
