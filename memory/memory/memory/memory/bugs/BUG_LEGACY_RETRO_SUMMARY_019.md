# BUG Legacy Retro Summary — BUG-019
...
Scan / Re-engaged Delivery Orders — Delivery Charge Not Mapped to Collect Bill (cashier under-collects)

## 3. Original April-2026 fix narrative (from master tracker)

FIXED (Apr-2026) — `orderFinancials` now carries `deliveryCharge` across 6 call sites; `CollectPaymentPanel` seeds `deliveryChargeInput` from prop and auto-locks field (`readOnly`) when backend-seeded (> 0). Fresh in-POS delivery orders remain editable.
...
`components/order-entry/OrderEntry.jsx`, `components/order-entry/CollectPaymentPanel.jsx`
...
- Master tracker: `/app/memory/BUG_TEMPLATE.md` (row for BUG-019)
...
— End of BUG-019 Legacy Retro Summary —
