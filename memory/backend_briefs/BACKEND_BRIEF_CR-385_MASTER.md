...
```
Version:      2.5 · 2026-09-22 · **v2.5: BQ-385-28 `upgrade_reason` on `charge` (Phase 4 GO, Q3 b) · BQ-385-29 charge-only room checkout body (owner D87 retire path) — both non-blocking** (v2.4 2026-09-22) · **v2.4: BQ-385-27 folio-level balance_due (owner D85; retires the FE sum in M5)** (v2.2 2026-09-22) (v2.1 2026-09-21) · **v2.3: BQ-385-26 (`direct-reservation` incomplete body → 200 `skipped:true`, should be 422; Phase 2 entry probe, not blocking)** (v1.0 2026-09-19) · **v2.1: BQ-385-23/24 (Phase 1 QA asks, not blocking)** · **v2.0: BQ-385-22 perf/aggregation asks (not blocking)** · **v1.9: N11 FIXED + verified (per-night GST slab, `nights_detail[]`) · new D14 (P2, calendar extend response `charge` stale by the same-call payment) · BQ-385-19 (P2 ask: `nights_detail` on LR list)** · supersedes BACKEND_BRIEF_CR-385_2026-09-17.md, _ADDENDUM_2026-09-18.md, _PHASE1_CONTRACT_2026-09-19.md (kept as history)
...
| **BQ-385-26** | — | **`direct-reservation` incomplete body → HTTP 200 `{status:true, skipped:true, data:null}`** instead of 422 (probe `{rooms:[]}`, 2026-09-22; LR count 112 → 112, nothing created). Ask: 422 + `{status:false, message}`. Evidence `evidence/CR-385/probes_2026_09_22_p2_entry/s7_direct_reservation_skipped.json`. | P3 | 2 | **OPEN 2026-09-22 (P2 entry probe, not blocking)** | Error UX only — FE M1 hard-fails on `skipped` / missing `data.reservation` (unit-tested) | — |
| **BQ-385-27** | — | **Expose a folio-level `balance_due`** per in-house reservation row (room ledger + room-service orders + transferred F&B, GST-inclusive, after advances) on `local-reservations` (e.g. `charge.folio_balance_due`) — so the Front Desk balance cell stops summing on the FE (D85 temporary exception; CR-358 `getInHouseGuests` fans out one folio call per stay). Round-off: also return `round_off` if the server applies the CR-170 rule, else FE keeps applying `applyGrandTotalRoundOff`. | P2 | 2 | **OPEN 2026-09-22 (owner D85)** | FE sum via `getInHouseGuests` (M5) until delivered | — |
| **BQ-385-28** | — | **Expose `upgrade_reason`** on `charge` (LR rows + check-in response) next to `upgrade_amount`. Today the reason lives only as the folio order line `"Room upgrade: <reason>"` (`get-single-order-new` orderDetails), which the Front Desk Bill parses to label the ROOM upgrade line (Q3 a). | P3 | 1 | **OPEN 2026-09-22 (Phase 4 GO, Q3 b)** | FE reads the folio line | — |
| **BQ-385-29** | — | **Charge-only room checkout body**: accept a room settlement where the FE sends intent + tendered amount only (`order_id`, `payment{method, amount, reference}` or split legs) and the server prices the room leg from the reservation `charge` (nights, upgrade, SGST/CGST, advances) + posted F&B — so the Front Desk Bill can stop sending the POS `collectBillExisting` body with `room_gst_tax` passthrough (owner D87 explicit D50 exception). | P2 | 3 | **OPEN 2026-09-22 (owner D87 retire path)** | unchanged POS body via the shared `CollectPaymentPanel` host (D87) | — |
...
## BQ-385-26 — `direct-reservation` accepts an incomplete body with HTTP 200 `skipped:true` (P3 · CONTRACT · not blocking)

### Endpoint
`POST /api/v2/vendoremployee/aiosell/direct-reservation`

### Reproduction (2026-09-22, sandbox-pms, read-only — nothing created)
Body `{"rooms": []}` (no guest, no dates) → **HTTP 200** `{"status": true, "skipped": true, "message": "Direct reservation skipped: incomplete stay dates.", "data": null}`. `local-reservations` count 112 / max id 235 before **and** after (`s6_lr_before.json`, `s7_direct_reservation_skipped.json`, `s8_lr_after.json`).

### Ask
Return **422** with `{"status": false, "message": "<field> is required"}` (same shape as the verified `"no rate configured"` 422) for any body that cannot create a reservation. A 200 with `status:true` for a non-created booking is a success-shaped failure: any client keying on the status code will toast "Booking saved" for nothing.

### Frontend behaviour (M1, regardless of the answer)
`frontDeskService.createBooking` → `NewBookingForm` treats `body.skipped === true || !body.data?.reservation` as a **hard `booking-error`** ("Booking not created — <server message>"), no toast, no refetch, form stays open. Unit-tested (`tests/cr385/phase2.cr385.test.jsx`, fixture `direct_reservation_skipped.json`).
...
| 2026-09-22 | **v2.5** — BQ-385-28 (`upgrade_reason` on charge) + BQ-385-29 (charge-only room checkout body, D87 retire path) filed at Phase 4 GO; both non-blocking. |
| 2026-09-22 | **v2.4** — BQ-385-27 (folio-level `balance_due` on LR rows, owner D85) filed at Phase 3 GO. |
| 2026-09-22 | **v2.3** — BQ-385-26 (`direct-reservation` with an incomplete body answers HTTP 200 `{status:true, skipped:true, data:null}` instead of 422) filed from the Phase 2 Entry Verification probe (`evidence/CR-385/probes_2026_09_22_p2_entry/`). FE M1 hard-fails on `skipped`/missing `data.reservation` regardless. |
