...
> ⚠ **2026-09-02 UPDATE (handover_3.md):** Auto-HK fires server-side on RM checkout via `order-bill-payment` V2 (`auto_hk_on_rm_checkout=true`). However, **HK does NOT push inventory**. Workflow: checkout → auto-HK (no push) → staff clears HK via `PATCH available` → inventory push fires. Full automatic inventory release on checkout is **NOT confirmed**. **Phase 5 `aiosell_sync_logs` probe still required.**
...
> ✅ **RESOLVED 2026-09-02 (handover_3.md):** OD-07 confirmed — backend field, persists across devices. Both endpoints delivered:
> - `GET /api/v2/vendoremployee/aiosell/room-status-board` — returns ALL RM rooms with server-computed `display_status` (5 states: available/occupied/booked/hk/ooo). Includes `guest{}` + `reservation{}` blocks. Exposes `auto_hk_on_rm_checkout` setting.
> - `PATCH /api/v2/vendoremployee/aiosell/room-status/{table_id}` — accepts `hk/ooo/available`. 422 if room occupied. `inventory_push_warning` in response if AIOSELL push fails.
> - **Migration required:** `php artisan migrate --path=database/migrations/2026_09_02_140000_add_room_operational_status.php` (adds `room_operational_status`, `room_operational_status_at`, `room_operational_status_by` on `restaurant_table`; `auto_hk_on_rm_checkout` on `restaurant_settings`).
> - **New FE constraint:** 422 when marking occupied room HK/OOO — FE must show error toast. `inventory_push_warning` → secondary warning toast.
...
| B-09 | HK/OOO state storage + room-status-board | ✅ **DELIVERED 2026-09-02** | NEW ENDPOINT + TABLE | `GET /aiosell/room-status-board` + `PATCH /aiosell/room-status/{table_id}` — full spec in handover_3.md. OD-07 resolved: backend field. Migration confirmed. |
