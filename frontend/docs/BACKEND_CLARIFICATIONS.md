...
> Status: Phase 3A — Socket integration done. Scan order confirm/cancel implemented.
...
## Socket Integration Clarifications (Phase 3)

### B22: `order_amount` field does NOT reflect cumulative total — BACKEND FIX NEEDED
**Severity:** HIGH
**Endpoint:** `POST /api/v2/vendoremployee/get-single-order-new`

**Observation:** The `order_amount` field in the API response does NOT equal the sum of all `orderDetails` items. After adding items to an order, `order_amount` reflects only the latest KOT/update amount.

**Evidence (order 730196):**

| Field | Value |
|---|---|
| `order_amount` | 180 |
| Item 1: Hot Coffee | 70 x 1 = 70 |
| Item 2: Fried Momos | 90 x 1 = 90 |
| Item 3: Margherita Pizza | 180 x 1 = 180 |
| **Expected total** | **340** |
| **API returns** | **180** (= price of last added item only) |

**Impact:** Table card displays wrong amount after order updates. The "Collect Bill" button computes total from items (correct), but table card uses `order_amount` (wrong).

**Recommended Fix:** Backend should return cumulative total in `order_amount`.
**Status:** OPEN — Waiting for backend fix.

### B23: Valid `order_status` values for `PUT /api/v2/vendoremployee/food-status-update`
**Discovered during:** Scan order confirm implementation

The `order_status` field in the `food-status-update` payload only accepts specific string values. Sending invalid values returns `"The selected order status is invalid."` error.

**Valid values:**
| Value | Meaning | f_order_status transition |
|---|---|---|
| `'preparing'` | Accept/confirm order | 7 → 1 |
| `'ready'` | Food ready | 1 → 2 |
| `'served'` | Food served | 2 → 5 |
| `'cancelled'` | Cancel order/item | any → 3 |

**Invalid values tested:**
- `'confirmed'` — Returns error `"The selected order status is invalid."`

**`cancel_type` field:**
- For confirm (accept): Pass `null`
- For cancel: Pass `'Pre-Serve'` or `'Post-Serve'`

**Status:** RESOLVED — Correct values documented above.

### B24: Scan order `f_order_status` is 7 (pending), not 0
**Observation:** Scan-and-order (QR code) orders arrive with `f_order_status: 7` which maps to `pending` → `yetToConfirm` on table card. Previously documented as `f_order_status: 0` — this was incorrect.
**Status:** CORRECTED

### B25: Socket event payload structure
**Observation:** Socket events arrive as arrays, not objects:
```
[eventName, orderId/tableId, restaurantId, status, payload?]
```
- Index 0: Event name (e.g., `"update-order"`)
- Index 1: Order ID or Table ID (numeric)
- Index 2: Restaurant ID
- Index 3: f_order_status (for orders) or table status string (for tables)
- Index 4: Full payload (only for `new-order`)

**Status:** CONFIRMED through implementation

### B26: Two socket events fire on order update
**Observation:** When an order is updated (items added/modified), the backend fires TWO events:
1. `update-table` (immediate, contains table status string like `"engage"`)
2. `update-order` (contains order ID, requires API call to get fresh data)

Both arrive on different channels (`update_table_{id}` vs `new_order_{id}`).
**Status:** CONFIRMED through testing

### B27: Socket channels are dynamic
**Observation:** Socket channels include the restaurant ID:
- `new_order_{restaurantId}` — carries: new-order, update-order, update-food-status, update-order-status, scan-new-order, delivery-assign-order
- `update_table_{restaurantId}` — carries: update-table
- `aggregator_order_{restaurantId}` — carries: aggrigator-order, aggrigator-order-update (Phase 3B)
...
| Own delivery | **Address details** — shown on click of pin icon in header (popup, not inline) |
**Status:** CONFIRMED by user
...
13. ~~Implement POST APIs for item-level actions (Ready, Serve, Cancel)~~ DONE (food-status-update endpoint)
14. ~~Implement POST API for order-level Accept/Reject~~ DONE (confirm uses `preparing`, cancel uses `cancelled`)
15. **BACKEND FIX: `order_amount` should return cumulative total** (B22)
