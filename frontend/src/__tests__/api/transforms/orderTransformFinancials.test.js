...
describe('Phase 1: orderTransform - Financial Fields', () => {
  ...
  describe('fromAPI.order() - New Financial Fields', () => {
    ...
    // -------------------------------------------------------------------------
    // Test 6: Transform-layer keeps subtotalBeforeTax as 0 when API field missing
    // (consumer-layer applies the order_amount fallback chain — see
    //  orderTransform.js:183 "No fallback — keep as 0" + L1360 consumer fallback)
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // Test 7: Transform-layer keeps subtotalAmount as 0 when API field missing
    // (consumer-layer applies the order_amount fallback chain)
    // -------------------------------------------------------------------------
    test('should keep subtotalAmount as 0 when order_sub_total_amount is missing (consumer-layer applies fallback)', () => {
      const apiResponse = createMockOrderResponse({
        order_amount: 100,
      ...
      // Transform-layer: no fallback (orderTransform.js:183). Consumer-layer
      // covers display fallback at orderTransform.js:1360.
    ...
    // -------------------------------------------------------------------------
    // Test 12: Handle null values gracefully (transform-layer: no fallback)
    // -------------------------------------------------------------------------
    test('should handle null values for financial fields (transform-layer keeps 0; consumer-layer fallback covers display)', () => {
      const apiResponse = createMockOrderResponse({
      ...
      // Transform-layer: no fallback per orderTransform.js:183 — null/missing
      // financial fields collapse to 0 here. The order_amount fallback chain
      // is applied at the consumer layer (orderTransform.js:1360).
      expect(result.subtotalBeforeTax).toBe(0);
      expect(result.serviceTax).toBe(0);
      expect(result.tipAmount).toBe(0);
      expect(result.tipTaxAmount).toBe(0);
