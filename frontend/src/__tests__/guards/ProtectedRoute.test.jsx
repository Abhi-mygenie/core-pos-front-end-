...
describe('Group B: Login Flow Regression', () => {
  ...
  test('B2: Dashboard redirects to /loading when authenticated but data not loaded', () => {
    mockAuthValue = { isAuthenticated: true };
    mockRestaurantValue = { isLoaded: false }; // override default — exercise CR-001 Fix B2

    // Per CR-001 Fix B2: when authenticated but contexts not yet hydrated,
    // ProtectedRoute itself routes through /loading (with a returnTo state)
    // before allowing the deep-linked target to mount. See
    // src/components/guards/ProtectedRoute.jsx L25-32.
