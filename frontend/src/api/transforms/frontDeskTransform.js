...
export const fromReservation = (r) => {
  ...
  return {
    ...
    charge,
    nightsDetail: charge?.nights_detail ?? null, // CR-385 M4 (BQ-385-19)
...
export const badgesFor = (row) => { // CR-385 BUG-447 (owner a): PAH badge AND the advance chip both render; prepaid alone
  const c = row?.charge ?? {};
  if (Number(c.prepaid_amount) > 0) return [{ kind: 'prepaid' }];
  const out = [];
  if (row?.pah) out.push({ kind: 'pah' });
  if (Number(c.advance_payment) > 0 && row?.operationalStatus !== 'departed') out.push({ kind: 'advance', amount: Number(c.advance_payment) });
  return out;
};
export const badgeFor = (row) => badgesFor(row)[0] ?? null; // primary badge (P0 API kept)
