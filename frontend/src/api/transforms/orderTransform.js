...
export const toAPI = {
  ...
  buildBillPrintPayload: (order, serviceChargePercentage = 0, overrides = {}) => {
    ...
    // Service charge on food subtotal
    // BUG-273/277: allow caller to override (e.g. when user toggled service charge OFF)
    ...
    // GST on service charge (use average GST rate from items) — only when service charge
    // is computed here (not overridden), otherwise caller has already accounted for it.
    if (overrides.serviceChargeAmount === undefined
        && serviceChargeAmount > 0 && computedSubtotal > 0) {
