...
export const getDailySalesReport = async (date) => {
  ...
  return {
    ...
    // Station Revenue
    stationRevenue: {
      kds: toNum(data.station_revenue?.KDS),
      bar: toNum(data.station_revenue?.BAR),
      other: toNum(data.station_revenue?.OTHER),
    },
 ...
 */
export const getOrderLogsReport = async (date, schedules, sortBy = 'collect_bill') => {
  const dateStr = formatDateParam(date);
  const { start, end, searchDates } = getBusinessDayRange(dateStr, schedules);

  // Fetch for each search date (1 or 2 if business day crosses midnight) and merge
  const responses = await Promise.all(
    searchDates.map(d =>
  ...
  // Merge orders from all responses, deduplicate by id
  const allRawOrders = responses.flatMap(r => r.order || []);
  const seen = new Set();
  const orders = allRawOrders.filter(o => {
    const oid = o.orders_table?.id;
  ...
  const transformedOrders = orders.map(orderWrapper => {
    ...
    let status = 'paid';
    if (fStatus === 7 || paymentMethod.toLowerCase() === 'cancelled') {
      status = 'cancelled';
    } else if (paymentMethod === 'Merge') {
      status = 'merged';
