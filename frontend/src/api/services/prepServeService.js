...
const parallelMap = async (items, fn, limit, onProgress) => {
  ...
  const worker = async () => {
    while (nextIdx < items.length) {
      ...
      catch (err) { results[idx] = { _error: err.message || 'Chunk failed' }; }
      completed++;
...
const fetchChunk = async (chunk, schedules) => {
 ...
 */
export const getPrepServeAnalytics = async (fromDate, toDate, schedules, onProgress) => {
  if (!fromDate || !toDate) return null;

  // Fetch
  const chunks = splitDateRange(fromDate, toDate);
  const chunkResults = await parallelMap(chunks, (chunk) => fetchChunk(chunk, schedules), PARALLEL_LIMIT, onProgress);
