...
const toLedgerRow = (o) => {
  ...
  return {
    ...
    contactNumber: '', dob: '', anniversary: '',
    addressType: '', area: '', pincode: '', completeAddress: '',
 ...
 */
export const getOrderLedgerForRange = async (fromDate, toDate, schedules, sortBy = 'collect_bill') => {
  ...
  // Room-excluded for operator-facing tabs
  const orders = filtered.filter((o) => !isRoomOrderForReport(o)).map(toLedgerRow);
