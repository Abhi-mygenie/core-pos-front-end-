...
/**
 * CR-004 Phase 2 — Fetch the full list of currently-active rooms.
 *
 * Drives the cross-day in-house view on `/reports/rooms`. The endpoint is a
 * GET (no body, just bearer token) and returns rooms regardless of
 * order-creation date, solving the "today's date only shows rooms checked
 * in today" gap left by the order-logs-report-driven default.
 *
 * @returns {Promise<Object>} response.data from the server (raw payload —
 *   the page-level transform is wired separately once the response shape is
...
export const splitRoomOrder = async ({ orderId, orderDetailIds, customerName, remark = '', targetTableId = null }) => {
  const payload = {
    ...
    remark,
    ...(targetTableId ? { target_table_id: targetTableId } : {}), // CR-163: Path A — land on existing free table
  };
...
// Called after storeTable() on Path B. old_table_id = 0 for newly split orders.
