  ...
  const worker = async () => {
    while (nextIdx < items.length) {
      ...
      try {
        ...
        results[idx] = { _error: err.message || 'Chunk failed' };
      }
      completed++;
 ...
 */
export const getFoodCourtForRange = async (fromDate, toDate, schedules, station, onProgress) => {
  ...
  const chunkResults = await parallelMap(
    chunks,
    (chunk) => fetchChunk(chunk, schedules),
    PARALLEL_LIMIT,
    onProgress
  );
