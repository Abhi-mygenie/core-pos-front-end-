 ...
 */
export const getItemSalesAggregated = async (fromDate, toDate, sortBy = 'collect_bill', schedules = []) => {
  // Backend only supports collect_bill | created_at. For "cancel_at" mode we fall back to
  // created_at server-side and filter cancellations client-side by their cancel_at timestamp.
  const isCancelDateMode = sortBy === 'cancel_at';
  const serverSortBy = isCancelDateMode ? 'created_at' : sortBy;

  // Parallel fetch: orders, products, categories, cancellation reasons
  const [ordersResp, productsResp, categoriesResp, cancelReasonsResp] = await Promise.all([
    api.post(API_ENDPOINTS.ORDER_LOGS_REPORT, {
  ...
  for (const wrapper of filteredOrders) {
    ...
    const orderRoundOff = parseFloat(ot.round_off) || 0;
    ...
    for (const line of items) {
      ...
      const itemTotal = unitPrice * qty + addonPrice + variationPrice;
      ...
      //   approved=true approvedDate="2026-06-03" approvedSource="Owner chat directive 2026-06-03"
      const isSold = !isCancelled && !isComplementary && isOrderPaid;
      const isPending = !isCancelled && !isComplementary && !isOrderPaid;

      // @audit:rule id="FE-12" name="Cancel-date mode line filter"
      //   explains="When attribution toggle = cancel_at, drop cancelled lines whose cancel_at date falls outside [fromDate,toDate]."
      //   approved=false approvedDate="" approvedSource=""
      if (isCancelDateMode && isCancelled) {
        const cancelDay = typeof line.cancel_at === 'string' ? line.cancel_at.slice(0, 10) : '';
        if (!cancelDay || cancelDay < fromDate || cancelDay > toDate) continue;
      }
      ...
      if (!isCancelled) {
        ...
        if (Math.abs(_drift) > 0.02) {
          const bucket = isCancelled ? 'cancelled' : isComplementary ? 'comp' : isPending ? 'pending' : 'sold';
      ...
      if (existing) {
        ...
        itemMap.set(foodId, {
          ...
          qtyComplementary:  isComplementary ? qty : 0,
      ...
      // Track sold lines for order-level charge distribution (Pass 2)
      if (isSold && totalRevenue > 0) {
        orderSoldLineRevenue += totalRevenue;
        orderLineData.push({ foodId, totalRevenue });
      }
    }
    ...
    if (orderLevelCharges !== 0 && orderSoldLineRevenue > 0 && orderLineData.length > 0) {
      for (const ld of orderLineData) {
        ...
        if (entry) {
          entry.totalRevenueSold += distributed;
        }
      }
    }
  }
 ...
 */
export const getDashboardAggregated = async (fromDate, toDate, schedules = []) => {
  const [ordersResp, cancelDataResp, cancelReasonsResp] = await Promise.all([
    api.post(API_ENDPOINTS.ORDER_LOGS_REPORT, {
      sort_by: 'created_at',
      from_date: fromDate,
      to_date: toDate,
    }),
    // Second fetch for cancel_at attribution (Option A: separate API call)
    api.post(API_ENDPOINTS.ORDER_LOGS_REPORT, {
      sort_by: 'created_at',
      from_date: fromDate,
      to_date: toDate,
  ...
  const orders = ordersResp.data?.order || [];
  const cancelDataOrders = cancelDataResp.data?.order || [];
  const cancelReasons = cancelReasonsResp.data?.reasons || [];
  const cancelReasonById = new Map(cancelReasons.map((r) => [r.id, r.reason]));
  ...
  const filteredCancelOrders = cancelDataOrders.filter(bdFilter);
  ...
  for (const wrapper of filteredOrders) {
    ...
    let channel = 'Dine-In';
    if (orderIn === 'RM' || orderIn === 'SRM') channel = 'Room';
    ...
    if (isPaid) {
      ...
      // Hourly buckets from created_at
      const createdAt = ot.created_at || '';
      if (createdAt) {
        const hour = createdAt.slice(11, 13) || '00';
        hourlyBuckets[hour] = (hourlyBuckets[hour] || 0) + orderAmount;
      }
      ...
      // Tile 4 — Payment
      const pm = ot.payment_method || 'Other';
      const pmKey = pm.toLowerCase().includes('upi') ? 'UPI'
        : pm.toLowerCase().includes('card') ? 'Card'
        : pm.toLowerCase().includes('cash') ? 'Cash'
        : pm.toLowerCase() === 'tab' ? 'TAB'
        : pm.toLowerCase().includes('transfertoroom') ? 'Room Transfer'
        : pm;
      paymentCounts[pmKey] = (paymentCounts[pmKey] || 0) + 1;
      paymentRevenue[pmKey] = (paymentRevenue[pmKey] || 0) + orderAmount;

      // TAB unsettled — use f_order_status (payment_status is unreliable from list endpoint)
      if (pm.toLowerCase() === 'tab' && fStatus !== '6') {
    ...
    // Channel counts — paid orders only (aligned with Net Sales gate)
    if (isPaid) {
      channelCounts[channel] = (channelCounts[channel] || 0) + 1;
      channelRevenue[channel] = (channelRevenue[channel] || 0) + orderAmount;
    }
  ...
  // --- Cancellations from cancel_at attribution (Option A: separate created_at fetch) ---
  // Uses cancelDataOrders (created_at sort) and filters by item-level cancel_at within range.
  for (const wrapper of filteredCancelOrders) {
    const ot = wrapper.orders_table || {};
    const items = wrapper.order_details_table || [];
    const fStatus = String(ot.f_order_status);
    const orderAmount = parseFloat(ot.order_amount) || 0;
    ...
    if (fStatus === '3') {
      // Order-level cancel: include if any item's cancel_at falls in range
      const hasItemInRange = items.some((item) => {
        const cancelDay = typeof item.cancel_at === 'string' ? item.cancel_at.slice(0, 10) : '';
        return cancelDay && cancelDay >= fromDate && cancelDay <= toDate;
      });
      if (hasItemInRange) {
        cancelledOrderCount += 1;
        cancelledRevenue += orderAmount;
        const reason = ot.cancellation_reason || 'No reason provided';
        cancelReasonCounts[reason] = (cancelReasonCounts[reason] || 0) + 1;
      }
    }

    // Item-level cancels: filter by cancel_at in date range
    // ONLY count items from NON-cancelled orders (avoids double-counting with order-level)
    if (fStatus !== '3') {
      for (const line of items) {
        if (String(line.food_status) === '3') {
          const cancelDay = typeof line.cancel_at === 'string' ? line.cancel_at.slice(0, 10) : '';
          if (cancelDay && cancelDay >= fromDate && cancelDay <= toDate) {
            cancelledItemCount += 1;
            cancelledItemRevenue += parseFloat(line.price) || 0;
            if (line.reason_type) {
  ...
  return {
    ...
    payments: {
      mix: paymentMix,
      unsettledTab: Math.round(unsettledTab),
    },
    cancellations: {
