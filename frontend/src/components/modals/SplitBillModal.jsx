...
const SplitBillModal = ({
  ...
  onClose,
  orderId,
  items = [],  // Cart items with id, name, qty, price (LINE TOTAL), unitPrice
  grandTotal = 0, // BUG-004: authoritative final total from CollectPaymentPanel
                   // (includes discount / SC / tax / tip / delivery / round-off).
                   // Falls back to items subtotal when 0.
  ...
  // BUG-004: items subtotal is the sum of line totals (items.price is LINE TOTAL
  // per BUG-004 parent mapping fix). totalBill prefers the authoritative grandTotal
  // from CollectPaymentPanel, falling back to itemsSubtotal for safety.
  const itemsSubtotal = useMemo(
    () => items.reduce((sum, item) => sum + (Number(item.price) || 0), 0),
    [items]
  );
  const totalBill = grandTotal > 0 ? grandTotal : itemsSubtotal;

  // BUG-004: per-person totals are scaled from line-item totals by the ratio of
  // totalBill/itemsSubtotal, so order-level charges (discount / SC / tax / tip /
  // delivery / round-off) are apportioned proportionally and the sum of all
  // persons' totals equals totalBill when every item is assigned.
  const personTotals = useMemo(() => {
    const totals = {};
    persons.forEach(p => { totals[p.id] = 0; });
    if (itemsSubtotal <= 0) return totals;
    const scaleFactor = totalBill / itemsSubtotal;
    Object.entries(assignments).forEach(([itemId, { personId, qty }]) => {
      const item = items.find(i => String(i.id) === String(itemId));
      if (!item || !personId) return;
      // item.price is the LINE TOTAL for full qty; derive per-unit line share.
      const unitLine = (Number(item.price) || 0) / (item.qty || 1);
      totals[personId] = (totals[personId] || 0) + (unitLine * qty * scaleFactor);
    });
    return totals;
  }, [assignments, items, persons, totalBill, itemsSubtotal]);

  // Calculate unassigned total
  const assignedTotal = Object.values(personTotals).reduce((sum, t) => sum + t, 0);
  const unassignedTotal = Math.max(0, totalBill - assignedTotal);
