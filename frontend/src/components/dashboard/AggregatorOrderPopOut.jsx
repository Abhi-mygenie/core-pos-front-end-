...
const AggregatorOrderPopOut = ({
  ...
  // MUST come before any useState() that reads `settings` to avoid TDZ error.
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      const printId = order.orderId; // BUG-290: must use internal DB ID — aggrId (e.g.'3H5H9488') casts to int 3 on backend → wrong restaurant prints
      if (printKot) {
        manuallyPrintAggregator(printId, 'aggr_kot').catch(err =>
          console.warn('[AggregatorPopOut] CR-118: KOT print failed (non-blocking):', err?.message));
      }
      if (printBill) {
        manuallyPrintAggregator(printId, 'aggr_bill').catch(err =>
