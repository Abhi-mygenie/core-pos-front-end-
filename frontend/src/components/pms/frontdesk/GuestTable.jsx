...
import { badgeFor } from '@/api/transforms/frontDeskTransform';
...
export const Badge = ({ badge, rowId, inHouse }) => {
  if (!badge) return null;
  const b = BADGE[badge.kind];
  const text = badge.kind === 'advance' ? `${inHouse ? 'Paid so far' : 'Advance'} ${fmtINR(badge.amount)}` : b.text;
  return <span data-testid={`fd-row-${rowId}-badge`} className="inline-block ml-1.5 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide" style={{ background: b.bg, color: b.color }}>{text}</span>;
};
...
export const commonColumns = ({ amountKey, amountLabel, statusOf, actions, badgeInHouse = false }) => [
  ...
  { key: 'channel', label: 'Source', render: (r) => <span>{channelLabel(r.channel)}<Badge badge={badgeFor(r)} rowId={r.id} inHouse={badgeInHouse} /></span> },
  ...
  { key: 'amount', label: amountLabel, sortable: true, className: 'text-right', render: (r) => <span data-testid={`fd-row-${r.id}-balance`} className="font-semibold tabular-nums">{fmtINR(r.charge?.[amountKey])}</span> },
...
export const PhaseButton = ({ testId, label, phase, danger, onClick }) => (
  <button type="button" data-testid={testId} disabled={!onClick} title={onClick ? undefined : `Available in Phase ${phase}`}
