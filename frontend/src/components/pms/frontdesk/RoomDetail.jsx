// CR-385 M0 — RoomDetail: 6 states (available · booked · occupied · occupied_hk · hk · ooo). Live: Mark Clean / Request HK / Set OOO / Back in service via patchRoomStatus (same call as RoomStatusPage). Check in / Bill / Extend phase-gated.
...
export const RoomDetail = ({ room, row, nextArrival, isTurn, busy, onPatch, onClose }) => {
  ...
  return (
    <div className="fd-expansion col-span-full bg-white rounded-xl border border-[#E5E5E5] p-4" data-testid={`fd-room-detail-${room.id}`} style={{ borderLeft: `4px solid ${color}` }}>
      ...
      <div className="mt-4 pt-3 border-t border-[#F0F0F0] flex flex-wrap gap-2 justify-end" data-testid={`fd-room-actions-${room.id}`}>
        ...
        {s === 'booked' && <PhaseButton testId={`fd-room-action-checkin-${room.id}`} label="Check In" phase={2} />}
        {(s === 'occupied' || s === 'occupied_hk') && (
          <>
            <PhaseButton testId={`fd-room-action-extend-${room.id}`} label="Extend" phase={3} />
            <PhaseButton testId={`fd-room-action-bill-${room.id}`} label="Bill" phase={4} />
          </>
        )}
      </div>
    </div>
