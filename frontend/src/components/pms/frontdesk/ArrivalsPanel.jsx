...
// CR-385 M2 — row actions live: Modify (server preview) · Cancel OR No-Show by nsOrCancel(row) (AC-13, EITHER/OR)
// CR-385 M3 — Check In live (P2): early check-in guard N7/D53 (rules.allowEarlyCheckin vs meta.business_date) · expansion kind 'checkin' → CheckInForm
...
import CheckInForm, { isEarly } from './CheckInForm'; // CR-385 M3
...
export const ArrivalsPanel = ({ rows, meta, kpis, expandedId, expandedKind = 'detail', onToggle, onOpen, onDone, cancelledBy, chip, onChip, rules, rooms }) => { // CR-385 M3 rules + rooms
  ...
  const actions = (r, variant = '') => { // BUG-439 variant '' = row cell · 'exp-' = expansion drawer → unique testids (D73)
    ...
    const blockedEarly = isEarly(r, bd) && !rules?.allowEarlyCheckin; // CR-385 M3 N7/D53: server meta.business_date only, never the browser clock
    return (
      <>
        <PhaseButton testId={`fd-row-${r.id}-${variant}checkin-btn`} label="Check In" phase={2} // BUG-439 · CR-385 M3 live
          onClick={blockedEarly ? undefined : () => onOpen?.(r.id, 'checkin')}
          title={blockedEarly ? `Arrives ${fmtDate(r.checkin)} — modify the booking dates to check in today` : undefined} />
  ...
  const renderExpansion = (row) => { // CR-385 M2 — one expansion at a time (F1); kind picks the body
    switch (expandedKind) {
      case 'checkin': // CR-385 M3
        return <CheckInForm row={row} meta={meta} rooms={rooms} rules={rules} onDone={onDone} onClose={close} />;
      case 'modify':
