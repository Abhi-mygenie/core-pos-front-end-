...
export const RoomsPanel = ({ snapshot, expandedRoomId, onToggleRoom, chip, onChip, busyId, onPatch, onRetry, retrying = false, onCheckIn, onExtend, onBill }) => { // CR-385 M6 onBill // CR-385 M4 onExtend // CR-385 M0.5 BUG-434 retrying · CR-385 M3 onCheckIn
          ...
          <div className="grid grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3">
            {g.rooms.map((room) => [
              ...
              String(expandedRoomId) === String(room.id) && (
                <RoomDetail key={`${room.id}-d`} room={room} row={inHouseFor(room)} nextArrival={nextArrivalFor(room)} isTurn={turnIds.has(room.id)} busy={busyId === room.id} onPatch={onPatch} onCheckIn={onCheckIn} onExtend={onExtend} onBill={onBill} onClose={() => onToggleRoom(null)} />
              ),
            ])}
          </div>
