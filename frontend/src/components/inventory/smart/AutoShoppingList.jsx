                ...
                {purchaseRows.map((r) => {
                  ...
                  return (
                    <tr key={`${r.ingredient_id}-${ix}`} className={`border-b border-slate-50 hover:bg-slate-50/40 ${rowBg(r)}`} data-testid={`plan-row-${r.ingredient_id}`}>
                      ...
                      <td className="py-2 px-3 text-sm text-slate-600">{fmtBreak(r.projected_need, r)}</td>{/* CR-387 */}
                      <td className="py-2 px-3 text-sm font-medium text-red-600">{fmtBreak(r.gap, r)}</td>{/* CR-387 */}
                      <td className="py-2 px-3">
                      ...
                      <td className="py-2 px-3">
                        <Input type="number" step="0.01" min="0" value={r.rate ?? ''}
                          ...
                          placeholder={r.suggestedRate ? `₹${(r.suggestedRate * (rowQuantity(r).baseQty || r.suggest_qty || 1)).toFixed(0)}` : '₹'}
                          className="h-8 text-sm w-20" data-testid={`row-rate-${r.ingredient_id}`} />
                        {r.suggestedRate && !r.rate && <div className="text-[10px] text-blue-500 font-medium mt-0.5">last total: ₹{(r.suggestedRate * (rowQuantity(r).baseQty || r.suggest_qty || 1)).toFixed(0)}</div>}{/* CR-348 */}
                      </td>
                      <td className="py-2 px-3">
                  ...
                  {items.map((r) => {
                    ...
                    return (
                      <tr key={`avail-${r.ingredient_id}`} className={`border-b border-slate-50 hover:bg-slate-50/40 ${rowBg(r)}`} data-testid={`avail-row-${r.ingredient_id}`}>
                        ...
                        <td className="py-2 px-3 text-xs text-slate-600">{r.projected_need > 0 ? fmtBreak(r.projected_need, r) : '—'}</td>{/* CR-387 */}
                        <td className="py-2 px-3 text-xs text-slate-600">{r.suggest_qty > 0 ? fmtBreak(r.suggest_qty, r) : '—'}</td>{/* CR-387 */}
                        <td className="py-2 px-3">
