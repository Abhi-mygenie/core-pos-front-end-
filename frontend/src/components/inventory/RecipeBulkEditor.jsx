...
export default function RecipeBulkEditor({ recipes, recipeType, onRefresh }) {
  ...
  // ── Load masters + hydrate rows when recipes/tab change ──────
  useEffect(() => {
    setRows((recipes || []).map(normaliseRecipe));
    setExpanded(new Set());
  ...
  useEffect(() => {
    (async () => {
      try {
        const [ings, foods] = await Promise.all([
  ...
  const addRecipe = () => {
    const nr = normaliseRecipe({ id: null });
  ...
  const handleImport = async (e) => {
    ...
    try {
      await recipeService.importRecipes(fd);
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
            <tr>
              ...
              <th className="w-20 p-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-500">Qty</th>
              <th className="w-24 p-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-500">Unit</th>
              <th className="w-20 p-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-500">Prep</th>
              <th className="w-20 p-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-500">Cook</th>
              <th className="w-20 p-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-500">Serves</th>
              <th className="w-24 p-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-500">Ingredients</th>
              <th className="w-24 p-2 text-right text-[10px] font-bold uppercase tracking-wider text-slate-500">Cost</th>
              <th className="w-20 p-2 text-right text-[10px] font-bold uppercase tracking-wider text-slate-500">Margin</th>
              <th className="w-10 p-2"></th>
            </tr>
          </thead>
          <tbody>
...
// ────────────────────────────────────────────────────────────────
function RecipeRow({ row, idx, isExp, cost, margin, marginBand, ingredientsMaster, foodsMaster, recipeType, onToggleExpand, onUpdateRow, onUpdateIng, onAddIng, onRemoveIng, onDeleteRow }) {
