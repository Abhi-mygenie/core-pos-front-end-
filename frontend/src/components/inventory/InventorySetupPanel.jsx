            ...
            <table className="w-full text-left border-collapse" data-testid="ingredient-table">
              <thead>
                <tr className="bg-slate-50/80">
                  ...
                  <th className="py-2.5 px-4 text-xs font-semibold uppercase tracking-wider text-slate-500 border border-slate-200 text-right" style={{ width: 100 }}>Min Alert</th>
              ...
              {/* BUG-212 B: Add Ingredient inline form — expanded to 7 fields */}
                  ...
                  <tr className="border-b border-slate-100 bg-green-50/30" data-testid="ingredient-add-row">
                    ...
                    <td className="py-2 px-4 text-center">
                      <Input type="number" value={newIng.conversionFactor} onChange={e => setNewIng(p => ({ ...p, conversionFactor: e.target.value }))}
                        placeholder="Factor..." className="h-8 text-xs" data-testid="new-ingredient-conversion" />
                    </td>
                    <td className="py-2 px-4 text-center">
                      <select className="h-8 text-xs border border-slate-200 rounded-md px-2 w-full outline-none"
                        value={newIng.smallUnit} onChange={e => setNewIng(p => ({ ...p, smallUnit: e.target.value }))} data-testid="new-ingredient-small-unit">
                ...
                {loading ? (
                  ...
                  <tr key={ing.id} className="border-b-2 border-blue-300 bg-blue-50/40" data-testid={`ingredient-edit-row-${ing.id}`}>
                    ...
                    <td className="py-2 px-4 text-center">
                      <select className="h-8 text-xs border border-slate-200 rounded-md px-2 w-full outline-none"
                        value={editIng.unit} onChange={e => setEditIng(p => ({ ...p, unit: e.target.value }))} data-testid="edit-ingredient-unit">
                        <option value="">Unit...</option>
                        {units.map((u, i) => <option key={i} value={typeof u === 'string' ? u : u.name}>{typeof u === 'string' ? u : u.name}</option>)}
                      </select>
                    </td>
                    <td className="py-2 px-4 text-center">
                      <Input type="number" value={editIng.conversionFactor} onChange={e => setEditIng(p => ({ ...p, conversionFactor: e.target.value }))}
                        placeholder="Factor" className="h-8 text-xs" data-testid="edit-ingredient-conversion" />
                    </td>
                    <td className="py-2 px-4 text-center">
                      <select className="h-8 text-xs border border-slate-200 rounded-md px-2 w-full outline-none"
                        value={editIng.smallUnit} onChange={e => setEditIng(p => ({ ...p, smallUnit: e.target.value }))} data-testid="edit-ingredient-small-unit">
                        <option value="">Small unit...</option>
                        {units.map((u, i) => <option key={i} value={typeof u === 'string' ? u : u.name}>{typeof u === 'string' ? u : u.name}</option>)}
                      </select>
                    </td>
                    <td className="py-2 px-4">
                      <div className="flex gap-1">
                        <Input type="number" value={editIng.minQtyAlert} onChange={e => setEditIng(p => ({ ...p, minQtyAlert: e.target.value }))}
                          placeholder="Min qty" className="h-8 text-xs w-16" data-testid="edit-ingredient-min-qty" />
                        <Input type="number" value={editIng.minUnitAlert} onChange={e => setEditIng(p => ({ ...p, minUnitAlert: e.target.value }))}
                          placeholder="Min unit" className="h-8 text-xs w-16" data-testid="edit-ingredient-min-unit" />
                      </div>
                    </td>
                    <td className="py-2 px-4 text-center">
                      <div className="flex flex-col gap-1">
