...
import { getWastageReport } from '@/api/services/wastageReportService'; // CR-349
...
// BUG-262: WastagePlaceholder removed — "Coming Soon" not appropriate for production.
// Restore when BUG-266 (backend wastage endpoint) is unblocked.
...
export default function InventoryIntelligencePanel() {
  ...
  const [wastageNet, setWastageNet] = useState(null); // CR-349: live net_wastage from API
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        ...
        getWastageReport(dates30.from_date, dates30.to_date), // CR-349
      ]);
      const [s, d30, d7, v, r, f, wResult] = results; // CR-349: added wResult
      ...
      // CR-349: wire wastage KPI — silent fail (allSettled handles it)
      if (wResult?.status === 'fulfilled') {
        setWastageNet(wResult.value?.summary?.net_wastage ?? null);
      }
    ...
    <div data-testid="inventory-intelligence-panel" className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3" data-testid="kpi-cards-row">
        ...
        <KpiCard icon={Trash2} iconBg="#FEF2F2" iconColor="#EF4444"
                 value={wastageNet !== null ? wastageNet : '—'}
                 label="Wastage Value · 30D"
                 testId="kpi-wastage-value" /> {/* CR-349: wired to live API, P2 badge removed */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* BUG-262: Wastage placeholders removed. Restore when BUG-266 ships. */}
      </div>
    </div>
  );
}
