...
export default function SmartPurchasePanel() {
  ...
  const validate = () => {
    ...
    // CR-100: validate split payment per vendor group (replaces single-string PM check)
    ...
    for (const [vid, group] of Object.entries(groupedByVendor)) {
      try {
        ...
        await inventoryService.addPurchase({
          ...
          purchaseDate,
          paymentType: pmData.type || 'paid',                                // CR-100: enum (was paymentMethod string)
          splits: pmData.splits || [],                                        // CR-100: payment split rows
          invoiceNumber: '',
