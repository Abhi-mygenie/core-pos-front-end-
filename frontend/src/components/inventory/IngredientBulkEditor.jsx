...
import { Search, Plus, Download, Trash2, X, Check, Loader2, GripVertical } from 'lucide-react';
...
export default function IngredientBulkEditor({ allItems, categories, units, onRefresh, onClose }) {
  ...
  const handleSave = async () => {
    ...
    // Process deletes first (rows marked _deleted that have _id)
  ...
  const handleExcel = () => {
    const data = filtered.map(r => ({
      ...
      'Base Unit': r.unit,
      'Small Unit': r.smallUnit,
      'Conversion Factor': r.conversionFactor,
      'Min Qty Alert': r.minQtyAlert,
      'Min Unit Alert': r.minUnitAlert,
    }));
