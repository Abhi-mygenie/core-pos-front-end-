// CR-133: Printer Agent Config — container view (5 tabs + Station Mapping, single shared state, sticky save)
import { useState, useEffect, useCallback } from "react";
import { Loader2, AlertCircle, RefreshCw } from "lucide-react";
import { COLORS } from "../../../../constants";
import { useToast } from "../../../../hooks/use-toast";
import { getConfig, saveConfig, getAreaOptions } from "../../../../api/services/printerAgentConfigService";
...
import { StationMappingTab } from "./StationMappingTab";  // CR-353
...
const TABS = [
  ...
  { id: "stationmapping", label: "Station Mapping" },  // CR-353
];
...
export const PrinterAgentConfigView = () => {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [data, areaOpts] = await Promise.all([getConfig(), getAreaOptions()]); // CR-167: load area-options for PrinterForm KOT routing
      setConfig({ ...data, options: { ...data.options, areaOptions: areaOpts } });
      setDirty(false);
    } catch (e) {
  ...
  return (
    <div className="flex flex-col" data-testid="printer-config-view">
      ...
      <div className="flex-1">
        ...
        {activeTab === "mapping"       && <PrinterMappingTab />}          {/* CR-160: own load/save */}
        {activeTab === "stationmapping" && <StationMappingTab />}           {/* CR-353: own load/save */}
      </div>
