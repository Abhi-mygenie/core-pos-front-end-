...
export default function RoleFormView({ role, onBack }) {
  ...
  // BUG-376-E: role type string lives in roleTypes state — exclude from checkedPerms so clearAll() never removes it (OD-1)
  ...
  useEffect(() => {
    if (catalogRoleTypes.length > 0 && roleTypes.length === 0) {
      setRoleTypes(catalogRoleTypes.map(rt => rt.value)); // BUG-376-C: string values not numeric IDs
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps -- BUG-235 intentional: only fire when catalog loads
  }, [catalogRoleTypes]);
  ...
  const applyTemplate = (templateId) => {  // CR-096 + BUG-376-B + BUG-376-C
    if (isReadOnly) return;
    if (!templateId) {
      setSelectedMasterId(null);                                                              // BUG-376-B: reset when "Build from scratch"
      clearAll();
  ...
  const handleSave = async () => {
    ...
    try {
      const data = {
        ...
        roleTypes,
        roleMasterId: selectedMasterId,  // BUG-376-B: pass template ID (null when "Build from scratch")
      };
      if (isEdit) {
