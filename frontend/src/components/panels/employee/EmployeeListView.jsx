...
export default function EmployeeListView() {
  ...
  const addRow = () => {
    ...
    const row = {
      ...
      password: '',
      roleId: roles.find(r => r.active)?.id || null,   // BUG-234 fix: isEditable excluded system roles; any active role is assignable
      roleName: roles.find(r => r.active)?.name || '',  // BUG-234 fix
    };
  ...
  // BUG-234 fix: removed r.isEditable — system/default roles have is_editable=false
  // but must still be assignable to employees. isEditable gates role definition
  // editing (RoleFormView), not role assignment here.
  const roleOptions = roles.filter(r => r.active);
