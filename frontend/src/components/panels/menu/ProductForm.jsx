      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <SelectField label="Sold By (Unit)" value={form.itemUnit} onChange={(v) => {
              update("itemUnit", v);
              // CR-010: Auto-populate unit price from price when unit is selected
              if (['Kg','gm','L','ml'].includes(v) && (!form.itemUnitPrice || form.itemUnitPrice === 0)) {
                update("itemUnitPrice", form.basePrice || 0);
              }
              if (!v || v === '') { update("itemUnitPrice", 0); }
            }}
              options={[
        ...
        <Section title="Pricing & Tax" defaultOpen={true}>
          ...
          {/* CR-010: Unit Price field (dropdown moved to Basic Info) */}
          {form.itemUnit && ['Kg','gm','L','ml'].includes(form.itemUnit) && (
            <div className="mt-2">
              <InputField label={`Unit Price (₹ per ${form.itemUnit})`} value={form.itemUnitPrice} onChange={(v) => update("itemUnitPrice", v)} type="number" min={0} step={0.01} />
            </div>
          )}
        </Section>
          ...
          <div className="pt-2">
            ...
            <div className="flex gap-2 items-end pt-2 border-t" style={{ borderColor: COLORS.borderGray }}>
              ...
              <button onClick={async () => {
                ...
                } catch (err) { toast({ title: "Error", description: "Failed to create add-on." }); }
