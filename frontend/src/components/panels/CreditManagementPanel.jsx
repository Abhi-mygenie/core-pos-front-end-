...
const CreditManagementPanel = ({ isOpen, onClose, sidebarWidth }) => {
  ...
  const handleGenerateStatement = async (customer, detail, dateRange, { isDetailed, preOpenedWindow } = {}) => {
    ...
    try {
      ...
      toast({ title: 'Statement failed', description: err?.message || 'Could not generate statement.', variant: 'destructive' });
    } finally {
  ...
  const handleRowDownload = async (customer) => {
    ...
    try {
      ...
      if (win && !win.closed) win.close();
      toast({ title: 'Download failed', description: 'Could not load customer data.', variant: 'destructive' });
    } finally {
      setGeneratingStatement(false);
    }
  };
  ...
  const handlePortfolioExport = async (filteredCustomers, filterLabel) => {
    ...
    try {
      ...
      if (win && !win.closed) win.close();
      toast({ title: 'Export failed', description: err?.message || 'Could not generate portfolio summary.', variant: 'destructive' });
    } finally {
      setGeneratingStatement(false);
    }
  };
