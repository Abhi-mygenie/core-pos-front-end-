...
const SettlementPanel = ({ isOpen, onClose, sidebarWidth }) => {
  ...
  const handleSettle = async () => {
    ...
    try {
      const w = settleModal;
      const expected = w.totalFunds - w.settled - w.pilferage;
      ...
      <div className="flex-1 overflow-auto">
        <div className="px-5 py-4">
          ...
          <div className="flex gap-2 mb-4">
            ...
            <KpiCard icon={TrendingUp} label="Cash Collected" value={totals.cashCollected} color={COLORS.primaryGreen} sub={`Total sale: ${fmt(totals.totalSale)}`} />
              ...
              <tbody>
                ...
                {waiters.length > 0 && (
                  <tr style={{ background: "#F0F0F0" }}>
                    ...
                    <td className="px-3 py-2 text-right text-sm font-mono font-bold" style={{ color: COLORS.darkText }}>{fmt((totals.totalFunds || 0) - (totals.settled || 0) - (totals.pilferage || 0))}</td>
          ...
          <div className="bg-white rounded-2xl w-[420px] shadow-xl">
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <input type="number" value={settleAmount} onChange={e => setSettleAmount(Math.max(Number(e.target.value), 0))}
                    ...
                    style={{ borderColor: settleAmount > (settleModal.totalFunds - settleModal.settled - settleModal.pilferage) ? "#EF4444" : COLORS.borderGray, color: COLORS.darkText }} data-testid="settle-amount-input" />
                </div>
                {settleAmount > (settleModal.totalFunds - settleModal.settled - settleModal.pilferage) && (
                  <p className="text-[11px] mt-1" style={{ color: "#EF4444" }}>Cannot settle more than expected balance ({fmt(settleModal.totalFunds - settleModal.settled - settleModal.pilferage)})</p>
                )}
              </div>
              <div>
                <label className="text-xs font-medium uppercase tracking-wide" style={{ color: COLORS.grayText }}>Type</label>
                <div className="flex gap-2 mt-1">
                  <button onClick={() => { setSettleType("full"); const actual = actualBalances[settleModal.waiterId]; const exp = settleModal.totalFunds - settleModal.settled - settleModal.pilferage; const val = actual !== undefined ? Math.min(Math.abs(Math.round(actual)), exp) : Math.min(Math.abs(Math.round(settleModal.balanceToSettle)), exp); setSettleAmount(Math.max(val, 0)); }}
              ...
              <div className="rounded-lg p-3" style={{ background: COLORS.sectionBg }}>
                <div className="flex justify-between text-xs"><span style={{ color: COLORS.grayText }}>Expected</span><span className="font-mono font-medium" style={{ color: COLORS.darkText }}>{fmt(settleModal.totalFunds - settleModal.settled - settleModal.pilferage)}</span></div>
                <div className="flex justify-between text-xs mt-1"><span style={{ color: COLORS.grayText }}>Pilferage</span><span className="font-mono" style={{ color: actualBalances[settleModal.waiterId] !== undefined && (settleModal.totalFunds - settleModal.settled - settleModal.pilferage) > actualBalances[settleModal.waiterId] ? "#EF4444" : COLORS.grayText }}>{actualBalances[settleModal.waiterId] !== undefined ? fmt(Math.max((settleModal.totalFunds - settleModal.settled - settleModal.pilferage) - actualBalances[settleModal.waiterId], 0)) : "₹0"}</span></div>
              </div>
            </div>
            <div className="px-6 py-4 border-t flex justify-end gap-2" style={{ borderColor: COLORS.borderGray }}>
              ...
              <button onClick={handleSettle} disabled={saving || settleAmount <= 0 || settleAmount > (settleModal.totalFunds - settleModal.settled - settleModal.pilferage)} className="px-6 py-2 text-sm font-medium rounded-lg text-white disabled:opacity-50" style={{ background: COLORS.primaryOrange }} data-testid="confirm-settle-btn">{saving ? "Settling..." : "Confirm"}</button>
            </div>
          </div>
        </div>
      )}
