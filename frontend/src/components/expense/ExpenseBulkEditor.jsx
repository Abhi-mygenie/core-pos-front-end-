...
const ExpenseBulkEditor = ({ items, categories, pricedItems = [], onRefresh, onClose }) => {
  ...
  const bulkMoveConfirmed = async () => {
    ...
    const succeededCount = results.length - failed.length;
    const failedIdSet = new Set(failed.map(f => f.id));
    // BUG-276: Keep item in visual position with "→ Category" badge instead of jumping
    setRows(prev => prev.map(r => {
      if (movableIds.includes(r._id) && !failedIdSet.has(r._id)) {
        return {
          ...r,
          _saveStatus: "saved",
          _movedTo: targetCat.name,
    ...
    // BUG-276: Don't auto-refresh after move — keep items in place with "→ Category" badge
  };
          ...
          return (
            <div key={row._id}
              ...
              <div className="flex-[3] px-2 flex items-center gap-2">
                ...
                />
                {/* BUG-276: Show badge when item was moved to a different category */}
                {row._movedTo && (
                  <span className="ml-1 text-[10px] px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 font-medium border border-blue-200 whitespace-nowrap"
