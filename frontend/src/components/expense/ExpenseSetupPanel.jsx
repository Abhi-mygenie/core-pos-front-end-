...
const ExpenseSetupPanel = () => {
  ...
  // BUG-201: item impact state (transaction count before delete)
  const [itemImpactData, setItemImpactData] = useState(null);
  ...
  const addCategory = async () => {
    ...
    try {
      ...
      // BUG-164: Backend now returns HTTP 409 for duplicates — Axios throws on 4xx.
      // Previous body-inspection workaround removed; catch block handles it.
  ...
  // ── Delete item ───────────────────────────────────────────────────
  // BUG-162: optimistic
  // BUG-201: fetch impact first, show count in modal
  const handleDeleteItemClick = useCallback(async (itemId) => {
    setDeletingItemId(itemId);
    setItemImpactData(null);
    setImpactLoading(true);
    try {
      const res = await expenseService.getItemImpact(itemId);
      setItemImpactData(res.data);
    } catch {
      // silently ignore — modal shows generic fallback text
    } finally {
  ...
  const deleteItem = async () => {
    ...
    try {
      await expenseService.deleteExpenseItem(itemId, 'Deleted by owner');
  ...
  const saveEditItem = async () => {
    ...
    try {
      // BUG-203: Single PUT with unit_price (backend now accepts it — 2-call workaround removed)
  ...
  return (
    <div className="p-6 min-h-full" data-testid="expense-setup-panel">
      ...
      {/* ── Delete Item Confirm ──────────────────────────────────── */}
      {/* BUG-201 Phase 1: impact-aware modal with real transaction count + amount from GET /item/{id}/impact */}
      {deletingItemId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 w-96 border" style={{ borderColor: COLORS.borderGray }}
            ...
            <p className="text-sm mb-5" style={{ color: COLORS.grayText }}>
              {impactLoading
                ...
                : itemImpactData?.transaction_count > 0
                  ? `This item has ${itemImpactData.transaction_count} transaction${itemImpactData.transaction_count === 1 ? '' : 's'} worth ₹${itemImpactData.total_amount}. Deleting will permanently remove the item and all related expense records. This cannot be undone.`
                  : 'Delete this item? This cannot be undone.'
              }
            </p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => { setDeletingItemId(null); setItemImpactData(null); }}
                className="px-4 py-2 text-sm rounded-lg border hover:bg-gray-50"
                style={{ borderColor: COLORS.borderGray, color: COLORS.grayText }}>Cancel</button>
              <button onClick={deleteItem} disabled={impactLoading}
