...
const StatusBadge = ({ status, paymentStatus, paymentMethod, isCancelled, isMerged }) => {
  const getStatusConfig = () => {
    ...
    if (paymentStatus === 'paid') return { label: 'Settled', color: 'bg-emerald-600' };
    if (status === 'delivered') return { label: 'Settled', color: 'bg-emerald-600' };
...
const OrderDetailSheet = ({ isOpen, onClose, order, tabId }) => {
  ...
  useEffect(() => {
    ...
    // DATA MODE: items already attached from order-logs-report transform
    if (order.items) {
      setDetails(order);
      setIsLoading(false);
      setError(null);
    ...
    // FETCH MODE: fallback for Credit panel and other consumers without items
  ...
  return (
    <>
      ...
      {/* Sheet */}
      <div 
        className="fixed right-0 top-0 h-full w-[480px] max-w-full z-[60] 
            ...
            <div className="p-5">
              ...
              <div className="mb-4">
                ...
                <div className="bg-zinc-50 rounded-lg p-3 space-y-2">
                  ...
                  {/* BUG-272: Partial payment leg badges */}
              ...
              {/* Order Note */}
              {displayData.orderNote && (
                <div className="mt-4" data-testid="audit-detail-order-note">
                  <div className="text-xs font-semibold uppercase tracking-wider text-zinc-400 mb-2">
                    Order Note
                  </div>
                  <div className="bg-amber-50 rounded-lg p-3 text-sm text-amber-800 italic">
                    "{displayData.orderNote}"
                  </div>
                </div>
              )}

              {/* Activity Log — from operations[] */}
              {displayData.operations?.length > 0 && (
                <div className="mt-4" data-testid="audit-detail-activity-log">
                  <div className="text-xs font-semibold uppercase tracking-wider text-zinc-400 mb-2">
                    Activity Log
                  </div>
                  <div className="bg-zinc-50 rounded-lg p-3 space-y-2">
                    {displayData.operations.map((op, idx) => {
                      const diff = op.current_order_amount && op.previous_order_amount && op.current_order_amount !== op.previous_order_amount
                        ? parseFloat(op.current_order_amount) - parseFloat(op.previous_order_amount)
                        : null;
                      return (
                        <div key={idx} className="flex items-start gap-2 text-xs py-1.5 border-b border-zinc-100 last:border-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              <span className="font-medium text-zinc-700 truncate">
                                ...
                                {op.itemName && <span className="text-zinc-500 font-normal"> — {op.itemName}</span>}
                              </span>
                              <span className="font-mono text-zinc-500 flex-shrink-0">
                                {formatTime(op.created_at)}
                              </span>
                            </div>
                            {op.vendor_employee_name && (
                              <div className="text-zinc-500 mt-0.5">
                                by <span className="font-medium text-zinc-600">{op.vendor_employee_name}</span>
                              </div>
                            )}
                            {op.current_payment_method && op.previous_payment_method && op.current_payment_method !== op.previous_payment_method && (
                              <div className="text-zinc-400 mt-0.5">
                                {op.previous_payment_method} &rarr; {op.current_payment_method}
                              </div>
                            )}
                            {diff !== null && (
                              <div className="text-zinc-400 mt-0.5">
                                {formatCurrency(op.previous_order_amount)} &rarr; {formatCurrency(op.current_order_amount)}
                                <span className={`ml-1 font-medium ${diff < 0 ? 'text-red-500' : 'text-emerald-500'}`}>
          ...
          <div className="border-t border-zinc-200 p-5 bg-zinc-50">
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-zinc-500">Item Total ({displayData.itemCount || 0} items)</span>
                <span className="font-mono text-zinc-700">{formatCurrency(displayData.itemTotal)}</span>
              </div>
              {/* 2. Discount / Coupon */}
              <div className="flex justify-between text-sm" data-testid="audit-detail-discount">
                <span className="text-zinc-500">Discount</span>
                <span className="font-mono text-red-600">{displayData.discountAmount > 0 ? `-${formatCurrency(displayData.discountAmount)}` : formatCurrency(0)}</span>
              </div>
              {displayData.couponCode && (
                <div className="flex justify-between text-sm" data-testid="audit-detail-coupon">
                  <span className="text-zinc-500">Coupon ({displayData.couponCode})</span>
                  <span className="font-mono text-red-600">-{formatCurrency(displayData.couponAmount)}</span>
                </div>
              )}
              {/* 3. Service Charge, Delivery Charge, Tip — before subtotal */}
              {displayData.serviceChargeAmount > 0 && (
                <div className="flex justify-between text-sm" data-testid="audit-detail-service-charge">
                  <span className="text-zinc-500">Service Charge</span>
                  <span className="font-mono text-zinc-700">{formatCurrency(displayData.serviceChargeAmount)}</span>
                </div>
              )}
              {Number(displayData.deliveryCharge) > 0 && (
                <div className="flex justify-between text-sm" data-testid="audit-detail-delivery-charge">
                  <span className="text-zinc-500">Delivery Charge</span>
                  <span className="font-mono text-zinc-700">{formatCurrency(displayData.deliveryCharge)}</span>
                </div>
              )}
              {displayData.tipAmount > 0 && (
                <div className="flex justify-between text-sm" data-testid="audit-detail-tip">
                  <span className="text-zinc-500">Tip</span>
                  <span className="font-mono text-zinc-700">{formatCurrency(displayData.tipAmount)}</span>
                </div>
              )}
              {/* 4. Subtotal — border above */}
              ...
              {/* 5. Tax + Round-off — only these after subtotal */}
              {displayData.gstAmount > 0 && (
                <div className="flex justify-between text-sm" data-testid="audit-detail-gst">
                  <span className="text-zinc-500">GST</span>
                  <span className="font-mono text-zinc-700">{formatCurrency(displayData.gstAmount)}</span>
                </div>
              )}
              {displayData.vatAmount > 0 && (
                <div className="flex justify-between text-sm" data-testid="audit-detail-vat">
                  <span className="text-zinc-500">VAT</span>
                  <span className="font-mono text-zinc-700">{formatCurrency(displayData.vatAmount)}</span>
                </div>
              )}
              {displayData.roundOff !== 0 && (
                <div className="flex justify-between text-sm" data-testid="audit-detail-roundoff">
                  <span className="text-zinc-500">Round-off</span>
                  <span className="font-mono text-zinc-700">{formatCurrency(displayData.roundOff)}</span>
                </div>
              )}
              {/* 6. Grand Total — border above */}
            ...
            {displayData.paymentStatus === 'paid' && !displayData.isCancelled && !effectiveIsMerged && (() => {
              ...
              return isCredit ? (
                ...
                <div className="mt-3 flex items-center justify-center gap-2 py-2 bg-emerald-100 text-emerald-700 rounded-lg text-sm font-medium">
                  <CreditCard className="w-4 h-4" />
                  Settled via {formatPaymentMethod(displayData.paymentMethod)}
                </div>
              );
            })()}
