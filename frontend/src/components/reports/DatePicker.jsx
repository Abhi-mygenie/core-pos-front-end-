...
/**
 * Format date for API (YYYY-MM-DD) — timezone safe
 */
 ...
 * @param {boolean} [disabled] - When true, the trigger and prev/next day
 *   buttons are inert and visually de-emphasised. The dropdown cannot open.
 *   Used by the Room Orders Report when the active filter pill is "Unpaid"
 *   (live in-house rooms — date does not apply).
 * @param {string} [tooltip] - Hover tooltip surfaced on the trigger button.
 *   Honoured both when `disabled` and when active (e.g., the "All" filter
 *   uses an active date picker but a clarifying tooltip).
 */
const DatePicker = ({ value, onChange, disabled = false, tooltip = '' }) => {
  ...
  const handleDateSelect = (day) => {
    ...
    if (selected > todayDate) return; // Block future dates
  ...
  return (
    <div className="relative flex items-center gap-1" ref={containerRef}>
      ...
      <button
        ...
        className={`p-1.5 rounded transition-colors ${disabled ? 'opacity-30 cursor-not-allowed' : 'hover:bg-zinc-100'}`}
        title={disabled ? tooltip : 'Previous day'}
      >
      ...
      <button
        ...
        className={`
          ...
          ${isOpen && !disabled ? 'border-zinc-950 ring-1 ring-zinc-950' : ''}
        `}
      >
      ...
      <button
        ...
        className={`p-1.5 rounded transition-colors ${(disabled || isToday) ? 'opacity-30 cursor-not-allowed' : 'hover:bg-zinc-100'}`}
        title={disabled ? tooltip : (isToday ? "Can't go to future" : 'Next day')}
      >
        ...
        >
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className={`p-1 rounded-sm transition-colors ${viewDate.getFullYear() === new Date().getFullYear() && viewDate.getMonth() >= new Date().getMonth() ? 'opacity-30 cursor-not-allowed' : 'hover:bg-zinc-100'}`}
