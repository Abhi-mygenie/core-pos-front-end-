...
const AddressFormModal = ({ onClose, onSave, initialData = null, saving = false, initialDeliveryCharge = 0 }) => {
  ...
  // CR-008 / Bucket D1-Cap (May-2026, delivery-charge capture): per-order charge
  // captured inside this modal (Phase 2 will auto-populate from Google Distance API).
  // Not persisted on the saved-address record — passed to parent via onSave(form, charge).
  const [chargeInput, setChargeInput] = useState(
    initialDeliveryCharge ? String(initialDeliveryCharge) : ''
  );
  ...
  const handleSave = () => {
    ...
    // CR-008 / Bucket D1-Cap (May-2026): emit charge alongside the address form.
    // OrderEntry.handleAddAddress(formData, newCharge) consumes this second arg.
    const parsedCharge = parseFloat(chargeInput);
    onSave(form, isNaN(parsedCharge) ? 0 : parsedCharge);
  };
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center" data-testid="address-form-modal">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {/* CR-008 / Bucket D1-Cap (May-2026, delivery-charge capture):
              per-order delivery charge captured inside the address modal so
              Phase 2 Google-Distance-API integration drops in here. Not saved
              as an address-record field; returned to OrderEntry via onSave. */}
          <div style={{ borderTop: `1px solid ${COLORS.borderGray}`, paddingTop: '12px' }}>
            <label className="text-xs font-medium mb-1.5 block" style={{ color: COLORS.grayText }}>
              Delivery Charge (₹) <span className="font-normal" style={{ color: COLORS.grayText }}>· per this order</span>
            </label>
            <input
              type="number"
              inputMode="decimal"
              ...
              className="w-full px-3 py-2.5 rounded-xl text-sm border focus:outline-none focus:ring-2"
              style={{ borderColor: COLORS.borderGray }}
              data-testid="address-form-delivery-charge"
            />
            <p className="text-[10px] mt-1 px-1" style={{ color: COLORS.grayText }}>
              Phase 2 will auto-compute from distance. Editable override.
            </p>
          </div>
        </div>
