...
import { ChevronLeft, CreditCard, Smartphone, Banknote, Split, FileText, Check, ArrowRightLeft, ChevronDown, ChevronUp, BellRing, RefreshCw, MoreHorizontal, Printer } from "lucide-react";
...
const CollectPaymentPanel = ({ 
  ...
  // Service charge on food subtotal (before discount, after items) — BUG-276: toggle-able per order
  ...
  // BUG-281: Subtotal = pre-tax total = (itemTotal − totalDiscount) + serviceCharge + tip
  //   itemTotal      = gross items
  //   totalDiscount  = manual + preset + loyalty + coupon + wallet (all visible discounts)
  //   serviceCharge  = live value honoring BUG-276 toggle
  //   tip            = user-entered, 0 when feature flag off
  ...
  // BUG-281 (Option A, Gap-4 print-path): Compute discount-adjusted tax for the
  // bill print payload ONLY. The UI sgst/cgst and paymentData.sgst/cgst remain on
  // gross tax (unchanged) to avoid backend regression until clarification Q-6.
  // Approach (b): per-item prorated discount + SC GST at weighted-avg rate.
  const printTaxTotals = useMemo(() => {
    const discountRatio = itemTotal > 0 ? totalDiscount / itemTotal : 0;
    let gst = 0, vat = 0, grossGstSum = 0;
    activeItems.forEach(it => {
      const tax = it.tax;
      if (!tax || !tax.percentage) return;
      const linePrice = getItemLinePrice(it);
      const grossTax = tax.isInclusive
        ? linePrice - (linePrice / (1 + tax.percentage / 100))
        : linePrice * (tax.percentage / 100);
      const netTax = grossTax * (1 - discountRatio);
      if ((tax.type || 'GST').toUpperCase() === 'VAT') vat += netTax;
      else { gst += netTax; grossGstSum += grossTax; }
    });
    if (serviceCharge > 0 && itemTotal > 0 && grossGstSum > 0) {
      const avgGstRate = grossGstSum / itemTotal;
      gst += serviceCharge * avgGstRate;
    }
    return {
      gst: Math.round(gst * 100) / 100,
      vat: Math.round(vat * 100) / 100,
    };
  }, [activeItems, totalDiscount, itemTotal, serviceCharge]);
  ...
  const handlePayment = () => {
    ...
    const paymentData = {
      ...
      serviceCharge,
      deliveryCharge,
      // BUG-281 (Option A): pre-computed discount-adjusted tax for the print payload only.
      // UI sgst/cgst and API paymentData.sgst/cgst stay gross to avoid backend regression.
      printGstTax: printTaxTotals.gst,
      printVatTax: printTaxTotals.vat,
    };
  ...
  // BUG-277 + BUG-281: Manual "Print Bill" — sends current CollectPaymentPanel values
  // as overrides so printout reflects live discounts / service-charge toggle /
  // delivery charge / tip before payment is collected.
  // Option A (Feb-2026): print payload `gst_tax` is discount-adjusted (Approach b,
  // sourced from printTaxTotals memo), but UI SGST/CGST rows + paymentData stay on
  // gross tax to avoid backend regression until backend clarification Q-6 lands.
  const handlePrintBill = async () => {
    ...
    try {
      ...
      const overrides = {
        ...
        deliveryCharge,
        gstTax:              printTaxTotals.gst,     // BUG-281 Gap-4 (print-only)
        vatTax:              printTaxTotals.vat,
    ...
    <div className="flex flex-col h-full" data-testid="collect-payment-panel">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        >
                        ...
                        <div className="flex gap-2 flex-1 justify-end">
                          ...
                          {discountType && (
                            <input
                              ...
                              value={discountValue}
                              onChange={(e) => setDiscountValue(e.target.value < 0 ? "" : e.target.value)}
                              min="0"
          ...
          {/* BUG-269: Delivery Charge — only for delivery orders */}
          {orderType === 'delivery' && (
            <div className="mt-3 pt-3 border-t" style={{ borderColor: COLORS.borderGray }} data-testid="delivery-charge-section">
              <div className="text-xs font-medium uppercase tracking-wide mb-2" style={{ color: COLORS.grayText }}>
                Delivery Charge
              </div>
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium" style={{ color: COLORS.darkText }}>₹</span>
                <input
                  type="number"
                  placeholder="0"
                  ...
                  min="0"
                  className="flex-1 px-3 py-2 rounded-lg border text-sm outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
          ...
          {/* BUG-281: Tip — flat ₹, gated by restaurant.features.tip */}
          {tipEnabled && (
            <div className="mt-3 pt-3 border-t" style={{ borderColor: COLORS.borderGray }} data-testid="tip-section">
              <div className="text-xs font-medium uppercase tracking-wide mb-2" style={{ color: COLORS.grayText }}>
                Tip
              </div>
              <div className="flex items-center gap-2">
