...
import { itemLevelPresets } from "../../data";

const ItemNotesModal = ({ item, onClose, onSave, initialNotes = [], customerId = null, customerIntel = null }) => {
  const [selectedNotes, setSelectedNotes] = useState(initialNotes);
  const [customNote, setCustomNote] = useState("");

  // Get customer preferences — CR-002: from CRM intel data (replaces mock)
  const hasCustomer = !!customerIntel;
  const customerData = (() => {
    ...
    return {
      ...
      preferences: notes.map(n => ({
        note: `${n.text}  (${n.usedCount}x · ${n.relativeTime})`,
        isAlert: false,
        source: n.source,
      })),
    };
  ...
  const togglePreset = (preset) => {
    setSelectedNotes((prev) => {
      ...
      return [...prev, { id: preset.id, label: preset.label, type: "preset" }];
    });
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center" data-testid="item-notes-modal">
      {/* Backdrop */}
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 max-h-[85vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: COLORS.grayText }}>
                {item?.name || "Item"} - Cooking instructions
              </p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          {/* Customer Preferences Section — shown FIRST when customer is tagged */}
          {customerData ? (
            <div>
              <label className="text-xs font-medium mb-2 block uppercase tracking-wide" style={{ color: COLORS.grayText }}>
                👤 Customer Preferences ({customerData.name})
              </label>
              <div className="space-y-2">
                {customerData.preferences.map((pref, idx) => (
                  <button
                    ...
                    style={{
                      backgroundColor: pref.isAlert ? "#fef2f215" : COLORS.sectionBg,
                      border: `1px solid ${pref.isAlert ? "#ef4444" : COLORS.borderGray}`,
                    }}
                  >
                    {pref.isAlert ? (
                    ...
                    <span style={{ color: pref.isAlert ? "#ef4444" : COLORS.darkText }}>{pref.note}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              <label className="text-xs font-medium mb-2 block uppercase tracking-wide" style={{ color: COLORS.grayText }}>
                👤 Customer Preferences ({customerIntel.customerSummary.name})
              </label>
              <div
                className="p-4 rounded-xl text-center text-sm"
                style={{ backgroundColor: COLORS.sectionBg, color: COLORS.grayText }}
              >
                No preferences found for this item.
              </div>
            </div>
          ) : null}
