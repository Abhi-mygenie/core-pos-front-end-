      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2" style={{ backgroundColor: COLORS.sectionBg }}>
          ...
          <div className="p-4 rounded-xl border" style={{ backgroundColor: '#ffffff', borderColor: COLORS.borderGray }}
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                {freeTables.map(t => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-2 h-2 rounded-full mb-1" style={{ backgroundColor: sel ? COLORS.primaryGreen : '#D1D5DB' }} />
                      <span className="text-sm font-bold" style={{ color: sel ? '#16A34A' : COLORS.darkText }}>{t.label}</span>
                      <span className="text-xs" style={{ color: sel ? '#16A34A' : COLORS.grayText }}>Free</span>
                    </button>
                  );
                })}
              </div>
            ...
            {/* Path B: create "Room r2" table row */}
            {roomNo && (
              <button
                ...
                style={{
                  backgroundColor: destination?.type === 'create' ? '#ECFDF5' : '#FFF7ED',
                  border: destination?.type === 'create'
                    ? `2px solid ${COLORS.primaryGreen}`
                    : `2px dashed ${COLORS.primaryOrange}`,
                  boxShadow: destination?.type === 'create' ? '0 0 0 3px rgba(50,153,55,.15)' : 'none',
                }}
                data-testid="destination-create-room-table-row"
              >
                <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0"
                  style={{ backgroundColor: destination?.type === 'create' ? '#DCFCE7' : '#FFEDD5' }}>
                  <PlusCircle className="w-4 h-4"
                    style={{ color: destination?.type === 'create' ? COLORS.primaryGreen : COLORS.primaryOrange }} />
                </div>
                <div className="flex-1 text-left">
                  <div className="text-sm font-bold"
                    style={{ color: destination?.type === 'create' ? '#15803D' : '#C2410C' }}>
                    Create &quot;Room {roomNo}&quot; table
                  </div>
                  <div className="text-xs"
                    style={{ color: destination?.type === 'create' ? '#16A34A' : COLORS.primaryOrange }}>
                    Auto-creates a dine-in table on the board
                  </div>
                </div>
                <span className="text-xs font-bold px-2 py-0.5 rounded-full flex-shrink-0"
                  style={{
                    backgroundColor: destination?.type === 'create' ? '#DCFCE7' : '#FFEDD5',
                    color: destination?.type === 'create' ? '#15803D' : '#C2410C',
                  }}>
                  Dynamic
                </span>
              </button>
            ...
            {/* Empty state */}
            {freeTables.length === 0 && !roomNo && (
              <p className="text-xs text-center py-3" style={{ color: COLORS.grayText }}>No destination available</p>
            )}
          </div>
