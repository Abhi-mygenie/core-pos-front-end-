  ...
  useEffect(() => {
    const fetchTables = async () => {
      try {
        ...
        // Filter: only free tables/rooms matching source type (table→table, room→room)
    ...
    fetchTables();
  }, [currentTable?.tableId, currentTable?.isRoom]);
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[85vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-xl font-bold" style={{ color: COLORS.darkText }}>
                {currentTable?.label || currentTable?.id} → Shift {currentTable?.isRoom ? 'Room' : 'Table'}
              </h2>
              <p className="text-sm mt-1" style={{ color: COLORS.grayText }}>
                Select a free {currentTable?.isRoom ? 'room' : 'table'} to shift to
              </p>
            </div>
        ...
        <div className="p-4 border-t" style={{ borderColor: COLORS.borderGray, backgroundColor: COLORS.sectionBg }}>
          {selectedTable ? (
            ...
            <div className="text-sm mb-3" style={{ color: COLORS.grayText }}>Select a free {currentTable?.isRoom ? 'room' : 'table'} to shift to</div>
          )}
