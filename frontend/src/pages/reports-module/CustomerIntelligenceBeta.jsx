...
import { Users, TrendingUp, Star, RotateCcw, FileDown } from 'lucide-react';
import * as XLSX from 'xlsx'; // BUG-191: Excel export
import jsPDF from 'jspdf'; // BUG-191: PDF export
import autoTable from 'jspdf-autotable'; // BUG-191: PDF table
...
const CustomerIntelligenceBeta = () => {
  ...
  // BUG-191: export state
  const [exportingTop, setExportingTop]     = useState(false);
  const [exportingChurn, setExportingChurn] = useState(false);

  // BUG-191: export helpers
  ...
  // BUG-191: Top Customers export handler (limit=200 for full export)
  const handleExportTop = async (format) => {
    setExportingTop(true);
    try {
      const res = await getTopCustomers(sortBy, 200);
      const rows = (res?.customers || []).map((c, i) => ({
        '#': i + 1,
        Name: c.name || '—',
        Phone: c.phone || '—',
        Tier: c.tier || '—',
        ...
        'Avg Order': c.avg_order_value ?? 0,
        'Last Visit': c.last_visit_days_ago != null ? `${c.last_visit_days_ago}d ago` : '—',
      }));
      if (format === 'excel') exportToExcel(rows, 'top-customers');
      else exportToPdf(rows, 'Top Customers', 'top-customers');
    } catch (_e) { /* silent — export failure is non-blocking */ }
    finally { setExportingTop(false); }
  };

  // BUG-191: Win-Back export handler (limit=200 for full export)
  const handleExportChurn = async (format) => {
    setExportingChurn(true);
    try {
      const res = await getChurnRisk(churnBand, 200);
      const rows = (res?.customers || []).map((c, i) => ({
        '#': i + 1,
        Name: c.name || '—',
        Phone: c.phone || '—',
        Tier: c.tier || '—',
        'Days Since Visit': c.last_visit_days_ago != null ? c.last_visit_days_ago : '—',
        'Total Spent': c.total_spent ?? 0,
        Visits: c.total_visits ?? '—',
      }));
      const label = churnBand === 'high' ? 'at-risk' : 'dormant';
      if (format === 'excel') exportToExcel(rows, `winback-${label}`);
      else exportToPdf(rows, `Win-Back — ${label === 'at-risk' ? 'At Risk' : 'Dormant'}`, `winback-${label}`);
    } catch (_e) { /* silent */ }
    finally { setExportingChurn(false); }
  };
              ...
              <>
                ...
                <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden" data-testid="customer-intel-beta-top-customers">
                  <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {/* BUG-191: Export buttons — Top Customers */}
                      <button onClick={() => handleExportTop('excel')} disabled={exportingTop}
                        className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-lg border border-zinc-200 text-zinc-600 hover:bg-zinc-50 disabled:opacity-40 transition-colors"
                        data-testid="export-top-excel">
                        <FileDown className="w-3.5 h-3.5" />Excel
                      </button>
                      <button onClick={() => handleExportTop('pdf')} disabled={exportingTop}
                        className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-lg border border-zinc-200 text-zinc-600 hover:bg-zinc-50 disabled:opacity-40 transition-colors"
                        data-testid="export-top-pdf">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        <thead className="bg-zinc-50"><tr>
                          ...
                          <th className="px-4 py-3 text-left text-[10px] font-semibold text-zinc-400 uppercase">Phone</th>{/* BUG-191 */}
                        ...
                        <tbody>
                          {(topData?.customers || []).map((c, i) => (
                            <tr key={c.customer_id} className="border-b border-zinc-50 hover:bg-zinc-50 transition-colors" data-testid={`top-customer-row-${i}`}>
                              ...
                              <td className="px-4 py-3 text-sm font-medium text-zinc-800">{c.name || '—'}</td>{/* BUG-191 */}
                              <td className="px-4 py-3 text-sm text-zinc-500">{c.phone || '—'}</td>{/* BUG-191 */}
                          ...
                          {!topData?.customers?.length && <tr><td colSpan={8} className="py-12 text-center text-zinc-400 text-sm">No customers found</td></tr>}
                        </tbody>
                      </table>
                    </div>
                  )}
                ...
                <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden" data-testid="customer-intel-beta-winback">
                  <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {/* BUG-191: Export buttons — Win-Back */}
                      <button onClick={() => handleExportChurn('excel')} disabled={exportingChurn}
                        className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-lg border border-zinc-200 text-zinc-600 hover:bg-zinc-50 disabled:opacity-40 transition-colors"
                        data-testid="export-churn-excel">
                        <FileDown className="w-3.5 h-3.5" />Excel
                      </button>
                      <button onClick={() => handleExportChurn('pdf')} disabled={exportingChurn}
                        className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-lg border border-zinc-200 text-zinc-600 hover:bg-zinc-50 disabled:opacity-40 transition-colors"
                        data-testid="export-churn-pdf">
                        <FileDown className="w-3.5 h-3.5" />PDF
                      </button>
                      ...
                      <tbody>
                        {(activeChurn?.customers || []).map((c, i) => (
                          <tr key={c.customer_id} className="border-b border-zinc-50 hover:bg-zinc-50 transition-colors" data-testid={`winback-row-${i}`}>
                            <td className="px-5 py-3 text-sm font-medium text-zinc-800">{c.name || '—'}</td>{/* BUG-191 */}
                            <td className="px-4 py-3 text-sm text-zinc-500">{c.phone || '—'}</td>{/* BUG-191 */}
                            <td className="px-4 py-3">{tierBadge(c.tier)}</td>
                        ...
                        {!activeChurn?.customers?.length && !churnLoading && (
                          <tr><td colSpan={7} className="py-12 text-center text-zinc-400 text-sm">No customers in this band</td></tr>
                        )}
                      </tbody>
                    </table>
                  )}
