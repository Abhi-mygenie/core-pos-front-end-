 ...
 * Groups raw payment_method into canonical buckets.
 * Razorpay is a gateway — resolves to Card or UPI based on hints in the raw string.
 * Any unrecognized method goes into "Other". */
/* Title-case a snake_case/lowercase string: "zomato_gold" → "Zomato Gold" */
const titleCase = (s) => s.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());

const classifyPaymentMethod = (raw) => {
  const pm = (raw || '').toLowerCase().trim();
  if (pm === 'cash' || pm === 'cash_on_delivery') return 'Cash';
  if (pm === 'card' || pm === 'credit_card' || pm === 'debit_card') return 'Card';
  if (pm === 'upi' || pm === 'gpay' || pm === 'phonepe' || pm === 'paytm') return 'UPI';
  if (pm === 'tab' || pm === 'transfertoroom') return 'TAB';
  if (pm.includes('razorpay') || pm.includes('razor')) {
  ...
  // Known normalized labels
  if (pm === 'zomato_gold') return 'Zomato Gold';
  // Skip non-payment statuses
  if (!pm || pm === 'cancelled' || pm === 'cancel' || pm === 'merge') return null;
  // Unknown method — show as title-cased raw value
  return titleCase(pm);
};
...
const METHOD_ORDER = ['Cash', 'Card', 'UPI', 'TAB', 'Partial', 'Zomato Gold'];
...
const PaymentsMockup = () => {
  ...
  const analytics = useMemo(() => {
    ...
    // Only paid/settled orders (fOrderStatus === 6)
    const revenueOrders = rawOrders.filter((o) => o.fOrderStatus === 6);
    const totalRevenue = revenueOrders.reduce((s, r) => s + (r.totalAmount || 0), 0);
    ...
    revenueOrders.forEach((o) => {
      ...
      if (!bucket) return; // skip cancelled/merge/null
    ...
    const tabRevenue = methodMap['TAB']?.revenue || 0;

    // ── Daily breakdown with per-method split ──
    const dailyMap = {};
    revenueOrders.forEach((o) => {
  ...
  const buildExportPayload = () => {
    ...
    return {
      ...
      kpis: [
        ...
        { label: 'TAB / Credit', value: analytics.tabRevenue, tone: 'bad', format: 'inr' },
      ...
      sheets: [
        ...
        },
        {
          ...
          columns: [
            ...
            { key: 'tab', label: 'TAB', format: 'inr', align: 'right', width: 100 },
          ],
              ...
              {analytics && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-4 gap-4" data-testid="payments-kpi-strip">
                    ...
                    <div className="bg-white border border-zinc-200 rounded-xl p-5">
                      <div className="flex items-center gap-2 mb-2"><Wallet className="w-4 h-4 text-[#8B5CF6]" /><span className="text-xs font-medium text-zinc-500 uppercase tracking-wide">TAB / Credit</span></div>
