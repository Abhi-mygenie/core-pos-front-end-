...
import { fetchInsightsSales } from '../../api/services/insightsService'; // CR-049: backend aggregation
// CR-049: kept for reference — import { getOrderLedgerForRange, getRevenueOrdersForRange, getTabSettlementsForRange, REVENUE_BASIS } from '../../api/services/orderLedgerService';
...
const PaymentsMockup = () => {
  ...
  // CR-049: salesData holds the backend response directly
  ...
  // CR-049: Fetch from backend insights-sales endpoint (replaces FE aggregation)
  ...
  /* ── Aggregations (CR-049: transformed from backend response) ── */
  const analytics = useMemo(() => {
    if (!salesData) return null;
    const s = salesData.data?.summary || salesData.summary || {}; // BUG-194: data access fix (CR-049 regression)
    const totalRevenue = s.total_revenue || 0;
    const totalOrders = s.total_orders || 0;
    const settlementTotal = s.tab_settlement_total || 0;

    // Payment methods from backend
    const backendPayments = salesData.data?.payments || salesData.payments || []; // BUG-194
    let colorIdx = 0;
    const methods = METHOD_ORDER
      .filter(m => backendPayments.find(p => p.method === m))
      .map(m => {
        const p = backendPayments.find(bp => bp.method === m);
        return {
          method: m,
          revenue: p.revenue || 0,
          orders: p.orders || 0,
          pct: totalRevenue > 0 ? ((p.revenue || 0) / totalRevenue * 100).toFixed(1) : '0',
          avg: p.orders > 0 ? p.revenue / p.orders : 0,
          color: getMethodColor(m, colorIdx++),
        };
      });
    // Add extras not in METHOD_ORDER
    backendPayments
      .filter(p => !METHOD_ORDER.includes(p.method))
      ...
      .forEach(p => {
        methods.push({
          ...
          pct: totalRevenue > 0 ? ((p.revenue || 0) / totalRevenue * 100).toFixed(1) : '0',
          avg: p.orders > 0 ? p.revenue / p.orders : 0,
          color: getMethodColor(p.method, colorIdx++),
        });
      });

    const allMethodNames = methods.map(m => m.method);
    const cashRevenue = methods.find(m => m.method === 'Cash')?.revenue || 0;
    const digitalRevenue = (methods.find(m => m.method === 'Card')?.revenue || 0) + (methods.find(m => m.method === 'UPI')?.revenue || 0);
    const tabRevenue = settlementTotal;

    // F-6: Daily charts removed but keep daily array for the breakdown table + export
    const daily = (salesData.data?.daily || salesData.daily || []).map(d => ({ // BUG-194
      date: d.date || '',
      total: d.revenue || 0,
      orders: d.orders || 0,
    }));
    ...
    const stackMethods = allMethodNames.filter(m => methods.find(mt => mt.method === m && mt.revenue > 0));
    ...
    return {
      ...
      daily, trend: [],
      topCashDay: null, topDigitalDay: null,
  ...
  return (
    <div className="flex h-screen bg-white font-sans" data-testid="payments-page">
      <Sidebar isExpanded={isSidebarExpanded} setIsExpanded={(v) => { setIsSidebarExpanded(v); localStorage.setItem('mygenie_sidebar_expanded', String(v)); }} onOpenSettings={() => {}} onOpenMenu={() => {}} onOpenCredit={() => {}} onRefresh={fetchData} isRefreshing={isLoading} isOrderEntryOpen={false} />
            ...
            <div className="flex-1 overflow-auto p-8">
              {analytics && (
                <div className="space-y-6">
                  ...
                  {/* F-6: Daily Payment Trends + Cash vs Digital charts removed — owner directive 2026-06-17 */}
