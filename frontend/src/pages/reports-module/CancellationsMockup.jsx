...
const CancellationsMockup = () => {
  ...
  const { data: rawOrders, isLoading, error, hasLoadedOnce, refetch } = useReportFetch(
    async (signal) => {
      ...
      const raw = resp.data?.order || [];
      const { start: dayStart } = getBusinessDayRange(appliedFrom, schedules);
      const { end: dayEnd } = getBusinessDayRange(appliedTo, schedules);
      return raw.filter((o) => {
        const ca = (o.orders_table?.created_at || '').replace('T', ' ').substring(0, 19);
        if (!ca) return false;
        if (appliedFrom === appliedTo) return isWithinBusinessDay(ca, dayStart, dayEnd);
  ...
  /* FIX 3B: Revenue formula aligned with insightsService.js (unitPrice*qty + addon + variation - discount + serviceCharge + tax) */
  const cancelledItems = useMemo(() => {
    ...
    for (const o of orders) {
      ...
      for (const item of (o.order_details_table || [])) {
        ...
        // Audited revenue formula (same as insightsService.js lines 89-116)
        const qty = parseFloat(item.quantity) || 0;
        const unitPrice = parseFloat(item.unit_price) || 0;
        const addonPrice = parseFloat(item.total_add_on_price) || 0;
        const variationPrice = parseFloat(item.total_variation_price) || 0;
        const discount = parseFloat(item.discount_on_food) || 0;
        const serviceCharge = parseFloat(item.service_charge) || 0;
        // VAT-FIX (2026-06-06): gst_tax_amount holds total tax (GST+VAT). Derive pure GST.
        const rawGst = parseFloat(item.gst_tax_amount) || 0;
        const vat = parseFloat(item.vat_tax_amount) || 0;
        ...
        items.push({
          orderNumber: roid,
          orderDate: dateOnly(item.cancel_at || item.created_at),
  ...
  const analytics = useMemo(() => {
    ...
    const totalLoss = cancelledItems.reduce((s, i) => s + i.amount, 0);
    const orderLoss = orderItems.reduce((s, i) => s + i.amount, 0);
    const itemLoss = itemItems.reduce((s, i) => s + i.amount, 0);

    const cancelledOrderNumbers = new Set(orderItems.map(i => i.orderNumber));
    const cancelledOrderCount = cancelledOrderNumbers.size;
    ...
    cancelledItems.forEach((i) => {
      ...
      if (i.scope === 'order') { dailyMap[date].orderCancels += i.qty; dailyMap[date].orderLoss += i.amount; }
