...
const ExpenseReportPage = () => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getExpenseAggregation(formatDateDDMMYYYY(appliedFrom), formatDateDDMMYYYY(appliedTo))
      .then((data) => { if (!cancelled) setServerAgg(data); })
      .catch(() => { if (!cancelled) setServerAgg(null); });
    return () => { cancelled = true; };
  }, [appliedFrom, appliedTo]);

  // Aggregated data — CR-062: prefer server-side, fallback to client-side
  const aggregated = useMemo(() => {
    ...
    // Fallback: client-side aggregation (CR-061)
                    ...
                    <table className="w-full" data-testid="expense-report-table">
                      ...
                      <tbody>
                        {filteredTransactions.length === 0 && (
                          <tr><td colSpan={11} className="px-5 py-10 text-center text-sm text-zinc-400">{debouncedSearch.trim() ? `No results match "${debouncedSearch}".` : 'No transactions found for this period.'}</td></tr>
                        )}
                      ...
                      {filteredTransactions.length > 0 && (
                        <tfoot className="bg-zinc-50 border-t-2 border-zinc-200">
                          <tr>
                            ...
                            <td colSpan={5} />{/* CR-087: 5+1+5=11 cols */}
                          </tr>
                        </tfoot>
                      )}
                    </table>
