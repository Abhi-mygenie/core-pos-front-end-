/**
 * CashierSettlementMockup — CR-011 S19 (Phase 3, Batch I)
 * Cashier Settlement: per-cashier collection breakdown with payment method split.
 * Data source: insights-staff.by_cashier[] + insights-sales.payments[]
 * Gate ①+④: Mockup with live API data.
 * 🟡 BE-1 caveat: partial payment leg accuracy approximate.
 */
...
import { ArrowLeft, CalendarIcon, Check, Download, FileSpreadsheet, FileDown, Mail, MessageCircle, Send, Banknote, CreditCard, Smartphone, User } from 'lucide-react';
...
const DOWNLOAD_MENU = [{ id: 'excel', label: 'Download as Excel', icon: FileSpreadsheet, enabled: true, testId: 's19-download-excel-btn' }, { id: 'pdf', label: 'Download as PDF', icon: FileDown, enabled: true, testId: 's19-download-pdf-btn' }, { id: 'email', label: 'Send via Email', icon: Mail, enabled: false, testId: 's19-share-email-btn' }, { id: 'whatsapp', label: 'Send via WhatsApp', icon: MessageCircle, enabled: false, testId: 's19-share-whatsapp-btn' }, { id: 'sms', label: 'Send via SMS', icon: Send, enabled: false, testId: 's19-share-sms-btn' }];

const CashierSettlementMockup = () => {
  ...
  const analytics = useMemo(() => {
    ...
    const cashiers = (rawData.by_cashier || []).map(c => ({ ...c, total: (c.cash_collected||0) + (c.card_collected||0) + (c.upi_collected||0) })).sort((a, b) => b.total - a.total);
    ...
    return { cashiers, totalCash, totalCard, totalUpi, totalCollected, totalOrders };
  }, [rawData]);

  const buildExportPayload = () => {
    if (!analytics) return null;
    return { title: 'Cashier Settlement', subtitle: 'Per-cashier payment collection', restaurant: { name: restaurant?.name || '' }, dateRange: { from: appliedFrom, to: appliedTo }, generatedBy: restaurant?.ownerName || '',
      ...
      sheets: [{ name: 'Settlement', columns: [{ key: 'name', label: 'Cashier', format: 'text', align: 'left', width: 150 }, { key: 'orders_processed', label: 'Orders', format: 'integer', align: 'right', width: 80 }, { key: 'cash_collected', label: 'Cash', format: 'inr', align: 'right', width: 110 }, { key: 'card_collected', label: 'Card', format: 'inr', align: 'right', width: 110 }, { key: 'upi_collected', label: 'UPI', format: 'inr', align: 'right', width: 110 }, { key: 'total', label: 'Total', format: 'inr', align: 'right', width: 120 }], rows: analytics.cashiers, totals: { name: 'TOTAL', orders_processed: analytics.totalOrders, cash_collected: analytics.totalCash, card_collected: analytics.totalCard, upi_collected: analytics.totalUpi, total: analytics.totalCollected } }] };
  };
  ...
  return (
    <div className="flex h-screen bg-white font-sans" data-testid="s19-cashier-settlement-page">
      ...
      <div className="flex-1 flex overflow-hidden">
        <main className="flex-1 flex flex-col overflow-hidden bg-zinc-50">
          <header className="flex items-center justify-between px-8 py-5 bg-white border-b border-zinc-200 shrink-0" data-testid="s19-header">
            <div className="flex items-center gap-4">
              <button className="p-2 hover:bg-zinc-100 rounded-lg" data-testid="s19-back-btn" onClick={() => navigate('/reports-module/payments')}><ArrowLeft className="w-5 h-5 text-zinc-600" /></button>
              <div><h1 className="text-2xl font-semibold tracking-tight text-zinc-950" style={{ fontFamily: "'Cabinet Grotesk', sans-serif" }}>Cashier Settlement</h1><p className="text-[11px] text-zinc-500 mt-0.5">Per-cashier payment collection · <span className="text-amber-600">partial split approximate</span>{' · '}<button onClick={() => navigate('/reports-module/definitions')} className="text-emerald-700 hover:underline font-medium">ⓘ Definitions</button></p></div>
            </div>
            <div className="flex items-center gap-3">
              <div className={`flex items-center gap-2 px-3 py-2 border ${draftDirty && draftValid ? 'border-[#F26B33]' : 'border-zinc-200'} bg-white rounded-lg`} data-testid="s19-daterange"><CalendarIcon className="w-4 h-4 text-zinc-500" /><label className="flex items-center gap-1.5 text-sm"><span className="text-xs text-zinc-400 uppercase">From</span><input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)} disabled={isLoading} max={fmtISO(today)} className="bg-transparent border-0 outline-none text-sm font-medium text-zinc-800 cursor-pointer focus:ring-0 p-0" data-testid="s19-date-from" /></label><span className="text-zinc-300">—</span><label className="flex items-center gap-1.5 text-sm"><span className="text-xs text-zinc-400 uppercase">To</span><input type="date" value={toDate} onChange={e => setToDate(e.target.value)} disabled={isLoading} max={fmtISO(today)} className="bg-transparent border-0 outline-none text-sm font-medium text-zinc-800 cursor-pointer focus:ring-0 p-0" data-testid="s19-date-to" /></label></div>
              <button onClick={handleApply} disabled={isLoading||!canApply} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium ${canApply ? 'bg-[#329937] text-white shadow-sm' : 'bg-zinc-100 text-zinc-400 cursor-not-allowed'}`} data-testid="s19-apply-btn"><Check className="w-4 h-4" /> Apply</button>
              <div className="flex items-center gap-1 px-1.5 py-1 bg-zinc-100 rounded-lg" data-testid="s19-presets">{['7D','30D','MTD'].map(p => (<button key={p} disabled={isLoading} className={`px-2.5 py-1 text-xs font-medium rounded-md ${activePreset === p ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-600 hover:bg-white/50'}`} onClick={() => handlePreset(p)}>{p}</button>))}</div>
              <div className="relative" ref={downloadRef}><button onClick={() => setShowDownloadMenu(v=>!v)} disabled={isLoading||!analytics} className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium border border-[#F26B33] text-[#F26B33] hover:bg-orange-50 shadow-sm ${isLoading||!analytics ? 'opacity-50 cursor-not-allowed' : ''}`} data-testid="s19-download-trigger"><Download className="w-4 h-4" /> Download</button>{showDownloadMenu && (<div className="absolute right-0 top-full mt-1 w-72 bg-white border border-zinc-200 rounded-xl shadow-lg z-50 overflow-hidden">{DOWNLOAD_MENU.map(item => (<button key={item.id} disabled={!item.enabled} onClick={() => item.enabled && handleDownloadAction(item.id)} className={`w-full flex items-center gap-3 px-4 py-3 text-left ${item.enabled ? 'hover:bg-zinc-50 text-zinc-800' : 'text-zinc-400 cursor-not-allowed'}`} data-testid={item.testId}><item.icon className={`w-4 h-4 ${item.enabled ? 'text-zinc-500' : 'text-zinc-300'}`} /><span className="text-sm font-medium">{item.label}</span></button>))}</div>)}</div>
            </div>
          </header>
          <ReportLoadingShield isLoading={isLoading} hasLoadedOnce={hasLoadedOnce} error={error} onRetry={fetchData}>
            <div className="flex-1 overflow-auto p-8">{analytics && (<div className="space-y-6">
              <div className="grid grid-cols-4 gap-4" data-testid="s19-kpi-strip">
                <div className="bg-white border border-zinc-200 rounded-xl p-5"><div className="flex items-center gap-2 mb-2"><User className="w-4 h-4 text-blue-500" /><span className="text-xs font-medium text-zinc-500 uppercase">Total Collected</span></div><div className="text-2xl font-bold text-zinc-950">{fmtINR(analytics.totalCollected)}</div></div>
                <div className="bg-white border border-zinc-200 rounded-xl p-5"><div className="flex items-center gap-2 mb-2"><Banknote className="w-4 h-4 text-emerald-500" /><span className="text-xs font-medium text-zinc-500 uppercase">Cash</span></div><div className="text-2xl font-bold text-zinc-950">{fmtINR(analytics.totalCash)}</div></div>
                <div className="bg-white border border-zinc-200 rounded-xl p-5"><div className="flex items-center gap-2 mb-2"><CreditCard className="w-4 h-4 text-[#F26B33]" /><span className="text-xs font-medium text-zinc-500 uppercase">Card</span></div><div className="text-2xl font-bold text-zinc-950">{fmtINR(analytics.totalCard)}</div></div>
                <div className="bg-white border border-zinc-200 rounded-xl p-5"><div className="flex items-center gap-2 mb-2"><Smartphone className="w-4 h-4 text-violet-500" /><span className="text-xs font-medium text-zinc-500 uppercase">UPI</span></div><div className="text-2xl font-bold text-zinc-950">{fmtINR(analytics.totalUpi)}</div></div>
              </div>
              <div className="bg-white border border-zinc-200 rounded-xl p-6" data-testid="s19-chart"><h2 className="text-sm font-semibold text-zinc-700 uppercase tracking-wide mb-4">Collection by Cashier</h2><ResponsiveContainer width="100%" height={Math.max(180, analytics.cashiers.length*50)}><BarChart data={analytics.cashiers} layout="vertical" margin={{ top: 5, right: 10, left: 10, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} /><XAxis type="number" tick={{ fontSize: 10, fill: '#a1a1aa' }} tickLine={false} axisLine={false} tickFormatter={v => v >= 1000 ? `${(v/1000).toFixed(0)}k` : v} /><YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: '#52525b' }} tickLine={false} axisLine={false} width={100} /><ReTooltip /><Legend iconType="circle" iconSize={8} /><Bar dataKey="cash_collected" name="Cash" fill="#329937" stackId="a" /><Bar dataKey="card_collected" name="Card" fill="#F26B33" stackId="a" /><Bar dataKey="upi_collected" name="UPI" fill="#8B5CF6" stackId="a" radius={[0,6,6,0]} /></BarChart></ResponsiveContainer></div>
              <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden" data-testid="s19-table">
                <div className="px-6 py-4 border-b border-zinc-100"><h2 className="text-sm font-semibold text-zinc-700 uppercase tracking-wide">Settlement Details</h2></div>
...
export default CashierSettlementMockup;
