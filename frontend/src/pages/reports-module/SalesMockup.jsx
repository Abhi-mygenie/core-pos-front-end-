...
const SalesMockup = () => {
  ...
  // CR-049: Transform backend response → analytics shape expected by rendering
  const analytics = useMemo(() => {
    ...
    const channels = (salesData.channels || []).map(c => ({ channel: c.channel, revenue: c.revenue || 0, orders: c.orders || 0 }));
    const payments = (salesData.payments || []).map(p => ({ method: p.method, revenue: p.revenue || 0, orders: p.orders || 0 }));
    const hourly = (salesData.hourly || []).map(h => ({ hour: String(h.hour).padStart(2, '0'), revenue: h.revenue || 0, orders: h.orders || 0 }));
    const peakHour = hourly.reduce((best, h) => h.revenue > (best?.revenue || 0) ? h : best, null);

    return {
      totalRevenue,
      totalOrders: s.total_orders || 0,
      totalTax: s.total_tax || 0,
      ...
      allOrders: s.total_orders || 0,
      creditSettled: s.tab_settlement_total || 0,
    };
  }, [salesData]);
