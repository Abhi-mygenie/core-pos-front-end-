 ...
 *  - Title chip says "Item Sales · Hybrid" so reviewer can tell screens apart
 *
...
const ItemSalesHybridMockup = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (activeTab === 'all_items') {
      ...
      // §3: fixed visual order — Sold → Pending Billing → Cancelled → Comp
      const result = [...sold, ...pending, ...cancelled, ...comp];
      // Attach bucket counts for separator rendering
      result.__bucketCounts = { sold: sold.length, pending: pending.length, cancelled: cancelled.length, comp: comp.length };
      return result;
    }
  ...
  const tabCounts = useMemo(() => ({
    ...
    all_items: apiRows.filter(d => d.qty > 0).length + apiRows.filter(d => d.hasPending).length + apiRows.filter(d => d.qtyCancelled > 0).length + apiRows.filter(d => d.isComplimentary).length,
  ...
  const lensFilteredData = useMemo(() => {
    ...
    if (activeTab === 'all_items' || activeTab === 'comp' || activeTab === 'audit') {
  ...
  const bucketSummary = useMemo(() => {
    ...
    const b = { sold: { count: 0, qty: 0, revenue: 0 }, pending: { count: 0, qty: 0, revenue: 0 }, cancelled: { count: 0, qty: 0, revenue: 0 }, comp: { count: 0, qty: 0, revenue: 0 } };
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-zinc-50 relative">
        ...
        {activeTab === 'all_items' && bucketSummary && !isLoading && hasLoadedOnce && (
          <div className="px-8 py-3 bg-white border-b border-zinc-100 shrink-0 space-y-3" data-testid="all-items-summary-panel">
            <div className="grid grid-cols-4 gap-3" data-testid="bucket-summary-bar">
            ...
            <div className="flex-1 bg-white border border-zinc-200 rounded-xl shadow-sm overflow-hidden flex flex-col" data-testid="audit-tab-panel">
              <div className="px-6 py-4 border-b border-zinc-200 bg-gradient-to-r from-amber-50 to-white">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h2 className="text-base font-semibold text-zinc-900">Audit · S5 Item Sales Hybrid</h2>
                </div>
                ...
                <thead>
                  <tr className="bg-zinc-50/80 border-b border-zinc-200">
                    ...
                    <th className="px-5 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider text-right cursor-pointer hover:bg-zinc-100 transition-colors" onClick={() => handleSort('qty')}>
                      {activeTab === 'all_items' ? 'Qty' : activeTab === 'cancelled' ? 'Cancelled Qty' : activeTab === 'comp' ? 'Comp Qty' : activeTab === 'pending' ? 'Pending Qty' : 'Qty Sold'} {getSortIcon('qty')}
                    </th>
                    ...
                    <th className="px-5 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider text-right cursor-pointer hover:bg-zinc-100 transition-colors" onClick={() => handleSort('totalRevenue')}>
                      {activeTab === 'all_items' ? 'Revenue' : activeTab === 'cancelled' || activeTab === 'comp' ? 'Lost Revenue' : activeTab === 'pending' ? 'Unbilled Revenue' : 'Total Revenue'} {getSortIcon('totalRevenue')}
                    </th>
