    ...
    return {
      dineIn: {
        ...
        items: [
          ...
          //
          // CR (May-2026) channel-view stability: `statusMatchesFilter` was
          // dropped from channel-view membership — status-chip selection
          // (`activeStatuses`) must NOT remove a card from a channel
          // column when its status flips. The chip state is still
          // maintained and continues to feed the Header pulse counters /
          // status view; only this membership filter is decoupled.
          // `platformMatches` (POS2-002 Phase 3) is orthogonal and kept.
          ...allTablesList.filter(t => !t.isRoom && !t.isWalkIn).map(enrichTable).filter(platformMatches),
          ...walkInOrders.map(adaptWalkIn).filter(platformMatches),
        ],
      ...
      takeAway: {
        id: 'takeAway',
        name: 'TakeAway',
        items: takeAwayOrders.map(o => adaptOrder(o, 'takeAway')).filter(platformMatches),
        enabled: features.takeaway !== false,
      },
      delivery: {
        id: 'delivery',
        name: 'Delivery',
        items: deliveryOrders.map(o => adaptOrder(o, 'delivery')).filter(platformMatches),
      ...
      room: {
        ...
        items: allRoomsList.filter(platformMatches),
        enabled: features.room !== false,
      },
    };
  }, [allTablesList, allRoomsList, takeAwayOrders, deliveryOrders, walkInOrders, features, dineInOrders, activeStatuses, platform]);
