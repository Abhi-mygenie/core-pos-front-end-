...
const DashboardPage = () => {
  ...
  // queue. Settles one order at a time with AUTO_SETTLE_DELAY_MS between calls.
  ...
  // CR-097: Single processor — drains queue one item at a time
  const processAutoSettleQueue = useCallback(async () => {
    ...
    while (autoSettleQueue.current.length > 0) {
      const item = autoSettleQueue.current[0]; // peek
  ...
  // CR-097: Enqueue effect — watches orders array, enqueues new auto-settle candidates
  useEffect(() => {
  ...
  // CR-097: Cleanup — clear queue on unmount to prevent stale callbacks
  useEffect(() => {
    return () => {
