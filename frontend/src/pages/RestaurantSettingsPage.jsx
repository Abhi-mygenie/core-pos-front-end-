...
const RestaurantSettingsPage = () => {
  ...
  const { restaurant, setRestaurant } = useRestaurant(); // BUG-337 + BUG-364
  ...
  useEffect(() => {
    getSettings()
      .then((data) => {
        // CR-352 BUG-364: backend's update-settings does not persist printer_agent reliably.
        // Use localStorage as source of truth if the user has saved a value this session/device.
        const savedType = localStorage.getItem('mygenie_printer_type');
        if (savedType) data.step1.printerType = savedType;
        setFormState(data);
      })
  ...
  const saveStep = async () => {
    ...
    try {
      ...
      // CR-352 BUG-364: persist printerType to localStorage so Settings → Printers tile routes correctly
      // (profile API does not include printer_agent; localStorage is the bridge)
      localStorage.setItem('mygenie_printer_type', formState.step1.printerType);
      return true;
    } catch (err) {
          ...
          {currentStep === 4 && (
            <div data-testid="step-4-content">
              ...
              <SectionCard title="GST Configuration" desc="GST affects every bill your restaurant generates">
                ...
                {s4.gstEnabled && (
                  <div className="mt-4"> {/* BUG-359: removed GST Mode, GST Tax%, Tax% — dead fields never used in order flow */}
