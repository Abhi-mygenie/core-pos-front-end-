              ...
              <div className="bg-white rounded-xl border border-[#E5E5E5] p-5">
                ...
                <div data-testid="nb-room-grid" className="grid grid-cols-3 gap-2.5">
                  {roomsLoading ? Array.from({ length: 5 }).map((_, i) => (
                    ...
                    const unavail = r.isOccupied || r.isOoo || r.isHk; // BUG-387
                    const oooBadge = r.isOoo ? { bg: '#FEF3C7', color: '#92400E', text: 'Out of Order' } : r.isHk ? { bg: '#E0F2FE', color: '#0C4A6E', text: 'Needs Cleaning' } : null;
                    return (
                    <button key={r.id} data-testid={`nb-room-pill-${r.id}`}
                      onClick={() => !unavail && setRoomId(r.id)}
                      disabled={unavail}
                      ...
                      <Home className={`w-5 h-5 mb-1.5 ${unavail ? 'text-[#888]' : 'text-[#329937]'}`} />
                      <div className={`font-bold text-[15px] ${unavail ? 'text-[#888]' : 'text-[#1A1A1A]'}`}>{r.tableNo}</div>
                      ...
                      {oooBadge && !r.isOccupied && (
                        <div className="absolute top-2 right-2 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wide" style={{ background: oooBadge.bg, color: oooBadge.color }} data-testid={`nb-room-status-badge-${r.id}`}>{oooBadge.text}</div>
                      )}
                      {!unavail && roomId === r.id && <div className="absolute top-2 right-2 w-5 h-5 bg-[#329937] rounded-full flex items-center justify-center"><Check className="w-3 h-3 text-white" /></div>}
                    </button>
                    );
                  })}
                </div>
