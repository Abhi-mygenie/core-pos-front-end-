...
const STATUS_BAR = { occupied: '#F26B33', occupied_hk: '#F26B33', booked: '#888', hk: '#F59E0B', ooo: '#EF4444', available: '#329937' }; // BUG-397: occupied_hk shares occupied orange (Option A)
const STATUS_LABELS = { occupied: 'Occupied', occupied_hk: 'Occupied · HK', booked: 'Booked', hk: 'Housekeeping', ooo: 'Out of Order', available: 'Available' }; // BUG-397
...
export default function RoomStatusPage() {
  ...
  const handleBulkClean = async () => {
    ...
    const occupiedHk   = hkRooms.filter(r => r.displayStatus === 'occupied' || r.displayStatus === 'occupied_hk'); // BUG-397: both occupied variants cannot be bulk-cleaned
    const cleanableIds = hkRooms.filter(r => r.displayStatus !== 'occupied' && r.displayStatus !== 'occupied_hk').map(r => r.id); // BUG-397
  ...
  return (
    <div data-testid={`rs-tile-${room.id}`}
      ...
      <div className="px-4 pt-3 pb-2">
        ...
        {s === 'occupied_hk' && ( // BUG-397: stayover room — guest inside, HK in progress
          <div className="mt-2">
            <div className="text-[13px] font-medium text-[#1A1A1A]">{room.guest?.name ?? 'Guest'}</div>
            {room.guest?.bookingId && <div className="text-[11px] text-[#888] mt-0.5">{room.guest.bookingId}</div>}
            <div className="text-[11px] mt-0.5 flex items-center gap-1" style={{ color: '#D97706' }}>
              <Clock className="w-3 h-3" />Housekeeping in progress
            </div>
          </div>
        )}
        {s === 'ooo' && (
