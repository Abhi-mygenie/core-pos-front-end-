...
import { RefreshCw, Loader2, AlertCircle, LogOut, Receipt, BedDouble, Calendar } from 'lucide-react'; // CR-362: Calendar
import { toast } from 'sonner';
import Sidebar from '@/components/layout/Sidebar';
import { getReservationOps, extendStay } from '@/api/services/pmsService'; // CR-362: extendStay
...
export default function DeparturesPage() {
  ...
  const [extendTarget, setExtendTarget] = useState(null); // CR-362: { orderId, guestName, channel, roomNo, checkout, currentPrice, nights }
                        ...
                        <tr key={lineId} className="border-b border-[#E5E5E5] hover:bg-[#FAFAFA]" data-testid={`dep-row-${lineId}`}>
                          ...
                          <td className="px-4 py-3">
                            {activeTab === 'checkedOut' ? (
                              ...
                              <div className="flex gap-2"> {/* CR-362 */}
                              ...
                              <button data-testid={`dep-extend-btn-${row.orderId || lineId}`}
                                disabled={!row.orderId}
                                onClick={() => row.orderId && setExtendTarget({ orderId: row.orderId, guestName: row.guestName, channel: row.channel ?? '—', roomNo: tNo, checkout: row.checkout, currentPrice: Number(row.amount ?? 0), nights: row.nights })}
                                className="flex items-center gap-1 px-3 py-1.5 rounded text-[12px] font-medium border border-[#E5E5E5] text-[#666] hover:bg-[#FAFAFA] disabled:opacity-40 disabled:cursor-not-allowed">
