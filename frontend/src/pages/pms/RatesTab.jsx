...
// BUG-450: room types are derived from the fetched rateplans (see roomTypes below) — never a literal list.
const DEFAULT_RESTRICTION = { stop_sell: false, minimum_stay: 1, close_on_arrival: false, close_on_departure: false };
...
// ─── Component ───────────────────────────────────────────────────────────────
// BUG-450: `aiosellRooms` (Room Mapping catalogue, optional) only supplies display names; codes always come from rateplans.
export default function RatesTab({ aiosellRooms = [] }) {
  ...
  const [invForm,       setInvForm]       = useState({}); // BUG-450: lazily keyed by real room codes (rrForm idiom)
  ...
  // BUG-450: distinct room codes from the fetched rateplans (same source Rate Restrictions uses); labels from the Room Mapping name, else the code.
  const roomTypes = useMemo(() => [...new Set((ratesData?.rateplans ?? []).map(p => p.roomCode).filter(Boolean))], [ratesData]);
  const roomLabel = useCallback((code) => aiosellRooms.find(r => r.roomCode === code)?.roomName ?? code, [aiosellRooms]);
  const invFor = (rt) => invForm[rt] ?? DEFAULT_RESTRICTION;
  ...
  const handlePushInvRestrictions = async () => {
    if (roomTypes.length === 0) { toast.error('Load rates first to populate room types'); return; } // BUG-450
              ...
              <table className="w-full border-collapse" style={{ minWidth: 640 }}>
                ...
                <tbody>
                  {ratesData.rateplans.map((plan, ri) => {
                    ...
                    return (
                      <Fragment key={`${plan.roomCode}|${plan.rateplanCode}`}>{/* BUG-450: keyed fragment (was a key-less <>) */}
                          ...
                          <tr key={`grp-${plan.roomCode}`}>
                            <td colSpan={dates.length + 1} className="px-4 py-1.5 bg-[#F7F7F7] border-b border-[#E5E5E5] border-t border-[#E5E5E5]">
                              <span className="text-[10px] font-bold uppercase tracking-widest text-[#666] flex items-center gap-1.5">
                                ...
                                {roomLabel(plan.roomCode)} {/* BUG-450 */}
                              </span>
                            </td>
                          </tr>
                        )}
      ...
      {subTab === 'inv' && (
        <div className="bg-white border border-[#E5E5E5] rounded-xl overflow-hidden" data-testid="rt-inv-form">
          <div className="px-5 py-3 border-b border-[#E5E5E5] flex items-center justify-between">
            ...
            <button onClick={handlePushInvRestrictions} disabled={invPushing || roomTypes.length === 0}
          ...
          <div className="p-4 flex flex-col gap-3">
            {roomTypes.length === 0 && <p className="text-[13px] text-[#888] py-4 text-center" data-testid="rt-inv-empty">Load rates first to populate room types.</p>} {/* BUG-450 */}
            {roomTypes.map(rt => (
              <div key={rt} className="border border-[#E5E5E5] rounded-xl overflow-hidden" data-testid={`rt-inv-card-${rt}`}>
                <div className="px-4 py-2.5 bg-[#FAFAFA] border-b border-[#E5E5E5] flex items-center gap-2">
                  ...
                  <span className="text-[13px] font-semibold" data-testid={`rt-inv-label-${rt}`}>{roomLabel(rt)}</span>
                </div>
                    ...
                    <div key={f.key}>
                      ...
                      {f.type === 'toggle' ? (
                        <div className="flex items-center gap-2 h-9">
                          <button onClick={() => setInvForm(p => ({ ...p, [rt]: { ...(p[rt] ?? DEFAULT_RESTRICTION), [f.key]: !invFor(rt)[f.key] } }))}
                            className={`w-10 h-5 rounded-full relative transition-colors flex-shrink-0 ${invFor(rt)[f.key] ? (f.key==='stop_sell'?'bg-[#EF4444]':'bg-[#329937]') : 'bg-[#E5E5E5]'}`}
                            data-testid={`rt-inv-${rt}-${f.key}`}>
                            <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${invFor(rt)[f.key] ? 'left-[22px]' : 'left-0.5'}`} />
                          </button>
                          <span className={`text-[12px] font-medium ${invFor(rt)[f.key] ? (f.key==='stop_sell'?'text-[#EF4444]':'text-[#329937]') : 'text-[#666]'}`}>
                            {invFor(rt)[f.key] ? 'On' : 'Off'}
                          </span>
                        </div>
                      ) : (
                        <input type="number" min={f.min} value={invFor(rt)[f.key]}
                          onChange={e => setInvForm(p => ({ ...p, [rt]: { ...(p[rt] ?? DEFAULT_RESTRICTION), [f.key]: parseInt(e.target.value)||1 } }))}
