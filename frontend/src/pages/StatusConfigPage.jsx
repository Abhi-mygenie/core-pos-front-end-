...
// successful collect-bill (line ~1492).
import {
...
// PROD-BUG-001 (2026-05-20): Auto Settle toggle.
...
const ALL_STATUSES = [
  ...
  // POS2-005: "Running" (fOrderStatus=8) removed — status 8 is now
  // Hold-classified and surfaced only in the Audit Report Hold tab, not
  // operator-toggleable on the dashboard.
...
const StatusConfigPage = () => {
  ...
  // CR-024: Only show channels that the API says are enabled
  const availableChannels = ALL_CHANNELS.filter(ch => {
  ...
  // Collect Bill" — local toggle state, persisted on Save Configuration.
  ...
  // PROD-BUG-001 (2026-05-20): Auto Settle toggle state.
  const [autoSettleEnabled, setAutoSettleEnabled] = useState(AUTO_SETTLE_FACTORY);

  // CR-010: Weight Entry Prompt toggle state.
  const [weightPromptEnabled, setWeightPromptEnabled] = useState(true);

  // CR-051: Customer field mandatoriness override states
  ...
  // CR-051 amendment: TakeAway Name toggle (default ON — mandatory)
  ...
  useEffect(() => {
    ...
    // CR-008 #4 Phase A / Bucket D1 (May-2026): hydrate "Stay on Order
    // Entry After Collect Bill" toggle. Default OFF preserves today's
    // post-collect-bill redirect to dashboard.
    try {
    ...
    // PROD-BUG-001 (2026-05-20): hydrate Auto Settle toggle
    try {
      const storedAutoSettle = localStorage.getItem(AUTO_SETTLE_KEY);
      if (storedAutoSettle === 'true') setAutoSettleEnabled(true);
    } catch (e) {
      console.error('Failed to read Auto Settle flag:', e);
    }

    // CR-010: hydrate Weight Entry Prompt toggle
    try {
      const storedWeight = localStorage.getItem('mygenie_weight_prompt_enabled');
      setWeightPromptEnabled(storedWeight !== 'false'); // default ON
    } catch (e) {
      console.error('Failed to read Weight Prompt flag:', e);
    }

    // CR-051: hydrate customer field mandatoriness overrides
    try {
      setWalkinNameReq(localStorage.getItem('mygenie_walkin_name_required') === 'true');
      setWalkinPhoneReq(localStorage.getItem('mygenie_walkin_phone_required') === 'true');
      setDineinNameReq(localStorage.getItem('mygenie_dinein_name_required') === 'true');
      setDineinPhoneReq(localStorage.getItem('mygenie_dinein_phone_required') === 'true');
      setTakeawayPhoneReq(localStorage.getItem('mygenie_takeaway_phone_required') === 'true');
      // CR-051 amendment: TakeAway Name — default ON if key absent
      setTakeawayNameReq(localStorage.getItem('mygenie_takeaway_name_required') !== 'false');
    } catch (e) {
      console.error('Failed to read customer field requirements:', e);
    }
  }, []);
  ...
  const resetToDefault = () => {
    ...
    // CR-008 #4 Phase A / Bucket D1 (May-2026): reset stay-on-order toggle
    // to factory (OFF — preserves today's redirect-to-dashboard behavior).
    ...
    // PROD-BUG-001: reset Auto Settle
    setAutoSettleEnabled(AUTO_SETTLE_FACTORY);
    ...
    // CR-051 amendment: TakeAway Name resets to ON (mandatory)
  ...
  const saveConfiguration = async () => {
    ...
    // CR-024: Clean stale channel IDs before saving — only keep API-enabled channels
    const availableIds = availableChannels.map(ch => ch.id);
    const cleanedConfig = {
      ...channelConfig,
      channels: channelConfig.channels.filter(id => availableIds.includes(id)),
    };
    ...
    // CR-008 #4 Phase A / Bucket D1 (May-2026): persist stay-on-order toggle.
    // Read site: OrderEntry.jsx (post Collect Bill success, line ~1492).
    ...
    // PROD-BUG-001: persist Auto Settle toggle
    localStorage.setItem(AUTO_SETTLE_KEY, autoSettleEnabled ? 'true' : 'false');
    // CR-010: persist Weight Entry Prompt toggle
    localStorage.setItem('mygenie_weight_prompt_enabled', weightPromptEnabled ? 'true' : 'false');

    // CR-051: persist customer field mandatoriness overrides
    localStorage.setItem('mygenie_walkin_name_required', walkinNameReq ? 'true' : 'false');
    localStorage.setItem('mygenie_walkin_phone_required', walkinPhoneReq ? 'true' : 'false');
    localStorage.setItem('mygenie_dinein_name_required', dineinNameReq ? 'true' : 'false');
    localStorage.setItem('mygenie_dinein_phone_required', dineinPhoneReq ? 'true' : 'false');
    localStorage.setItem('mygenie_takeaway_phone_required', takeawayPhoneReq ? 'true' : 'false');
    // CR-051 amendment: persist TakeAway Name toggle
    ...
    if (nextEnabled && nextStations.length > 0) {
      try {
        ...
        toast({ title: "Error", description: e.readableMessage || e.message, variant: "destructive" });
      }
    } else {
        ...
        <main className="flex-1 overflow-auto p-6">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mt-10 pt-8" style={{ borderTop: `2px solid ${COLORS.borderGray}` }}>
              ...
              {/* CR-008 #4 Phase A / Bucket D1 (May-2026): "Stay on Order
                  Entry After Collect Bill" — when ON, the cashier stays on
                  OrderEntry (walk-in, fresh cart) after a successful bill
                  collection instead of returning to the dashboard. Persists
                  via the page-level Save Configuration button. */}
              <div
                className="rounded-lg p-4 border-2 flex items-start justify-between gap-4 mt-3"
                style={{
                  backgroundColor: stayOnOrderAfterBill ? `${COLORS.primaryGreen}05` : COLORS.lightBg,
                  borderColor: stayOnOrderAfterBill ? COLORS.primaryGreen : COLORS.borderGray,
                }}
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium" style={{ color: COLORS.darkText }}>
                      Stay on Order Entry After Collect Bill
                    </span>
                    <span
                      className="text-xs px-2 py-0.5 rounded-full"
                      style={{
                        backgroundColor: stayOnOrderAfterBill ? COLORS.primaryGreen : COLORS.grayText,
                        color: '#fff',
                      }}
                    >
                      {stayOnOrderAfterBill ? 'ON' : 'OFF'}
                    </span>
                  </div>
                  <p className="text-xs leading-relaxed" style={{ color: COLORS.grayText }}>
                    When ON, after collecting a bill, the cashier stays on the order entry screen
                    (walk-in, fresh cart) instead of being redirected to the dashboard. Useful for
                    rapid-fire counter or walk-in workflows.
                  </p>
                </div>
                <button
                  data-testid="stay-on-order-after-bill-toggle"
                  onClick={() => { setStayOnOrderAfterBillState(v => !v); setHasChanges(true); }}
                  className="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full transition-colors focus:outline-none"
                  style={{ backgroundColor: stayOnOrderAfterBill ? COLORS.primaryGreen : COLORS.borderGray }}
                  role="switch"
                  aria-checked={stayOnOrderAfterBill}
                >
                  <span
                    className="inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform"
                    style={{ transform: stayOnOrderAfterBill ? 'translateX(22px)' : 'translateX(2px)', marginTop: '2px' }}
                  />
                </button>
              </div>
              ...
              {/* BUG-099: QSR Discount toggle — only visible when QSR mode is ON */}
              {qsrModeEnabled && (
              <div
                className="rounded-lg p-4 border-2 flex items-start justify-between gap-4 mt-3"
                style={{
              ...
              <div
                className="rounded-lg p-4 border-2 flex items-start justify-between gap-4 mt-3"
                style={{
                  backgroundColor: autoSettleEnabled ? `${COLORS.primaryGreen}05` : COLORS.lightBg,
                  borderColor: autoSettleEnabled ? COLORS.primaryGreen : COLORS.borderGray,
                }}
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className="text-xs px-2 py-0.5 rounded-full"
                      style={{
                        backgroundColor: autoSettleEnabled ? COLORS.primaryGreen : COLORS.grayText,
                        color: '#fff',
                      }}
                    >
                      {autoSettleEnabled ? 'ON' : 'OFF'}
                    </span>
                  </div>
                  <p className="text-xs leading-relaxed" style={{ color: COLORS.grayText }}>
                    When ON, prepaid orders that are already paid are automatically settled
                    when they reach the served/delivered stage. Cashier does not need to click
                    Settle. PayLater orders are excluded (require explicit settlement).
                  </p>
                </div>
                <button
                  data-testid="auto-settle-toggle"
                  onClick={() => { setAutoSettleEnabled(v => !v); setHasChanges(true); }}
                  className="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full transition-colors focus:outline-none"
                  style={{ backgroundColor: autoSettleEnabled ? COLORS.primaryGreen : COLORS.borderGray }}
                  role="switch"
                  aria-checked={autoSettleEnabled}
                >
                  <span
                    className="inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform"
                    style={{ transform: autoSettleEnabled ? 'translateX(22px)' : 'translateX(2px)', marginTop: '2px' }}
                  />
                </button>
              </div>

              {/* CR-010: Weight Entry Prompt toggle */}
              <div
                className="rounded-lg p-4 border-2 flex items-start justify-between gap-4 mt-3"
                style={{
                  backgroundColor: weightPromptEnabled ? `${COLORS.primaryGreen}05` : COLORS.lightBg,
                  borderColor: weightPromptEnabled ? COLORS.primaryGreen : COLORS.borderGray,
                }}
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className="text-xs px-2 py-0.5 rounded-full"
                      style={{
                        backgroundColor: weightPromptEnabled ? COLORS.primaryGreen : COLORS.grayText,
                        color: '#fff',
                      }}
                    >
                      {weightPromptEnabled ? 'ON' : 'OFF'}
                    </span>
                  </div>
                  <p className="text-xs leading-relaxed" style={{ color: COLORS.grayText }}>
                    When ON, clicking a weight-based item (Kg/gm/L/ml) shows a weight input
                    prompt before adding to cart. When OFF, item is added with default weight
                    (1 Kg or 1 L).
                  </p>
                </div>
                <button
                  data-testid="weight-prompt-toggle"
                  onClick={() => { setWeightPromptEnabled(v => !v); setHasChanges(true); }}
                  className="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full transition-colors focus:outline-none"
                  style={{ backgroundColor: weightPromptEnabled ? COLORS.primaryGreen : COLORS.borderGray }}
                  role="switch"
                  aria-checked={weightPromptEnabled}
                >
                  <span
                    className="inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform"
                    style={{ transform: weightPromptEnabled ? 'translateX(22px)' : 'translateX(2px)', marginTop: '2px' }}
                  />
                </button>
              </div>
            </div>
            ...
            <div className="mt-6 pt-4" style={{ borderTop: `1px solid ${COLORS.borderGray}` }}>
              ...
              <p className="text-xs mb-3" style={{ color: COLORS.grayText }}>
                Override which customer fields are mandatory per order type. These are per-device settings.
              </p>

              {/* Walk-in Orders */}
              <div className="mb-3">
                <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: COLORS.primaryOrange }}>Walk-in Orders</span>
                <div className="mt-1.5 space-y-1.5">
                  {[
                    { label: 'Name mandatory', checked: walkinNameReq, setter: setWalkinNameReq, testId: 'walkin-name-req-toggle' },
                    { label: 'Phone mandatory', checked: walkinPhoneReq, setter: setWalkinPhoneReq, testId: 'walkin-phone-req-toggle' },
                  ].map(({ label, checked, setter, testId }) => (
                    <div key={testId} className="flex items-center justify-between rounded-lg px-3 py-2 border" style={{ borderColor: checked ? COLORS.primaryGreen : COLORS.borderGray, backgroundColor: checked ? `${COLORS.primaryGreen}05` : '#fff' }}>
                      <span className="text-sm" style={{ color: COLORS.darkText }}>{label}</span>
                      <button data-testid={testId} onClick={() => { setter(v => !v); setHasChanges(true); }} className="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full transition-colors" style={{ backgroundColor: checked ? COLORS.primaryGreen : COLORS.borderGray }} role="switch" aria-checked={checked}>
                        <span className="inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform" style={{ transform: checked ? 'translateX(17px)' : 'translateX(2px)', marginTop: '2px' }} />
                      </button>
                    </div>
                  ))}
                </div>
              ...
              {/* Dine-in Orders */}
              <div className="mb-3">
                <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: COLORS.primaryOrange }}>Dine-in Orders</span>
                <div className="mt-1.5 space-y-1.5">
                  {[
                    { label: 'Name mandatory', checked: dineinNameReq, setter: setDineinNameReq, testId: 'dinein-name-req-toggle' },
                    { label: 'Phone mandatory', checked: dineinPhoneReq, setter: setDineinPhoneReq, testId: 'dinein-phone-req-toggle' },
                  ].map(({ label, checked, setter, testId }) => (
                    <div key={testId} className="flex items-center justify-between rounded-lg px-3 py-2 border" style={{ borderColor: checked ? COLORS.primaryGreen : COLORS.borderGray, backgroundColor: checked ? `${COLORS.primaryGreen}05` : '#fff' }}>
                      <span className="text-sm" style={{ color: COLORS.darkText }}>{label}</span>
                      <button data-testid={testId} onClick={() => { setter(v => !v); setHasChanges(true); }} className="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full transition-colors" style={{ backgroundColor: checked ? COLORS.primaryGreen : COLORS.borderGray }} role="switch" aria-checked={checked}>
              ...
              {/* TakeAway Orders */}
              <div className="mb-1">
                <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: COLORS.primaryOrange }}>TakeAway Orders</span>
                <div className="mt-1.5 space-y-1.5">
                  <div className="flex items-center justify-between rounded-lg px-3 py-2 border" style={{ borderColor: takeawayNameReq ? COLORS.primaryGreen : COLORS.borderGray, backgroundColor: takeawayNameReq ? `${COLORS.primaryGreen}05` : '#fff' }}>
                    <span className="text-sm" style={{ color: COLORS.darkText }}>Name mandatory</span>
                    <button data-testid="takeaway-name-req-toggle" onClick={() => { setTakeawayNameReq(v => !v); setHasChanges(true); }} className="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full transition-colors" style={{ backgroundColor: takeawayNameReq ? COLORS.primaryGreen : COLORS.borderGray }} role="switch" aria-checked={takeawayNameReq}>
                      <span className="inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform" style={{ transform: takeawayNameReq ? 'translateX(17px)' : 'translateX(2px)', marginTop: '2px' }} />
                    </button>
                  </div>
                  <div className="flex items-center justify-between rounded-lg px-3 py-2 border" style={{ borderColor: takeawayPhoneReq ? COLORS.primaryGreen : COLORS.borderGray, backgroundColor: takeawayPhoneReq ? `${COLORS.primaryGreen}05` : '#fff' }}>
                    <span className="text-sm" style={{ color: COLORS.darkText }}>Phone mandatory</span>
                    <button data-testid="takeaway-phone-req-toggle" onClick={() => { setTakeawayPhoneReq(v => !v); setHasChanges(true); }} className="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full transition-colors" style={{ backgroundColor: takeawayPhoneReq ? COLORS.primaryGreen : COLORS.borderGray }} role="switch" aria-checked={takeawayPhoneReq}>
                      <span className="inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform" style={{ transform: takeawayPhoneReq ? 'translateX(17px)' : 'translateX(2px)', marginTop: '2px' }} />
                    </button>
                  </div>
                </div>
              </div>
