...
export function computePlan({ stockInventory, dcrStockSummary, horizonDays, showAll = false }) {
  ...
  const alertRows = stockInventory
    ...
    .map(item => {
      ...
      return {
        ...
        display_qty_text: item.displayQtyText || '', // BUG-455
        small_unit:        item.smallUnit || item.unit || '',  // CR-387
        conversion_factor: Number(item.conversionFactor) || 0, // CR-387
        has_conversion:    !!item.hasUnitConversion && (Number(item.conversionFactor) || 0) > 0 && String(item.displayUnit || '').toLowerCase() !== String(item.smallUnit || item.unit || '').toLowerCase(), // CR-387
        display_qty_parts: item.displayQtyParts || null,       // CR-387
        velocity_per_day: 0,
  ...
  // BUG-B fix: exclude items already in velocityRows OR alertRows — same ingredient_id can appear
  // in both alertRows (stock alert) and inStockRows (gap=0) simultaneously, causing duplicate rows
  // and double entries in the purchase list (BUG-C cascades from BUG-B).
  const alreadyCovered = new Set([
    ...velocityRows.map(r => String(r.ingredient_id)),
    ...alertRows.map(r => String(r.ingredient_id)),
  ]);
  const inStockRows = showAll
    ? rows.filter(r => r.gap >= 0 && !alreadyCovered.has(String(r.ingredient_id)))
          .map(r => ({ ...r, suggest_qty: 0, origin: 'in_stock' }))
    : [];
